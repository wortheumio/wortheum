...
from worthapi.worthnoderpc import WorthNodeRPC

class DebugNode( object ):
   """ Wraps the worths debug node plugin for easier automated testing of the Wortheum Network"""

   def __init__( self, worths, data_dir, args='', plugins=[], apis=[], worths_out=None, worths_err=None ):
      """ Creates a worths debug node.
      ...
      For all other requests, the python-worth library should be used.

      args:
         worths -- The string path to the location of the worths binary
         data_dir -- The string path to an existing worths data directory which will be used to pull blocks from.
         args -- Other string args to pass to worths.
         ...
         worths_stdout -- A stream for worths's stdout. Default is to pipe to /dev/null
         worths_stderr -- A stream for worths's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._worths_bin = None
      self._worths_lock = None
      self._worths_process = None
      self._temp_data_dir = None

      self._worths_bin = Path( worths )
      if( not self._worths_bin.exists() ):
         raise ValueError( 'worths does not exist' )
      if( not self._worths_bin.is_file() ):
         raise ValueError( 'worths is not a file' )

      self._data_dir = Path( data_dir )
      if( not self._data_dir.exists() ):
         raise ValueError( 'data_dir either does not exist or is not a properly constructed worth data directory' )
      ...
      if( worths_out != None ):
         self.worths_out = worths_out
      else:
         self.worths_out = self._FNULL

      if( worths_err != None ):
         self.worths_err = worths_err
      else:
         self.worths_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._worths_lock = Lock()


   def __enter__( self ):
      self._worths_lock.acquire()
      ...
      worths = [ str( self._worths_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      worths.extend( self._args )

      self._worths_process = Popen( worths, stdout=self.worths_out, stderr=self.worths_err )
      self._worths_process.poll()
      sleep( 5 )
      if( not self._worths_process.returncode ):
         self._rpc = WorthNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "worths did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._worths_process != None ):
         self._worths_process.poll()

         if( not self._worths_process.returncode ):
            self._worths_process.send_signal( SIGINT )

            sleep( 7 )
            self._worths_process.poll()

            if( not self._worths_process.returncode ):
               self._worths_process.send_signal( SIGTERM )

               sleep( 5 )
               self._worths_process.poll()

               if( self._worths_process.returncode ):
                  loggin.error( 'worths did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._worths_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._worths_lock.release()
   ...
   def debug_generate_blocks( self, count ):
      ...
      `get_dev_key worth debug`. Do not use this key on the live chain for any reason.
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is WORTH_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both worths and the data directory,
      ...
      parser = ArgumentParser( description='Run a Debug Node on an existing chain. This simply replays all blocks ' + \
                                 ...
                                 'the CLI wallet' )
      parser.add_argument( '--worths', '-s', type=str, required=True, help='The location of a worths binary to run the debug node' )
      ...
      worths = Path( args.worths )
      if( not worths.exists() ):
         print( 'Error: worths does not exist.' )
         return

      worths = worths.resolve()
      if( not worths.is_file() ):
         print( 'Error: worths is not a file.' )
         return

      data_dir = Path( args.data_dir )
      if( not data_dir.exists() ):
         print( 'Error: data_dir does not exist or is not a properly constructed worths data directory' )
      ...
      debug_node = DebugNode( str( worths ), str( data_dir ), worths_err=sys.stderr )
