...
import worth_utils.utils


LOG_LEVEL = logging.INFO
LOG_FORMAT = "%(asctime)-15s - %(name)s - %(levelname)s - %(message)s"
MAIN_LOG_PATH = "./wps_list_proposal.log"
...
def create_proposals(node_client, creator_account, receiver_account):
    ...
    start_date, end_date = worth_utils.utils.get_isostr_start_end_date(now, 10, 2)
    ...
    node_client.commit.post("Worthpy proposal title", "Worthpy proposal body", creator["name"], permlink = "worthpy-proposal-title", tags = "proposals")

    logger.info("Creating proposals...")
    for start_end_subject in START_END_SUBJECTS:
        start_date, end_date = worth_utils.utils.get_isostr_start_end_date(now, start_end_subject[0], start_end_subject[1])
