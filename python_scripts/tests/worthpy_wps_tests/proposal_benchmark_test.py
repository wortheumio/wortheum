...
#example A:
#"/home/a/worths" Path to worths exe
# "/home/a/data"  Path to blockchain directory
# "./worth_utils/resources/config.ini.in" Path to config.ini.in - usually ./worth_utils/resources/config.ini.in
#  1 - number of proposals for every account
#  200 - number of accounts
#  200000.000- number of WORTH's for every account
#  30000.000- number of WBD's for every account
...
#  ./proposal_benchmark_test.py "/home/a/worths" "/home/a/data" "./worth_utils/resources/config.ini.in" initminer 1 200 200000.000 30000.000 100000.000
...
#  ./proposal_benchmark_test.py "/home/a/worths" "/home/a/data" "./worth_utils/resources/config.ini.in" initminer 2 300 200000.000 30000.000 100000.000

#Time[ms] is saved in `r_advanced_benchmark.json` ( position{"op_name": "wps_processor"} ) in directory where this script is called
...
delayed_blocks = 1
delayed_blocks_ex = 3
WPS_API_SINGLE_QUERY_LIMIT = 1000
WORTH_PROPOSAL_MAX_IDS_NUMBER = 5

LOG_LEVEL = logging.INFO
LOG_FORMAT = "%(asctime)-15s - %(name)s - %(levelname)s - %(message)s"
MAIN_LOG_PATH = "./wps_benchmark_test.log"
...
def create_accounts(node, creator, accounts):
    for account in accounts:
        ...
        node.commit.create_account(account['name'], 
            ...
            creator=creator,
            asset='TESTS')
    worth_utils.worth_tools.wait_for_blocks_produced(delayed_blocks, node.url)
...
def transfer_assets_to_accounts(node, from_account, accounts, worths, wbds):
    for acnt in accounts:
        logger.info("Transfer from {} to {} amount {} {}".format(from_account, acnt['name'], worths, "TESTS"))
        node.commit.transfer(acnt['name'], worths, "TESTS", memo = "initial transfer", account = from_account)
    worth_utils.worth_tools.wait_for_blocks_produced(delayed_blocks, node.url)
    for acnt in accounts:
        logger.info("Transfer from {} to {} amount {} {}".format(from_account, acnt['name'], wbds, "TBD"))
        node.commit.transfer(acnt['name'], wbds, "TBD", memo = "initial transfer", account = from_account)
    worth_utils.worth_tools.wait_for_blocks_produced(delayed_blocks, node.url)
...
def create_posts(node, accounts):
    ...
    for acnt in accounts:
        ...
        i += 1
        if ( i % data_size ) == 0:
            worth_utils.worth_tools.wait_for_blocks_produced(delayed_blocks_ex, node.url)
...
def generate_dates( node ):
    ...
    #because of WORTH_PROPOSAL_MAINTENANCE_CLEANUP
...
def list_proposals(node, ids, limit ):
    ...
    last_id = None
    while cnt < limit:
        nr_proposals = ( limit - cnt if limit - cnt <= WPS_API_SINGLE_QUERY_LIMIT else WPS_API_SINGLE_QUERY_LIMIT )
...
def vote_proposals(node, ids, accounts ):
    ...
    proposal_limit = WORTH_PROPOSAL_MAX_IDS_NUMBER
...
def get_block_time( node ):
    global_properties = worth_utils.worth_tools.get_dynamic_global_properties( node.url )
...
if __name__ == '__main__':
    ...
    parser.add_argument("path", type=str, help = "Path to worths")
    parser.add_argument("dir", type=str, help = "Path to blockchain directory")
    parser.add_argument("ini", type=str, help = "Path to config.ini.in - usually ./worth_utils/resources/config.ini.in")
    ...
    parser.add_argument("worths", type=str, default="200000.000", help = "WORTH's")
    parser.add_argument("wbds", type=str, default="30000.000", help = "WBD's")
    parser.add_argument("vests", type=str, default="100000.000", help = "VEST's")

    args = parser.parse_args()

    node = worth_utils.worth_runner.WorthNode( args.path, args.dir, args.ini )
    ...
    try:
        if node.is_running():
            node_client = Worth(nodes = [node_url], no_broadcast = False, keys = keys)

            create_accounts(node_client, args.creator, accounts)
            transfer_to_vesting(node_client, args.creator, accounts, args.vests )
            transfer_assets_to_accounts(node_client, args.creator, accounts, args.worths, args.wbds )
