...
def test_create_proposal(node, creator_account, receiver_account, wif, subject):
    ...
    s = Worth(nodes = [node], no_broadcast = False, keys = [wif])
    ...
    from worth.account import Account
    try:
    ...
    ret = s.commit.post("Worthpy proposal title", "Worthpy proposal body", creator["name"], permlink = "worthpy-proposal-title", tags = "proposals")
    ...
    assert ret["operations"][0][1]["permlink"] == "worthpy-proposal-title"
...
def test_vote_proposal(node, account, wif, subject):
    ...
    worth_utils.worth_tools.wait_for_blocks_produced(2, node)
...
def test_iterate_results_test(node, creator_account, receiver_account, wif, subject, remove):
    ...
    worth_utils.worth_tools.wait_for_blocks_produced(5, node)
    ...
    if remove:
        ...
        for a in range(0, 2):
            ...
            worth_utils.worth_tools.wait_for_blocks_produced(3, node)
...
if __name__ == '__main__':
    ...
    try:
        if node is None or node.is_running():
            ...
            worth_utils.worth_tools.wait_for_blocks_produced(2, node_url)
