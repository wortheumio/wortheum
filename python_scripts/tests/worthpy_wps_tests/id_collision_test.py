...
MAIN_LOG_PATH = "./wps_id_collision_test.log"
...
class ProposalsCreatorThread(threading.Thread):
    def __init__(self, node_url, proposals, private_key, delay):
        ...
        self.node_client = Worth(nodes = [self.node_url], keys = [self.private_key])
...
def list_proposals_by_node(creator, private_key, nodes, subjects):
    for idx in range(0, len(nodes)):
        ...
        s = Worth(nodes = [node], keys = [private_key])
...
if __name__ == "__main__":
    ...
    parser.add_argument("nodes_url", type=str, nargs="+", help="Url of working worth node")
    ...
    node_client = Worth(nodes = args.nodes_url, keys = [args.wif])
    logger.info("New post ==> ({},{},{},{},{})".format(
        "Worthpy proposal title [{}]".format(args.creator), 
        "Worthpy proposal body [{}]".format(args.creator), 
        args.creator, 
        get_permlink(args.creator), 
        "proposals"
    ))

    node_client.commit.post("Worthpy proposal title [{}]".format(args.creator), 
        "Worthpy proposal body [{}]".format(args.creator), 
        args.creator, 
    ...
    worth_utils.worth_tools.wait_for_blocks_produced(5, args.nodes_url[0])
    ...
    for idx in range(0, len(node_subjects)):
        ...
        s = Worth(nodes = [node], keys = [args.wif])
