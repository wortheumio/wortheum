...
[ $? -ne 0 ] && echo FATAL: worths not running? && exit -1
...
pyresttest $NODE:$RPC_PORT ./wps_api/wps_api_test.yaml --import_extensions='validator_ex;'$COMPARATOR
[ $? -ne 0 ] && EXIT_CODE=-1

pyresttest $NODE:$RPC_PORT ./condenser_api/wps_via_condenser_test.yaml --import_extensions='validator_ex;'$COMPARATOR
[ $? -ne 0 ] && EXIT_CODE=-1
