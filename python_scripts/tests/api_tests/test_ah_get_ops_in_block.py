...
from jsonsocket import worths_call
...
def get_last_block(url, max_tries=10, timeout=0.1):
  ...
  status, response = worths_call(url, data=request, max_tries=max_tries, timeout=timeout)
...
def compare_results(f_block, l_block, url1, url2, max_tries=10, timeout=0.1):
  ...
  for i in range(f_block, l_block+1):
    ...
    #with ProcessPoolExecutor(max_workers=2) as executor:
      future1 = executor.submit(worths_call, url1, data=request, max_tries=max_tries, timeout=timeout)
      future2 = executor.submit(worths_call, url2, data=request, max_tries=max_tries, timeout=timeout)

    status1, json1 = future1.result()
    status2, json2 = future2.result()
    
    #status1, json1 = worths_call(url1, data=request, max_tries=max_tries, timeout=timeout)
    #status2, json2 = worths_call(url2, data=request, max_tries=max_tries, timeout=timeout)
