...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db->get_account( WORTH_INIT_MINER_NAME );
      asset init_starting_balance = init.balance;

      account_create_operation op;

      op.new_account_name = "alice";
      op.creator = WORTH_INIT_MINER_NAME;
      ...
      BOOST_REQUIRE( acct.wbd_balance.amount.value == ASSET( "0.000 TBD" ).amount.value );
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db->get_account( WORTH_INIT_MINER_NAME ).balance.amount + 1, WORTH_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      sign( tx, init_account_priv_key );
      ...
      fund( WORTH_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );
      vest( WORTH_INIT_MINER_NAME, WORTH_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );

      BOOST_TEST_MESSAGE( "--- Test account creation with temp account does not set recovery account" );
      op.creator = WORTH_TEMP_ACCOUNT;
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      WORTH_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      WORTH_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      sign( tx, active_key );
      sign( tx, active_key );
      WORTH_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      sign( tx, active_key );
      WORTH_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / WORTH_BLOCK_INTERVAL );
      ...
      WORTH_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      tx.signatures.clear();
      sign( tx, bob_private_key );
      WORTH_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db->head_block_time() + fc::seconds( WORTH_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + WORTH_CASHOUT_WINDOW_SECONDS );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + WORTH_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / WORTH_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = worth::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + WORTH_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / WORTH_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      vest( WORTH_INIT_MINER_NAME, "alice", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + WORTH_MIN_TRANSACTION_EXPIRATION_LIMIT );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_block();
      vote.weight = -1 * WORTH_100_PERCENT;
      tx.clear();
      tx.operations.push_back( vote );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      generate_blocks( WORTH_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / WORTH_BLOCK_INTERVAL );
      ...
      generate_blocks( WORTH_CASHOUT_WINDOW_SECONDS / WORTH_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         int64_t max_vote_denom = ( db->get_dynamic_global_properties().vote_power_reserve_rate * WORTH_VOTING_MANA_REGENERATION_SECONDS ) / (60*60*24);

         BOOST_REQUIRE( alice.last_vote_time == db->head_block_time() );
         BOOST_REQUIRE( alice_comment.net_rshares.value == ( old_mana - alice.voting_manabar.current_mana ) - WORTH_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + WORTH_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == ( old_mana - alice.voting_manabar.current_mana ) - WORTH_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();

         BOOST_TEST_MESSAGE( "--- Test reduced power for quick voting" );

         generate_blocks( db->head_block_time() + WORTH_MIN_VOTE_INTERVAL_SEC );

         util::manabar old_manabar = db->get_account( "alice" ).voting_manabar;
         util::manabar_params params( util::get_effective_vesting_shares( db->get_account( "alice" ) ), WORTH_VOTING_MANA_REGENERATION_SECONDS );
         ...
         BOOST_REQUIRE( bob_comment.net_rshares.value == ( old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana ) - WORTH_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + WORTH_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         generate_blocks( db->head_block_time() + fc::seconds( ( WORTH_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
         ...
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + ( old_mana - new_bob.voting_manabar.current_mana ) - WORTH_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( new_alice_comment.cashout_time == new_alice_comment.created + WORTH_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         op.weight = -1 * WORTH_100_PERCENT / 2;
         op.voter = "sam";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         util::manabar_params downvote_params( util::get_effective_vesting_shares( db->get_account( "alice" ) ) / 4, WORTH_VOTING_MANA_REGENERATION_SECONDS );
         old_downvote_manabar.regenerate_mana( downvote_params, db->head_block_time() );
         int64_t sam_weight = old_downvote_manabar.current_mana - db->get_account( "sam" ).downvote_manabar.current_mana - WORTH_VOTE_DUST_THRESHOLD;
         ...
         BOOST_REQUIRE( new_bob_comment.cashout_time == new_bob_comment.created + WORTH_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         int64_t regenerated_power = (WORTH_100_PERCENT * ( db->head_block_time() - db->get_account( "alice").last_vote_time ).to_seconds() ) / WORTH_VOTING_MANA_REGENERATION_SECONDS;
         ...
         int64_t new_rshares = ( ( fc::uint128_t( db->get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / WORTH_100_PERCENT ).to_uint64() - WORTH_VOTE_DUST_THRESHOLD;

         BOOST_REQUIRE( db->get_comment( "alice", string( "foo" ) ).cashout_time == db->get_comment( "alice", string( "foo" ) ).created + WORTH_CASHOUT_WINDOW_SECONDS );
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( WORTH_1_PERCENT * 25 * ( get_voting_power( new_alice ) ) / WORTH_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana - WORTH_VOTE_DUST_THRESHOLD;
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( uint64_t( WORTH_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / WORTH_100_PERCENT;
         ...
         new_rshares = old_downvote_manabar.current_mana - db->get_account( "alice" ).downvote_manabar.current_mana - WORTH_VOTE_DUST_THRESHOLD;
...

         {
            ...
            auto max_vote_denom = db->get_dynamic_global_properties().vote_power_reserve_rate * WORTH_VOTING_MANA_REGENERATION_SECONDS;
...

         {
            const auto& bob_comment = db->get_comment( "bob", string( "foo" ) );
            const auto& alice = db->get_account( "alice" );
            BOOST_REQUIRE( bob_comment.net_rshares == old_net_rshares - alice_weight + WORTH_VOTE_DUST_THRESHOLD );
            BOOST_REQUIRE( bob_comment.abs_rshares == old_abs_rshares + alice_weight - WORTH_VOTE_DUST_THRESHOLD );
            BOOST_REQUIRE( alice_bob_vote->rshares == -1 * ( alice_weight - WORTH_VOTE_DUST_THRESHOLD ) );
            BOOST_REQUIRE( alice_bob_vote->last_update == db->head_block_time() );
            BOOST_REQUIRE( alice_bob_vote->vote_percent == op.weight );
         ...
         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - WORTH_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + WORTH_BLOCK_INTERVAL ), true );
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - WORTH_VOTE_DUST_THRESHOLD;
         new_rshares = ( new_rshares * ( WORTH_UPVOTE_LOCKOUT_SECONDS - WORTH_BLOCK_INTERVAL ) ) / WORTH_UPVOTE_LOCKOUT_SECONDS;
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - WORTH_VOTE_DUST_THRESHOLD;
         new_rshares = ( new_rshares * ( WORTH_UPVOTE_LOCKOUT_SECONDS - WORTH_BLOCK_INTERVAL - WORTH_BLOCK_INTERVAL ) ) / WORTH_UPVOTE_LOCKOUT_SECONDS;
...
BOOST_AUTO_TEST_CASE( transfer_validate )
{
   try
   {
      ...
      op.to = "bob";
      op.memo = "Memo";
      op.amount = asset( 100, WORTH_SYMBOL );
      op.validate();
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure transfering WORTH to worth.dao" );
      op.from = "bob";
      op.to = WORTH_TREASURY_ACCOUNT;
      op.amount = ASSET( "1.000 TESTS" );
      tx.signatures.clear();
      tx.operations.clear();
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      WORTH_REQUIRE_THROW( db->push_transaction( tx ), fc::exception );

      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WORTH_TREASURY_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      validate_database();

      BOOST_TEST_MESSAGE( "--- Test transfering WBD to worth.dao" );
      auto treasury_wbd_balance = db->get_account( WORTH_TREASURY_ACCOUNT ).wbd_balance;
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).wbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WORTH_TREASURY_ACCOUNT ).wbd_balance == treasury_wbd_balance + ASSET( "1.000 TBD" ) );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
   try
   {
      ...
      auto vests = asset( gpo.total_vesting_fund_worth.amount, WORTH_SYMBOL );
      ...
      BOOST_REQUIRE( gpo.total_vesting_fund_worth.amount.value == vests.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_shares.amount.value == shares.amount.value );
      validate_database();

      op.to = "bob";
      op.amount = asset( 2000, WORTH_SYMBOL );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( WORTH_VESTING_WITHDRAW_INTERVALS * 2 ) ).value + 1 );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + WORTH_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE( alice.vesting_shares.amount.value == old_vesting_shares.amount.value );
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( WORTH_VESTING_WITHDRAW_INTERVALS * 3 ) ).value + 1 );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + WORTH_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            gpo.current_supply += wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" ) - gpo.total_vesting_fund_worth;
            gpo.total_vesting_fund_worth = wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" );
         });
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = legacy_worth_asset::from_asset( asset(WORTH_MIN_ACCOUNT_CREATION_FEE + 10, WORTH_SYMBOL) );
      op.props.maximum_block_size = WORTH_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == WORTH_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxy == WORTH_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.vesting_shares.amount );
      BOOST_REQUIRE( dave.proxy == WORTH_PROXY_TO_SELF_ACCOUNT );
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = WORTH_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( bob.proxy == WORTH_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( sam.proxy == "dave" );
      ...
      vote.account= "bob";
      vote.witness = WORTH_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( db->get_witness( WORTH_INIT_MINER_NAME ).votes == ( alice.vesting_shares + bob.vesting_shares ).amount );
      validate_database();
      ...
      BOOST_REQUIRE( db->get_witness( WORTH_INIT_MINER_NAME ).votes == bob.vesting_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( custom_json_rate_limit )
{
   try
   {
      ...
      WORTH_REQUIRE_THROW( db->push_transaction( tx, 0 ), plugin_exception );
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
   try
   {
      ...
      feed_publish_operation op;
      op.publisher = "alice";
      op.exchange_rate = price( ASSET( "1.000 TBD" ), ASSET( "1000.000 TESTS" ) ); // 1000 WORTH : 1 WBD
      ...
      BOOST_REQUIRE( alice_witness.wbd_exchange_rate == op.exchange_rate );
      BOOST_REQUIRE( alice_witness.last_wbd_exchange_update == db->head_block_time() );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- Test failure publishing with WBD base symbol" );
      ...
      // BOOST_REQUIRE( std::abs( alice_witness.wbd_exchange_rate.to_real() - op.exchange_rate.to_real() ) < 0.0000005 );
      BOOST_REQUIRE( alice_witness.last_wbd_exchange_update == db->head_block_time() );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( new_bob.wbd_balance.amount.value == ASSET( "7.000 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( new_alice.wbd_balance.amount.value == ASSET( "2.500 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- Test success converting WBD to TESTS" );
      op.owner = "bob";
      ...
      BOOST_REQUIRE( new_bob.balance.amount.value == ASSET( "3.000 TESTS" ).amount.value );
      BOOST_REQUIRE( new_bob.wbd_balance.amount.value == ASSET( "4.000 TBD" ).amount.value );
      ...
      BOOST_REQUIRE( convert_request->conversion_date == db->head_block_time() + WORTH_CONVERSION_DELAY );
...
BOOST_AUTO_TEST_CASE( fixture_convert_checks_balance )
{
   ...
   try
   {
      ...
      fund( "dany", 5000 );
      WORTH_REQUIRE_THROW( convert( "dany", ASSET( "5000.000 TESTS" ) ), fc::exception );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.wbd_balance.amount.value == ASSET( "1000.000 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( alice.wbd_balance.amount.value == ASSET( "0.000 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( WBD_SYMBOL, WORTH_SYMBOL ) );
      ...
      // Alice has order for 15 WBD at a price of 2:3
      // Fill 5 WORTH for 7.5 WBD
      ...
      BOOST_REQUIRE( limit_order_idx.find( boost::make_tuple( "bob", op.orderid ) ) == limit_order_idx.end() );
      BOOST_REQUIRE( alice.balance.amount.value == ASSET( "990.000 TESTS" ).amount.value );
      BOOST_REQUIRE( alice.wbd_balance.amount.value == ASSET( "7.500 TBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "5.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.wbd_balance.amount.value == ASSET( "992.500 TBD" ).amount.value );
      ...
      BOOST_REQUIRE( alice.balance.amount.value == ASSET( "990.000 TESTS" ).amount.value );
      BOOST_REQUIRE( alice.wbd_balance.amount.value == ASSET( "15.000 TBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "10.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.wbd_balance.amount.value == ASSET( "977.500 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( alice.wbd_balance.amount.value == ASSET( "22.500 TBD" ).amount.value );
      ...
      BOOST_REQUIRE( alice.wbd_balance.amount.value == ASSET( "33.500 TBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "25.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.wbd_balance.amount.value == ASSET( "965.500 TBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( alice.wbd_balance.amount.value == ASSET( "45.500 TBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "35.909 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.wbd_balance.amount.value == ASSET( "954.500 TBD" ).amount.value );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( limit_order_create2_apply )
{
   try
   {
      ...
      /// First check validation on price constructor level:
      {
        price broken_price;
        /// Invalid base value
        WORTH_REQUIRE_THROW(broken_price=price(ASSET("0.000 TESTS"), ASSET("1.000 TBD")),
          fc::exception);
        /// Invalid quote value
        WORTH_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TBD")),
          fc::exception);
        /// Invalid symbol (same in base & quote)
        WORTH_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TESTS")),
          fc::exception);
      }
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      vest( WORTH_INIT_MINER_NAME, "bob", asset( 1000, WORTH_SYMBOL ) );
      ...
      generate_blocks( db->head_block_time() + WORTH_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db->head_block_time() + WORTH_OWNER_UPDATE_LIMIT + fc::seconds( WORTH_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db->head_block_time() + WORTH_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - WORTH_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db->head_block_time() + ( WORTH_OWNER_AUTH_RECOVERY_PERIOD - WORTH_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      WORTH_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      WORTH_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      WORTH_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db->head_block_time() + WORTH_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( WORTH_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      WORTH_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      WORTH_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.from = "alice";
      op.to = "bob";
      op.wbd_amount = ASSET( "1.000 TBD" );
      op.worth_amount = ASSET( "1.000 TESTS" );
      op.escrow_id = 0;
      op.agent = "sam";
      op.fee = ASSET( "0.100 TESTS" );
      op.json_meta = "";
      ...
      BOOST_TEST_MESSAGE( "--- failure when wbd symbol != WBD" );
      op.wbd_amount.symbol = WORTH_SYMBOL;
      WORTH_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when worth symbol != WORTH" );
      op.wbd_amount.symbol = WBD_SYMBOL;
      op.worth_amount.symbol = WBD_SYMBOL;
      WORTH_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when fee symbol != WBD and fee symbol != WORTH" );
      op.worth_amount.symbol = WORTH_SYMBOL;
      op.fee.symbol = VESTS_SYMBOL;
      WORTH_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when wbd == 0 and worth == 0" );
      op.fee.symbol = WORTH_SYMBOL;
      op.wbd_amount.amount = 0;
      op.worth_amount.amount = 0;
      WORTH_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when wbd < 0" );
      op.wbd_amount.amount = -100;
      op.worth_amount.amount = 1000;
      WORTH_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when worth < 0" );
      op.wbd_amount.amount = 1000;
      op.worth_amount.amount = -100;
      WORTH_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when from cannot cover wbd amount" );
      signed_transaction tx;
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      ...
      auto alice_worth_balance = alice.balance - op.worth_amount - op.fee;
      auto alice_wbd_balance = alice.wbd_balance - op.wbd_amount;
      auto bob_worth_balance = bob.balance;
      auto bob_wbd_balance = bob.wbd_balance;
      auto sam_worth_balance = sam.balance;
      auto sam_wbd_balance = sam.wbd_balance;
      ...
      BOOST_REQUIRE( escrow.wbd_balance == op.wbd_amount );
      BOOST_REQUIRE( escrow.worth_balance == op.worth_amount );
      ...
      BOOST_REQUIRE( alice.balance == alice_worth_balance );
      BOOST_REQUIRE( alice.wbd_balance == alice_wbd_balance );
      BOOST_REQUIRE( bob.balance == bob_worth_balance );
      BOOST_REQUIRE( bob.wbd_balance == bob_wbd_balance );
      BOOST_REQUIRE( sam.balance == sam_worth_balance );
      BOOST_REQUIRE( sam.wbd_balance == sam_wbd_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      et_op.to = "bob";
      et_op.agent = "sam";
      et_op.worth_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( escrow.wbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( escrow.worth_balance == ASSET( "1.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + WORTH_BLOCK_INTERVAL );
      sign( tx, bob_private_key );
      WORTH_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      ...
      WORTH_REQUIRE_THROW( db->get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + WORTH_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.worth_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db->head_block_time() + WORTH_BLOCK_INTERVAL;
      et_op.escrow_expiration = db->head_block_time() + 2 * WORTH_BLOCK_INTERVAL;
      ...
      BOOST_REQUIRE( escrow.ratification_deadline == et_op.ratification_deadline );
      BOOST_REQUIRE( escrow.escrow_expiration == et_op.escrow_expiration );
      BOOST_REQUIRE( escrow.wbd_balance == et_op.wbd_amount );
      BOOST_REQUIRE( escrow.worth_balance == et_op.worth_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when worth < 0" );
      op.worth_amount.amount = -1;
      WORTH_REQUIRE_THROW( op.validate(), fc::exception );


      BOOST_TEST_MESSAGE( "--- failure when wbd < 0" );
      op.worth_amount.amount = 0;
      op.wbd_amount.amount = -1;
      WORTH_REQUIRE_THROW( op.validate(), fc::exception );


      BOOST_TEST_MESSAGE( "--- failure when worth == 0 and wbd == 0" );
      op.wbd_amount.amount = 0;
      WORTH_REQUIRE_THROW( op.validate(), fc::exception );


      BOOST_TEST_MESSAGE( "--- failure when wbd is not wbd symbol" );
      op.wbd_amount = ASSET( "1.000 TESTS" );
      WORTH_REQUIRE_THROW( op.validate(), fc::exception );


      BOOST_TEST_MESSAGE( "--- failure when worth is not worth symbol" );
      op.wbd_amount.symbol = WBD_SYMBOL;
      op.worth_amount = ASSET( "1.000 TBD" );
      WORTH_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      op.worth_amount = ASSET( "0.100 TESTS" );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).worth_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).worth_balance == ASSET( "0.800 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "0.100 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- failure when releasing more wbd than available" );
      op.worth_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_TEST_MESSAGE( "--- failure when releasing less worth than available" );
      op.worth_amount = ASSET( "0.000 TESTS" );
      op.wbd_amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).worth_balance == ASSET( "0.700 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).worth_balance == ASSET( "0.600 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).worth_balance == ASSET( "0.500 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
      tx.clear();
      op.worth_amount = ASSET( "0.500 TESTS" );
      tx.operations.push_back( op );
      sign( tx, sam_private_key );
      db->push_transaction( tx, 0 );

      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "9.700 TESTS" ) );
      WORTH_REQUIRE_THROW( db->get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).worth_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).worth_balance == ASSET( "0.800 TESTS" ) );
      ...
      tx.clear();
      op.worth_amount = ASSET( "0.600 TESTS" );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "success when amount is WBD" );
      op.amount = ASSET( "1.000 TBD" );
      op.validate();


      BOOST_TEST_MESSAGE( "success when amount is WORTH" );
      op.amount = ASSET( "1.000 TESTS" );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance == ASSET( "10.000 TBD" ) );
      ...
      BOOST_TEST_MESSAGE( "--- Failure when transferring to worth.dao" );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring WORTH to self" );
      op.to = "alice";
      op.amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring WBD to self" );
      op.amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance == ASSET( "9.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_wbd_balance == ASSET( "1.000 TBD" ) );
      validate_database();


      BOOST_TEST_MESSAGE( "--- success transferring WORTH to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring WBD to other" );
      op.amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance == ASSET( "8.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).savings_wbd_balance == ASSET( "1.000 TBD" ) );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Failure withdrawing TESTS to worth.dao" );
      ...
      BOOST_TEST_MESSAGE( "--- Success withdrawing TBD to worth.dao" );
      ...
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_wbd_balance == ASSET( "9.000 TBD" ) );
      validate_database();

      BOOST_TEST_MESSAGE( "--- success withdrawing WORTH to self" );
      op.to = "alice";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id++;
      ...
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).complete == db->head_block_time() + WORTH_SAVINGS_WITHDRAW_TIME );
      validate_database();


      BOOST_TEST_MESSAGE( "--- success withdrawing WBD to self" );
      op.amount = ASSET( "1.000 TBD" );
      op.request_id++;
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_wbd_balance == ASSET( "8.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == op.request_id + 1 );
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).from == op.from );
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).to == op.to );
      BOOST_REQUIRE( to_string( db->get_savings_withdraw( "alice", op.request_id ).memo ) == op.memo );
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing WORTH to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id = 3;
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing WBD to other" );
      op.amount = ASSET( "1.000 TBD" );
      op.request_id = 4;
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_wbd_balance == ASSET( "7.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == op.request_id + 1 );
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).from == op.from );
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).to == op.to );
      BOOST_REQUIRE( to_string( db->get_savings_withdraw( "alice", op.request_id ).memo ) == op.memo );
      ...
      generate_blocks( db->head_block_time() + WORTH_SAVINGS_WITHDRAW_TIME - fc::seconds( WORTH_BLOCK_INTERVAL ), true );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance == ASSET( "1.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).wbd_balance == ASSET( "1.000 TBD" ) );
      ...
      for( int i = 0; i < WORTH_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = WORTH_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      WORTH_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == WORTH_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db->head_block_time() + WORTH_OWNER_AUTH_RECOVERY_PERIOD );
      ...
      vote.weight = WORTH_1_PERCENT * 50;
      tx.clear();
      tx.operations.push_back( vote );
      sign( tx, alice_private_key );
      WORTH_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      op.account = "alice";
      op.reward_worth = ASSET( "0.000 TESTS" );
      op.reward_wbd = ASSET( "0.000 TBD" );
      ...
      op.reward_worth.amount = 1000;
      op.validate();

      op.reward_worth.amount = 0;
      op.reward_wbd.amount = 1000;
      op.validate();

      op.reward_wbd.amount = 0;
      ...
      BOOST_TEST_MESSAGE( "Testing wrong WORTH symbol" );
      op.reward_worth = ASSET( "1.000 TBD" );
      WORTH_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing wrong WBD symbol" );
      op.reward_worth = ASSET( "1.000 TESTS" );
      op.reward_wbd = ASSET( "1.000 TESTS" );
      WORTH_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing wrong VESTS symbol" );
      op.reward_wbd = ASSET( "1.000 TBD" );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Attempting to claim more WORTH than exists in the reward balance." );

      claim_reward_balance_operation op;
      signed_transaction tx;

      op.account = "alice";
      op.reward_worth = ASSET( "20.000 TESTS" );
      op.reward_wbd = ASSET( "0.000 TBD" );
      op.reward_vests = ASSET( "0.000000 VESTS" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_worth + op.reward_worth );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_worth_balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance == alice_wbd + op.reward_wbd );
      ...
      op.reward_worth = ASSET( "10.000 TESTS" );
      op.reward_wbd = ASSET( "10.000 TBD" );
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_authorities )
{
   try
   {
      ...
      signed_transaction tx;
      ACTORS( (alice)(bob) )
      vest( WORTH_INIT_MINER_NAME, "alice", ASSET( "10000.000 TESTS" ) );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      util::manabar_params params( util::get_effective_vesting_shares( db->get_account( "bob" ) ), WORTH_VOTING_MANA_REGENERATION_SECONDS );
      old_manabar.regenerate_mana( params, db->head_block_time() );
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - WORTH_VOTE_DUST_THRESHOLD );
      BOOST_REQUIRE( itr->rshares == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - WORTH_VOTE_DUST_THRESHOLD );

      generate_block();
      ACTORS( (sam)(dave) )
      generate_block();

      vest( WORTH_INIT_MINER_NAME, "sam", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, sam_private_key );
      WORTH_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      BOOST_REQUIRE( gpo.delegation_return_period == WORTH_DELEGATION_RETURN_PERIOD_HF20 );
      ...
      generate_blocks( exp_obj->expiration + WORTH_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), WORTH_100_PERCENT + 1 ) );
      op.extensions.insert( b );
      WORTH_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), WORTH_1_PERCENT * 75 ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), WORTH_1_PERCENT * 75 ) );
      op.extensions.clear();
      op.extensions.insert( b );
      WORTH_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", WORTH_1_PERCENT * 2 ) );
      b.beneficiaries.push_back( beneficiary_route_type( "bob", WORTH_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      WORTH_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", WORTH_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( "alice", WORTH_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      WORTH_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         db.modify( db.get_account( WORTH_TREASURY_ACCOUNT ), [=]( account_object& a )
         {
            a.wbd_balance.amount.value = 0;
         });
      });
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), WORTH_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( WORTH_INIT_MINER_NAME + fc::to_string( i ) ), WORTH_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      WORTH_REQUIRE_THROW( db->push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), WORTH_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * WORTH_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * WORTH_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( WORTH_TREASURY_ACCOUNT ), 10 * WORTH_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * WORTH_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      sign( tx, alice_private_key );
      WORTH_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - WORTH_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_worth;
            gpo.total_reward_fund_worth = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_worth;
         });
      });
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_worth.amount + db->get_account( "bob" ).reward_wbd_balance.amount + db->get_account( "sam" ).reward_vesting_worth.amount + db->get_account( "sam" ).reward_wbd_balance.amount + db->get_account( WORTH_TREASURY_ACCOUNT ).wbd_balance.amount == db->get_comment( "alice", string( "test" ) ).beneficiary_payout_value.amount );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_wbd_balance.amount + db->get_account( "alice" ).reward_vesting_worth.amount + db->get_account( WORTH_TREASURY_ACCOUNT ).wbd_balance.amount ) == db->get_account( "bob" ).reward_vesting_worth.amount + db->get_account( "bob" ).reward_wbd_balance.amount + 1 );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_wbd_balance.amount + db->get_account( "alice" ).reward_vesting_worth.amount + db->get_account( WORTH_TREASURY_ACCOUNT ).wbd_balance.amount ) == ( db->get_account( "sam" ).reward_vesting_worth.amount + db->get_account( "sam" ).reward_wbd_balance.amount ) / 2 + 1 );
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_worth.amount == db->get_account( "bob" ).reward_wbd_balance.amount + 1 );
      BOOST_REQUIRE( db->get_account( "sam" ).reward_vesting_worth.amount == db->get_account( "sam" ).reward_wbd_balance.amount + 1 );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
   try
   {
      ...
      prop_op.owner = "alice";
      WORTH_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting maximum_block_size below WORTH_MIN_BLOCK_SIZE_LIMIT" );
      prop_op.props.erase( "account_creation_fee" );
      prop_op.props[ "maximum_block_size" ] = fc::raw::pack_to_vector( WORTH_MIN_BLOCK_SIZE_LIMIT - 1 );
      WORTH_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting wbd_interest_rate with negative number" );
      prop_op.props.erase( "maximum_block_size" );
      prop_op.props[ "wbd_interest_rate" ] = fc::raw::pack_to_vector( -700 );
      WORTH_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting wbd_interest_rate to WORTH_100_PERCENT + 1" );
      prop_op.props[ "wbd_interest_rate" ].clear();
      prop_op.props[ "wbd_interest_rate" ] = fc::raw::pack_to_vector( WORTH_100_PERCENT + 1 );
      WORTH_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting new wbd_exchange_rate with WBD / WORTH" );
      prop_op.props.erase( "wbd_interest_rate" );
      prop_op.props[ "wbd_exchange_rate" ] = fc::raw::pack_to_vector( price( ASSET( "1.000 TESTS" ), ASSET( "10.000 TBD" ) ) );
      WORTH_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting new url with length of zero" );
      prop_op.props.erase( "wbd_exchange_rate" );
      ...
      uint64_t unit = uint64_t(1) << WORTH_RD_DECAY_DENOM_SHIFT;

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one year" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / WORTH_BLOCKS_PER_YEAR ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one day" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / WORTH_BLOCKS_PER_DAY ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one hour" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / ((60*60)/WORTH_BLOCK_INTERVAL) ) );
      prop_op.validate();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_authorities )
{
   try
   {
      ...
      expected_keys.push_back( authority( 1, WORTH_NULL_ACCOUNT, 1 ) );
      op.get_required_authorities( key_auths );
      BOOST_REQUIRE( key_auths == expected_keys );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
   try
   {
      ...
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.maximum_block_size == WORTH_MIN_BLOCK_SIZE_LIMIT + 1 );

      // Setting wbd_interest_rate
      prop_op.props.erase( "maximum_block_size" );
      prop_op.props[ "wbd_interest_rate" ] = fc::raw::pack_to_vector( 700 );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.wbd_interest_rate == 700 );
      ...
      // Setting new wbd_exchange_rate
      prop_op.props.erase( "new_signing_key" );
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "wbd_exchange_rate" ] = fc::raw::pack_to_vector( price( ASSET(" 1.000 TBD" ), ASSET( "100.000 TESTS" ) ) );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.wbd_exchange_rate == price( ASSET( "1.000 TBD" ), ASSET( "100.000 TESTS" ) ) );
      BOOST_REQUIRE( alice_witness.last_wbd_exchange_update == db->head_block_time() );
      ...
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "account_subsidy_budget" ] = fc::raw::pack_to_vector( WORTH_ACCOUNT_SUBSIDY_PRECISION );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.account_subsidy_budget == WORTH_ACCOUNT_SUBSIDY_PRECISION );

      BOOST_TEST_MESSAGE( "--- Testing setting account subsidy pool cap" );
      uint64_t day_decay = ( uint64_t(1) << WORTH_RD_DECAY_DENOM_SHIFT ) / WORTH_BLOCKS_PER_DAY;
...
BOOST_AUTO_TEST_CASE( claim_account_apply )
{
   try
   {
      ...
      generate_blocks( WORTH_BLOCKS_PER_HOUR/2 );
      ...
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).balance == ASSET( "5.000 TESTS" ) );
      ...
      BOOST_CHECK_EQUAL( c_subs, prev_c_subs - WORTH_ACCOUNT_SUBSIDY_PRECISION );
      BOOST_CHECK_EQUAL( nc_subs, prev_nc_subs );
      ...
      int64_t new_value = prev_c_subs - WORTH_ACCOUNT_SUBSIDY_PRECISION;     // Usage applied before decay
      new_value = new_value
          + 5000                                                             // Budget
          - ((new_value*249617279) >> WORTH_RD_DECAY_DENOM_SHIFT);           // Decay
...
BOOST_AUTO_TEST_CASE( create_claimed_account_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.wbd_balance.amount.value == ASSET( "0.000 TBD" ).amount.value );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( WORTH_TEMP_ACCOUNT ), [&]( account_object& a )
         {
...
BOOST_AUTO_TEST_CASE( account_auth_tests )
{
   try
   {
      ...
      vest( WORTH_INIT_MINER_NAME, "alice" , ASSET( "10.000 TESTS" ) );
      vest( WORTH_INIT_MINER_NAME, "bob" , ASSET( "10.000 TESTS" ) );
      vest( WORTH_INIT_MINER_NAME, "charlie" , ASSET( "10.000 TESTS" ) );
      generate_block();
