...
BOOST_AUTO_TEST_CASE( undo_object_disapear )
{
   try
   {
      ...
      */
      ...
      //WORTH_REQUIRE_THROW( ao.modify( obj1, [&]( account_object& obj ){ obj.name = "name00"; obj.proxy = "proxy00"; } ), boost::exception );
...
BOOST_AUTO_TEST_CASE( undo_key_collision )
{
   try
   {
      ...
      ao.create( [&]( account_object& obj ){ obj.name = "name00"; } );
      WORTH_REQUIRE_THROW( ao.create( [&]( account_object& obj ){ obj.name = "name00"; } ), boost::exception );
...
BOOST_AUTO_TEST_CASE( undo_generate_blocks )
{
   try
   {
      ...
      _data data[4]=
      {
        _data( "bob", "post4", std::string(WORTH_ROOT_POST_PARENT), "pl4", "t4", "b4" ),
        _data( "alice", "post", std::string(WORTH_ROOT_POST_PARENT), "pl", "t", "b" ),
