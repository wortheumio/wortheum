...
BOOST_FIXTURE_TEST_SUITE( proposal_tests, wps_proposal_database_fixture )
...
BOOST_AUTO_TEST_CASE( generating_payments )
{
   try
   {
      ...
      FUND( WORTH_TREASURY_ACCOUNT, ASSET( "5000.000 TBD" ) );
      ...
      vest(WORTH_INIT_MINER_NAME, voter_01, ASSET( "1.000 TESTS" ));
      generate_blocks( 1 );

      //skipping interest generating is necessary
      transfer( WORTH_INIT_MINER_NAME, receiver, ASSET( "0.001 TBD" ));
      generate_block( 5 );
      transfer( WORTH_INIT_MINER_NAME, WORTH_TREASURY_ACCOUNT, ASSET( "0.001 TBD" ) );
      generate_block( 5 );

      const auto& dgpo = db->get_dynamic_global_properties();
      auto old_wbd_supply = dgpo.current_wbd_supply;
      ...
      const account_object& _treasury = db->get_account( WORTH_TREASURY_ACCOUNT );

      {
         BOOST_TEST_MESSAGE( "---Payment---" );

         auto before_creator_wbd_balance = _creator.wbd_balance;
         auto before_receiver_wbd_balance = _receiver.wbd_balance;
         auto before_voter_01_wbd_balance = _voter_01.wbd_balance;
         auto before_treasury_wbd_balance = _treasury.wbd_balance;

         auto next_block = get_nr_blocks_until_maintenance_block();
         generate_blocks( next_block - 1 );
         generate_blocks( 1 );

         auto treasury_wbd_inflation = dgpo.current_wbd_supply - old_wbd_supply;
         auto after_creator_wbd_balance = _creator.wbd_balance;
         auto after_receiver_wbd_balance = _receiver.wbd_balance;
         auto after_voter_01_wbd_balance = _voter_01.wbd_balance;
         auto after_treasury_wbd_balance = _treasury.wbd_balance;

         BOOST_REQUIRE( before_creator_wbd_balance == after_creator_wbd_balance );
         BOOST_REQUIRE( before_receiver_wbd_balance == after_receiver_wbd_balance - hourly_pay );
         BOOST_REQUIRE( before_voter_01_wbd_balance == after_voter_01_wbd_balance );
         BOOST_REQUIRE( before_treasury_wbd_balance == after_treasury_wbd_balance - treasury_wbd_inflation + hourly_pay );
      }
...
BOOST_AUTO_TEST_CASE( generating_payments_01 )
{
   try
   {
      ...
      for( auto item : inits )
      {
         ...
         vest(WORTH_INIT_MINER_NAME, item.account, ASSET( "300.000 TESTS" ));
      }
      ...
      FUND( WORTH_TREASURY_ACCOUNT, ASSET( "5000000.000 TBD" ) );
      //=====================preparing=====================
      ...
      for( auto item : inits )
      {
         ...
         before_tbds[ item.account ] = account.wbd_balance;
      }
      ...
      {
         const account_object& account = db->get_account( item.account );
         auto after_tbd = account.wbd_balance;
...
BOOST_AUTO_TEST_CASE( generating_payments_02 )
{
   try
   {
      ...
      const auto block_interval = fc::seconds( WORTH_BLOCK_INTERVAL );
   ...
   try
   {
      ...
      for( auto item : inits )
      {
         if( item.first == tester02_account )
         {
            ...
            vest(WORTH_INIT_MINER_NAME, item.first, ASSET( "31.000 TESTS" ));
         }
         else
         {
            FUND( item.first, ASSET( "40.000 TESTS" ) );
            FUND( item.first, ASSET( "40.000 TBD" ) );
            vest(WORTH_INIT_MINER_NAME, item.first, ASSET( "30.000 TESTS" ));
         }
      }
      ...
      for( auto item : inits )
      {
         ...
         before_tbds[ item.first ] = account.wbd_balance;
      }
...
BOOST_AUTO_TEST_CASE( proposals_maintenance)
{
   try
   {
      ...
      auto daily_pay = asset( 100, WBD_SYMBOL );
...

      {
         ...
         generate_blocks( start_time + fc::seconds( WORTH_PROPOSAL_MAINTENANCE_CLEANUP ) );
...
BOOST_AUTO_TEST_CASE( proposal_object_apply )
{
   try
   {
      ...
      auto fee = asset( WORTH_TREASURY_FEE, WBD_SYMBOL );
      ...
      const account_object& before_treasury_account = db->get_account(WORTH_TREASURY_ACCOUNT);
      ...
      auto before_alice_wbd_balance = before_alice_account.wbd_balance;
      auto before_bob_wbd_balance = before_bob_account.wbd_balance;
      auto before_treasury_balance = before_treasury_account.wbd_balance;
      ...
      const auto& after_treasury_account = db->get_account(WORTH_TREASURY_ACCOUNT);
      ...
      auto after_alice_wbd_balance = after_alice_account.wbd_balance;
      auto after_bob_wbd_balance = after_bob_account.wbd_balance;
      auto after_treasury_balance = after_treasury_account.wbd_balance;

      BOOST_REQUIRE( before_alice_wbd_balance == after_alice_wbd_balance + fee );
      BOOST_REQUIRE( before_bob_wbd_balance == after_bob_wbd_balance );
...
BOOST_AUTO_TEST_CASE( proposal_vote_object_01_apply )
{
   try
   {
      ...
      auto daily_pay_00 = asset( 100, WBD_SYMBOL );
      auto daily_pay_01 = asset( 101, WBD_SYMBOL );
      auto daily_pay_02 = asset( 102, WBD_SYMBOL );
...
struct create_proposal_data {
      ...
      worth::protocol::asset daily_pay ;
      std::string subject ;
      std::string url     ;
      ...
      create_proposal_data(fc::time_point_sec _start) {
         ...
         daily_pay  = asset( 100, WBD_SYMBOL );
         subject    = "hello";
...
BOOST_AUTO_TEST_CASE( create_proposal_001 )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: create proposal: opration arguments validation - invalid creator" );
      {
         ...
         WORTH_REQUIRE_THROW( create_proposal( "", cpd.receiver, cpd.start_date, cpd.end_date, cpd.daily_pay, alice_private_key ), fc::exception);
...
BOOST_AUTO_TEST_CASE( create_proposal_002 )
{
   try
   {
      ...
      generate_block();
      FUND( cpd.creator, ASSET( "80.000 TBD" ) );
      WORTH_REQUIRE_THROW(create_proposal( cpd.creator, "", cpd.start_date, cpd.end_date, cpd.daily_pay, alice_private_key ), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( create_proposal_003 )
{
   try
   {
      ...
      WORTH_REQUIRE_THROW(create_proposal( cpd.creator, cpd.receiver, cpd.start_date, cpd.end_date, cpd.daily_pay, alice_private_key ), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( create_proposal_005 )
{
   try
   {
      ...
      cpo.daily_pay  = asset( 100, WBD_SYMBOL );
      cpo.subject    = "";
...
BOOST_AUTO_TEST_CASE( create_proposal_008 )
{
   try
   {
      ...
      cpd.daily_pay = asset( -10, WBD_SYMBOL );
      WORTH_REQUIRE_THROW(create_proposal( cpd.creator, cpd.receiver, cpd.start_date, cpd.end_date, cpd.daily_pay, alice_private_key ), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( update_proposal_votes_002 )
{
   try
   {
      ...
      WORTH_REQUIRE_THROW( vote_proposal("carol", proposals, true, carol_private_key), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( update_proposal_votes_004 )
{
   try
   {
      ...
      BOOST_REQUIRE(proposal_1 >= 0);
      std::vector< int64_t > proposals = {proposal_1};
      WORTH_REQUIRE_THROW(vote_proposal("urp", proposals, false, carol_private_key), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( update_proposal_votes_005 )
{
   try
   {
      ...
      std::vector< int64_t > proposals;
      for(int i = 0; i <= WORTH_PROPOSAL_MAX_IDS_NUMBER; i++) {
...
BOOST_AUTO_TEST_CASE( remove_proposal_009 )
{
   try
   {
      ...
      int64_t proposal_1 = create_proposal( cpd.creator, cpd.receiver, cpd.start_date, cpd.end_date, cpd.daily_pay, alice_private_key );
      flat_set<int64_t> proposals = { proposal_1 };
      WORTH_REQUIRE_THROW(remove_proposal(cpd.receiver, proposals, bob_private_key), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( remove_proposal_010 )
{
   try
   {
      ...
      WORTH_REQUIRE_THROW(remove_proposal(cpd.creator, proposals, bob_private_key), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( proposals_maintenance_01 )
{
   try
   {
      ...
      auto threshold = db->get_wps_remove_threshold();
...
BOOST_AUTO_TEST_CASE( proposals_maintenance_02 )
{
   try
   {
      ...
      auto itr_begin_2 = proposals_id.begin() + WORTH_PROPOSAL_MAX_IDS_NUMBER;
...
BOOST_FIXTURE_TEST_SUITE( proposal_tests_performance, wps_proposal_database_fixture_performance )
...
BOOST_AUTO_TEST_CASE( proposals_removing_with_threshold_03 )
{
   try
   {
      ...
      {
         ids.push_back( id );
         if( ids.size() == WORTH_PROPOSAL_MAX_IDS_NUMBER )
         {
            for( auto item : inits )
            {
               ++i;
      ...
      int32_t benchmark_time = get_time( *db, wps_processor::get_removing_name() );
...
BOOST_AUTO_TEST_CASE( generating_payments )
{
   try
   {
      ...
      for( auto item : inits )
      {
         ...
         vest(WORTH_INIT_MINER_NAME, item.account, ASSET( "30.000 TESTS" ));
