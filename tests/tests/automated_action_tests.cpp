...
BOOST_AUTO_TEST_CASE( push_pending_required_actions )
{
   ...
   req_action.account = WORTH_INIT_MINER_NAME;
   req_action.validate();
   db->push_required_action( req_action, db->head_block_time() + WORTH_BLOCK_INTERVAL );
   ...
   BOOST_REQUIRE( pending_execution == db->head_block_time() + WORTH_BLOCK_INTERVAL );

   BOOST_TEST_MESSAGE( "--- Failure pushing past action" );
   BOOST_REQUIRE_THROW( db->push_required_action( req_action, db->head_block_time() - WORTH_BLOCK_INTERVAL ), fc::assert_exception );

   BOOST_TEST_MESSAGE( "--- Success pushing action now" );
   req_action.account = WORTH_TEMP_ACCOUNT;
...
BOOST_AUTO_TEST_CASE( push_pending_optional_actions )
{
   ...
   BOOST_TEST_MESSAGE( "--- Success pushing future action" );
   opt_action.account = WORTH_INIT_MINER_NAME;
   opt_action.validate();
   db->push_optional_action( opt_action, db->head_block_time() + WORTH_BLOCK_INTERVAL );
   ...
   BOOST_TEST_MESSAGE( "--- Failure pushing past action" );
   BOOST_REQUIRE_THROW( db->push_optional_action( opt_action, db->head_block_time() - WORTH_BLOCK_INTERVAL ), fc::assert_exception );

   BOOST_TEST_MESSAGE( "--- Success pushing action now" );
   opt_action.account = WORTH_TEMP_ACCOUNT;
...
{ try {
   ...
   db_plugin->debug_update( [&num_actions, &req_action, &opt_action](database& db)
   {
      ...
      uint64_t action_partition_size = ( db.get_dynamic_global_properties().maximum_block_size * db.get_dynamic_global_properties().required_actions_partition_percent ) / WORTH_100_PERCENT;

      while( block_size < action_partition_size )
      {
         req_action.account = WORTH_INIT_MINER_NAME + fc::to_string( num_actions );
         db.push_required_action( req_action );
   ...
   block.sign( WORTH_INIT_PRIVATE_KEY );
...

   {
      ...
      BOOST_REQUIRE( pending_req_action->execution_time == db->head_block_time() - WORTH_BLOCK_INTERVAL );
      ...
      BOOST_REQUIRE( pending_opt_action->execution_time == db->head_block_time() - WORTH_BLOCK_INTERVAL );
   }
...
{ try {
   ...
   db_plugin->debug_update( [&req_action](database& db)
   {
      ...
      req_action.account = WORTH_NULL_ACCOUNT;
      db.push_required_action( req_action, db.head_block_time() + (2 * WORTH_BLOCK_INTERVAL ) );
   });
...
{ try {
   ...
   for( uint32_t i = 0; i < WORTH_MAX_WITNESSES; i++ )
   {
      generate_block();
   }
   ...
   db_plugin->debug_update( [=]( database& db )
   {
      ...
      {
         example_optional_action opt_action;
         opt_action.account = WORTH_NULL_ACCOUNT;
         o.action = opt_action;
