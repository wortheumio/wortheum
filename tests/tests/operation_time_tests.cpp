...
#include <worth/plugins/rc/rc_objects.hpp>
...
using namespace worth::chain::util;
...
BOOST_AUTO_TEST_CASE( comment_payout_equalize )
{
   try
   {
      ...
      // set a ridiculously high WORTH price ($1 / satoshi) to disable dust threshold
      ...
      for( const auto& author : authors )
      {
         ...
         com.permlink = "mypost";
         com.parent_author = WORTH_ROOT_POST_PARENT;
      ...
      //auto reward_worth = db->get_dynamic_global_properties().total_reward_fund_worth;

      // generate a few blocks to seed the reward fund
      generate_blocks(10);
      //const auto& rf = db->get< reward_fund_object, by_name >( WORTH_POST_REWARD_FUND_NAME );
      //idump( (rf) );
      ...
      for( const auto& author : authors )
      {
         ...
         ilog( "${n} : ${worth} ${wbd}", ("n", author.name)("worth", a.reward_worth_balance)("wbd", a.reward_wbd_balance) );
      }
      for( const auto& voter : voters )
      {
         const account_object& a = db->get_account(voter.name);
         ilog( "${n} : ${worth} ${wbd}", ("n", voter.name)("worth", a.reward_worth_balance)("wbd", a.reward_wbd_balance) );
      }
      */
      ...
      BOOST_CHECK( alice_account.reward_wbd_balance == ASSET( "6236.000 TBD" ) );
      BOOST_CHECK( bob_account.reward_wbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_CHECK( dave_account.reward_wbd_balance == alice_account.reward_wbd_balance );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_payout_dust )
{
   try
   {
      ...
      vote.author = "alice";
      vote.permlink = "test";
      vote.weight = 81 * WORTH_1_PERCENT;
      ...
      vote.voter = "bob";
      vote.author = "bob";
      vote.weight = 59 * WORTH_1_PERCENT;
      ...
      // If comments are paid out independent of order, then the last satoshi of WORTH cannot be divided among them
      const auto rf = db->get< reward_fund_object, by_name >( WORTH_POST_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( reward_funds )
{
   try
   {
      ...
      {
         const auto& post_rf = db->get< reward_fund_object, by_name >( WORTH_POST_REWARD_FUND_NAME );
         const auto& comment_rf = db->get< reward_fund_object, by_name >( WORTH_COMMENT_REWARD_FUND_NAME );

         BOOST_REQUIRE( post_rf.reward_balance.amount == 0 );
         BOOST_REQUIRE( comment_rf.reward_balance.amount > 0 );
         BOOST_REQUIRE( db->get_account( "alice" ).reward_wbd_balance.amount > 0 );
         BOOST_REQUIRE( db->get_account( "bob" ).reward_wbd_balance.amount == 0 );
         validate_database();
      }
...
BOOST_AUTO_TEST_CASE( recent_claims_decay )
{
   try
   {
      ...
      auto alice_vshares = util::evaluate_reward_curve( db->get_comment( "alice", string( "test" ) ).net_rshares.value,
         db->get< reward_fund_object, by_name >( WORTH_POST_REWARD_FUND_NAME ).author_reward_curve,
         db->get< reward_fund_object, by_name >( WORTH_POST_REWARD_FUND_NAME ).content_constant );
      ...
      {
         alice_vshares -= ( alice_vshares * WORTH_BLOCK_INTERVAL ) / WORTH_RECENT_RSHARES_DECAY_TIME_HF19.to_seconds();
         const auto& post_rf = db->get< reward_fund_object, by_name >( WORTH_POST_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      //generate_blocks( db->get_comment( "bob", string( "test" ) ).cashout_time - WORTH_BLOCK_INTERVAL, true );

      auto reward_worth = db->get_dynamic_global_properties().total_reward_fund_worth + ASSET( "1.667 TESTS" );
      ...
      auto bob_wbd_balance = db->get_account( "bob" ).wbd_balance;

      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_worth.amount.value ) / total_rshares2 ).to_uint64(), WORTH_SYMBOL );
      auto bob_comment_discussion_rewards = asset( bob_comment_payout.amount / 4, WORTH_SYMBOL );
      bob_comment_payout -= bob_comment_discussion_rewards;
      auto bob_comment_wbd_reward = db->to_wbd( asset( bob_comment_payout.amount / 2, WORTH_SYMBOL ) );
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, WORTH_SYMBOL) ) * db->get_dynamic_global_properties().get_vesting_share_price();

      BOOST_TEST_MESSAGE( "Cause first payout" );

      generate_block();

      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_worth == reward_worth - bob_comment_payout );
      BOOST_REQUIRE( db->get_comment( "bob", string( "test" ) ).total_payout_value == bob_comment_vesting_reward * db->get_dynamic_global_properties().get_vesting_share_price() + bob_comment_wbd_reward * exchange_rate );
      BOOST_REQUIRE( db->get_account( "bob" ).vesting_shares == bob_vest_shares + bob_comment_vesting_reward );
      BOOST_REQUIRE( db->get_account( "bob" ).wbd_balance == bob_wbd_balance + bob_comment_wbd_reward );
      ...
      bob_wbd_balance = db->get_account( "bob" ).wbd_balance;
      ...
      BOOST_REQUIRE( bob_wbd_balance.amount.value == db->get_account( "bob" ).wbd_balance.amount.value );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}*/
...
BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      generate_blocks( fc::time_point_sec( db->head_block_time().sec_since_epoch() + WORTH_CASHOUT_WINDOW_SECONDS / 2 ), true );
      ...
      generate_blocks( db->get_comment( "bob", string( "test" ) ).cashout_time - ( WORTH_BLOCK_INTERVAL / 2 ), true );
      ...
      auto reward_worth = db->get_dynamic_global_properties().total_reward_fund_worth + ASSET( "2.000 TESTS" );
      auto total_rshares2 = db->get_dynamic_global_properties().total_reward_shares2;
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_worth.amount.value ) / total_rshares2 ).to_uint64(), WORTH_SYMBOL );
      auto bob_comment_vote_rewards = asset( bob_comment_payout.amount / 2, WORTH_SYMBOL );
      bob_comment_payout -= bob_comment_vote_rewards;
      auto bob_comment_wbd_reward = asset( bob_comment_payout.amount / 2, WORTH_SYMBOL ) * exchange_rate;
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, WORTH_SYMBOL ) ) * db->get_dynamic_global_properties().get_vesting_share_price();
      auto unclaimed_payments = bob_comment_vote_rewards;
      auto alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), WORTH_SYMBOL );
      auto alice_vote_vesting = alice_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), WORTH_SYMBOL );
      auto bob_vote_vesting = bob_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), WORTH_SYMBOL );
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_worth.amount.value == reward_worth.amount.value - ( bob_comment_payout + bob_comment_vote_rewards - unclaimed_payments ).amount.value );
      BOOST_REQUIRE( db->get_comment( "bob", string( "test" ) ).total_payout_value.amount.value == ( ( bob_comment_vesting_reward * db->get_dynamic_global_properties().get_vesting_share_price() ) + ( bob_comment_wbd_reward * exchange_rate ) ).amount.value );
      BOOST_REQUIRE( db->get_account( "bob" ).wbd_balance.amount.value == ( bob_wbd_balance + bob_comment_wbd_reward ).amount.value );
      BOOST_REQUIRE( db->get_comment( "alice", string( "test" ) ).net_rshares.value > 0 );
      BOOST_REQUIRE( db->get_comment( "bob", string( "test" ) ).net_rshares.value == 0 );
      ...
      BOOST_REQUIRE( bob_comment_reward.payout.amount.value == bob_comment_wbd_reward.amount.value );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - ( WORTH_BLOCK_INTERVAL / 2 ), true );
      ...
      reward_worth = db->get_dynamic_global_properties().total_reward_fund_worth + ASSET( "2.000 TESTS" );
      ...
      auto alice_wbd_balance = db->get_account( "alice" ).wbd_balance;
      ...
      u256 rf( reward_worth.amount.value );
      u256 trs2 = total_rshares2.hi;
      trs2 = ( trs2 << 64 ) + total_rshares2.lo;
      auto rs2 = rs*rs;

      auto alice_comment_payout = asset( static_cast< uint64_t >( ( rf * rs2 ) / trs2 ), WORTH_SYMBOL );
      auto alice_comment_vote_rewards = asset( alice_comment_payout.amount / 2, WORTH_SYMBOL );
      alice_comment_payout -= alice_comment_vote_rewards;
      auto alice_comment_wbd_reward = asset( alice_comment_payout.amount / 2, WORTH_SYMBOL ) * exchange_rate;
      auto alice_comment_vesting_reward = ( alice_comment_payout - asset( alice_comment_payout.amount / 2, WORTH_SYMBOL ) ) * db->get_dynamic_global_properties().get_vesting_share_price();
      unclaimed_payments = alice_comment_vote_rewards;
      alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), WORTH_SYMBOL );
      alice_vote_vesting = alice_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), WORTH_SYMBOL );
      bob_vote_vesting = bob_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), WORTH_SYMBOL );
      sam_vote_vesting = sam_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto dave_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "dave" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), WORTH_SYMBOL );
      ...
      BOOST_REQUIRE( ( db->get_dynamic_global_properties().total_reward_fund_worth + alice_comment_payout + alice_comment_vote_rewards - unclaimed_payments ).amount.value == reward_worth.amount.value );
      BOOST_REQUIRE( db->get_comment( "alice", string( "test" ) ).total_payout_value.amount.value == ( ( alice_comment_vesting_reward * db->get_dynamic_global_properties().get_vesting_share_price() ) + ( alice_comment_wbd_reward * exchange_rate ) ).amount.value );
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance.amount.value == ( alice_wbd_balance + alice_comment_wbd_reward ).amount.value );
      BOOST_REQUIRE( db->get_comment( "alice", string( "test" ) ).net_rshares.value == 0 );
      BOOST_REQUIRE( db->get_comment( "alice", string( "test" ) ).net_rshares.value == 0 );
      ...
      BOOST_REQUIRE( alice_comment_reward.payout.amount.value == alice_comment_wbd_reward.amount.value );
      ...
      bob_vest_shares = db->get_account( "bob" ).vesting_shares;
      auto bob_wbd = db->get_account( "bob" ).wbd_balance;
      ...
      BOOST_REQUIRE( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "dave" ) ).id ) ) == vote_idx.end() );
      BOOST_REQUIRE( bob_vest_shares.amount.value == db->get_account( "bob" ).vesting_shares.amount.value );
      BOOST_REQUIRE( bob_wbd.amount.value == db->get_account( "bob" ).wbd_balance.amount.value );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
OOST_AUTO_TEST_CASE( nested_comments )
{
   try
   {
      ...
      vote_op.weight = WORTH_100_PERCENT;
      ...
      tx.operations.push_back( vote_op );
      vote_op.author = "dave";
      vote_op.weight = WORTH_1_PERCENT * 20;
      tx.operations.push_back( vote_op );
      sign( tx, bob_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - fc::seconds( WORTH_BLOCK_INTERVAL ), true );

      auto gpo = db->get_dynamic_global_properties();
      uint128_t reward_worth = gpo.total_reward_fund_worth.amount.value + ASSET( "2.000 TESTS" ).amount.value;
      ...
      auto alice_comment_reward = ( ( reward_worth * alice_comment.net_rshares.value * alice_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( alice_comment.net_rshares.value ) * ( alice_comment.net_rshares.value );
      reward_worth -= alice_comment_reward;
      auto alice_comment_vote_rewards = alice_comment_reward / 2;
      alice_comment_reward -= alice_comment_vote_rewards;

      auto alice_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), WORTH_SYMBOL );
      auto bob_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), WORTH_SYMBOL );
      reward_worth += alice_comment_vote_rewards - ( alice_vote_alice_reward + bob_vote_alice_reward ).amount.value;

      auto bob_comment_reward = ( ( reward_worth * bob_comment.net_rshares.value * bob_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( bob_comment.net_rshares.value ) * bob_comment.net_rshares.value;
      reward_worth -= bob_comment_reward;
      auto bob_comment_vote_rewards = bob_comment_reward / 2;
      bob_comment_reward -= bob_comment_vote_rewards;

      auto alice_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), WORTH_SYMBOL );
      auto bob_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), WORTH_SYMBOL );
      auto sam_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), WORTH_SYMBOL );
      reward_worth += bob_comment_vote_rewards - ( alice_vote_bob_reward + bob_vote_bob_reward + sam_vote_bob_reward ).amount.value;

      auto dave_comment_reward = ( ( reward_worth * dave_comment.net_rshares.value * dave_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( dave_comment.net_rshares.value ) * dave_comment.net_rshares.value;
      reward_worth -= dave_comment_reward;
      auto dave_comment_vote_rewards = dave_comment_reward / 2;
      dave_comment_reward -= dave_comment_vote_rewards;

      auto bob_vote_dave_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "dave", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * dave_comment_vote_rewards ) / dave_comment.total_vote_weight ), WORTH_SYMBOL );
      reward_worth += dave_comment_vote_rewards - bob_vote_dave_reward.amount.value;

      // Calculate rewards paid to parent posts
      auto alice_pays_alice_wbd = alice_comment_reward / 2;
      auto alice_pays_alice_vest = alice_comment_reward - alice_pays_alice_wbd;
      auto bob_pays_bob_wbd = bob_comment_reward / 2;
      auto bob_pays_bob_vest = bob_comment_reward - bob_pays_bob_wbd;
      auto dave_pays_dave_wbd = dave_comment_reward / 2;
      auto dave_pays_dave_vest = dave_comment_reward - dave_pays_dave_wbd;

      auto bob_pays_alice_wbd = bob_pays_bob_wbd / 2;
      auto bob_pays_alice_vest = bob_pays_bob_vest / 2;
      bob_pays_bob_wbd -= bob_pays_alice_wbd;
      bob_pays_bob_vest -= bob_pays_alice_vest;

      auto dave_pays_sam_wbd = dave_pays_dave_wbd / 2;
      auto dave_pays_sam_vest = dave_pays_dave_vest / 2;
      dave_pays_dave_wbd -= dave_pays_sam_wbd;
      dave_pays_dave_vest -= dave_pays_sam_vest;
      auto dave_pays_bob_wbd = dave_pays_sam_wbd / 2;
      auto dave_pays_bob_vest = dave_pays_sam_vest / 2;
      dave_pays_sam_wbd -= dave_pays_bob_wbd;
      dave_pays_sam_vest -= dave_pays_bob_vest;
      auto dave_pays_alice_wbd = dave_pays_bob_wbd / 2;
      auto dave_pays_alice_vest = dave_pays_bob_vest / 2;
      dave_pays_bob_wbd -= dave_pays_alice_wbd;
      dave_pays_bob_vest -= dave_pays_alice_vest;

      // Calculate total comment payouts
      auto alice_comment_total_payout = db->to_wbd( asset( alice_pays_alice_wbd + alice_pays_alice_vest, WORTH_SYMBOL ) );
      alice_comment_total_payout += db->to_wbd( asset( bob_pays_alice_wbd + bob_pays_alice_vest, WORTH_SYMBOL ) );
      alice_comment_total_payout += db->to_wbd( asset( dave_pays_alice_wbd + dave_pays_alice_vest, WORTH_SYMBOL ) );
      auto bob_comment_total_payout = db->to_wbd( asset( bob_pays_bob_wbd + bob_pays_bob_vest, WORTH_SYMBOL ) );
      bob_comment_total_payout += db->to_wbd( asset( dave_pays_bob_wbd + dave_pays_bob_vest, WORTH_SYMBOL ) );
      auto sam_comment_total_payout = db->to_wbd( asset( dave_pays_sam_wbd + dave_pays_sam_vest, WORTH_SYMBOL ) );
      auto dave_comment_total_payout = db->to_wbd( asset( dave_pays_dave_wbd + dave_pays_dave_vest, WORTH_SYMBOL ) );

      auto alice_starting_vesting = db->get_account( "alice" ).vesting_shares;
      auto alice_starting_wbd = db->get_account( "alice" ).wbd_balance;
      auto bob_starting_vesting = db->get_account( "bob" ).vesting_shares;
      auto bob_starting_wbd = db->get_account( "bob" ).wbd_balance;
      auto sam_starting_vesting = db->get_account( "sam" ).vesting_shares;
      auto sam_starting_wbd = db->get_account( "sam" ).wbd_balance;
      auto dave_starting_vesting = db->get_account( "dave" ).vesting_shares;
      auto dave_starting_wbd = db->get_account( "dave" ).wbd_balance;
      ...
      BOOST_REQUIRE( com_vop.payout.amount.value == dave_pays_alice_wbd );
      ...
      BOOST_REQUIRE( com_vop.originating_author == "dave" );
      BOOST_REQUIRE( com_vop.originating_permlink == "test" );
      BOOST_REQUIRE( com_vop.payout.amount.value == dave_pays_bob_wbd );
      ...
      BOOST_REQUIRE( com_vop.originating_author == "dave" );
      BOOST_REQUIRE( com_vop.originating_permlink == "test" );
      BOOST_REQUIRE( com_vop.payout.amount.value == dave_pays_sam_wbd );
      ...
      BOOST_REQUIRE( com_vop.originating_author == "dave" );
      BOOST_REQUIRE( com_vop.originating_permlink == "test" );
      BOOST_REQUIRE( com_vop.payout.amount.value == dave_pays_dave_wbd );
      ...
      BOOST_REQUIRE( com_vop.originating_permlink == "test" );
      BOOST_REQUIRE( com_vop.payout.amount.value == bob_pays_alice_wbd );
      ...
      BOOST_REQUIRE( com_vop.originating_author == "bob" );
      BOOST_REQUIRE( com_vop.originating_permlink == "test" );
      BOOST_REQUIRE( com_vop.payout.amount.value == bob_pays_bob_wbd );
      ...
      BOOST_REQUIRE( com_vop.originating_permlink == "test" );
      BOOST_REQUIRE( com_vop.payout.amount.value == alice_pays_alice_wbd );
      ...
      auto alice_total_wbd = alice_starting_wbd + asset( alice_pays_alice_wbd + bob_pays_alice_wbd + dave_pays_alice_wbd, WORTH_SYMBOL ) * exchange_rate;
      auto alice_total_vesting = alice_starting_vesting + asset( alice_pays_alice_vest + bob_pays_alice_vest + dave_pays_alice_vest + alice_vote_alice_reward.amount + alice_vote_bob_reward.amount, WORTH_SYMBOL ) * gpo.get_vesting_share_price();
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance.amount.value == alice_total_wbd.amount.value );
      BOOST_REQUIRE( db->get_account( "alice" ).vesting_shares.amount.value == alice_total_vesting.amount.value );

      auto bob_total_wbd = bob_starting_wbd + asset( bob_pays_bob_wbd + dave_pays_bob_wbd, WORTH_SYMBOL ) * exchange_rate;
      auto bob_total_vesting = bob_starting_vesting + asset( bob_pays_bob_vest + dave_pays_bob_vest + bob_vote_alice_reward.amount + bob_vote_bob_reward.amount + bob_vote_dave_reward.amount, WORTH_SYMBOL ) * gpo.get_vesting_share_price();
      BOOST_REQUIRE( db->get_account( "bob" ).wbd_balance.amount.value == bob_total_wbd.amount.value );
      BOOST_REQUIRE( db->get_account( "bob" ).vesting_shares.amount.value == bob_total_vesting.amount.value );

      auto sam_total_wbd = sam_starting_wbd + asset( dave_pays_sam_wbd, WORTH_SYMBOL ) * exchange_rate;
      auto sam_total_vesting = bob_starting_vesting + asset( dave_pays_sam_vest + sam_vote_bob_reward.amount, WORTH_SYMBOL ) * gpo.get_vesting_share_price();
      BOOST_REQUIRE( db->get_account( "sam" ).wbd_balance.amount.value == sam_total_wbd.amount.value );
      BOOST_REQUIRE( db->get_account( "sam" ).vesting_shares.amount.value == sam_total_vesting.amount.value );

      auto dave_total_wbd = dave_starting_wbd + asset( dave_pays_dave_wbd, WORTH_SYMBOL ) * exchange_rate;
      auto dave_total_vesting = dave_starting_vesting + asset( dave_pays_dave_vest, WORTH_SYMBOL ) * gpo.get_vesting_share_price();
      BOOST_REQUIRE( db->get_account( "dave" ).wbd_balance.amount.value == dave_total_wbd.amount.value );
...
BOOST_AUTO_TEST_CASE( vesting_withdrawals )
{
   try
   {
      ...
      auto next_withdrawal = db->head_block_time() + WORTH_VESTING_WITHDRAW_INTERVAL_SECONDS;
      ...
      generate_blocks( next_withdrawal - ( WORTH_BLOCK_INTERVAL / 2 ), true);
      ...
      for( int i = 1; i < WORTH_VESTING_WITHDRAW_INTERVALS - 1; i++ )
      {
         generate_blocks( db->head_block_time() + WORTH_VESTING_WITHDRAW_INTERVAL_SECONDS );
         ...
         if ( i == WORTH_VESTING_WITHDRAW_INTERVALS - 1 )
            BOOST_REQUIRE( alice.next_vesting_withdrawal == fc::time_point_sec::maximum() );
         else
            BOOST_REQUIRE( alice.next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + WORTH_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
      ...
      generate_blocks( db->head_block_time() + WORTH_VESTING_WITHDRAW_INTERVAL_SECONDS, true );
...
BOOST_AUTO_TEST_CASE( vesting_withdraw_route )
{
   try
   {
      ...
      op.to_account = "bob";
      op.percent = WORTH_1_PERCENT * 50;
      op.auto_vest = true;
      tx.operations.push_back( op );

      BOOST_TEST_MESSAGE( "Setting up sam destination" );
      op.to_account = "sam";
      op.percent = WORTH_1_PERCENT * 30;
      op.auto_vest = false;
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...

      {
         ...
         BOOST_REQUIRE( alice.balance == old_alice_balance + asset( ( vesting_withdraw_rate.amount * WORTH_1_PERCENT * 20 ) / WORTH_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
         BOOST_REQUIRE( bob.vesting_shares == old_bob_vesting + asset( ( vesting_withdraw_rate.amount * WORTH_1_PERCENT * 50 ) / WORTH_100_PERCENT, VESTS_SYMBOL ) );
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * WORTH_1_PERCENT * 30 ) / WORTH_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
      ...
      op.to_account = "sam";
      op.percent = WORTH_1_PERCENT * 50 + 1;
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      ...
      generate_blocks( db->get_account( "alice" ).next_vesting_withdrawal, true );
      {
         ...
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * WORTH_1_PERCENT * 50 ) / WORTH_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
      }
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_mean )
{
   try
   {
      ...
      generate_blocks( 30 / WORTH_BLOCK_INTERVAL );
      ...
      for( int i = 0; i < 7; i++ )
      {
         transfer( WORTH_INIT_MINER_NAME, accounts[i], asset( 10000, WORTH_SYMBOL ) );
      ...
      ops[0].exchange_rate = price( asset( 1000, WBD_SYMBOL ), asset( 100000, WORTH_SYMBOL ) );
      ops[1].exchange_rate = price( asset( 1000, WBD_SYMBOL ), asset( 105000, WORTH_SYMBOL ) );
      ops[2].exchange_rate = price( asset( 1000, WBD_SYMBOL ), asset(  98000, WORTH_SYMBOL ) );
      ops[3].exchange_rate = price( asset( 1000, WBD_SYMBOL ), asset(  97000, WORTH_SYMBOL ) );
      ops[4].exchange_rate = price( asset( 1000, WBD_SYMBOL ), asset(  99000, WORTH_SYMBOL ) );
      ops[5].exchange_rate = price( asset( 1000, WBD_SYMBOL ), asset(  97500, WORTH_SYMBOL ) );
      ops[6].exchange_rate = price( asset( 1000, WBD_SYMBOL ), asset( 102000, WORTH_SYMBOL ) );

      for( int i = 0; i < 7; i++ )
      {
         txs[i].set_expiration( db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION );
      ...
      generate_blocks( WORTH_BLOCKS_PER_HOUR ); // Jump forward 1 hour
      ...
      BOOST_REQUIRE( feed_history.current_median_history == price( asset( 1000, WBD_SYMBOL ), asset( 99000, WORTH_SYMBOL) ) );
      BOOST_REQUIRE( feed_history.price_history[ 0 ] == price( asset( 1000, WBD_SYMBOL ), asset( 99000, WORTH_SYMBOL) ) );
      validate_database();
      ...
      for ( int i = 0; i < 23; i++ )
      {
         ...
         for( int j = 0; j < 7; j++ )
         {
            ...
            ops[j].exchange_rate = price( ops[j].exchange_rate.base, asset( ops[j].exchange_rate.quote.amount + 10, WORTH_SYMBOL ) );
            txs[j].set_expiration( db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( convert_delay )
{
   try
   {
      ...
      tx.signatures.clear();
      op.owner = "alice";
      op.amount = asset( 2000, WBD_SYMBOL );
      op.requestid = 2;
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      ...
      generate_blocks( db->head_block_time() + WORTH_CONVERSION_DELAY - fc::seconds( WORTH_BLOCK_INTERVAL / 2 ), true );
      ...
      BOOST_REQUIRE( alice_2.wbd_balance.amount.value == ( start_balance - op.amount ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( alice_3.wbd_balance.amount.value == ( start_balance - op.amount ).amount.value );
...
BOOST_AUTO_TEST_CASE( worth_inflation )
{
   try
   {
   /*
      BOOST_TEST_MESSAGE( "Testing WORTH Inflation until the vesting start block" );
      ...
      auto new_rewards = std::max( WORTH_MIN_CONTENT_REWARD, asset( ( WORTH_CONTENT_APR * gpo.virtual_supply.amount ) / ( WORTH_BLOCKS_PER_YEAR * 100 ), WORTH_SYMBOL ) )
         + std::max( WORTH_MIN_CURATE_REWARD, asset( ( WORTH_CURATE_APR * gpo.virtual_supply.amount ) / ( WORTH_BLOCKS_PER_YEAR * 100 ), WORTH_SYMBOL ) );
      auto witness_pay = std::max( WORTH_MIN_PRODUCER_REWARD, asset( ( WORTH_PRODUCER_APR * gpo.virtual_supply.amount ) / ( WORTH_BLOCKS_PER_YEAR * 100 ), WORTH_SYMBOL ) );
      auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
      auto new_vesting_worth = asset( 0, WORTH_SYMBOL );
      auto new_vesting_shares = gpo.total_vesting_shares;

      if ( db->get_account( witness_name ).vesting_shares.amount.value == 0 )
      {
         new_vesting_worth += witness_pay;
         new_vesting_shares += witness_pay * ( gpo.total_vesting_shares / gpo.total_vesting_fund_worth );
      }

      auto new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_worth;
      new_rewards += gpo.total_reward_fund_worth;
      new_vesting_worth += gpo.total_vesting_fund_worth;
      ...
      BOOST_REQUIRE( gpo.total_reward_fund_worth.amount.value == new_rewards.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_fund_worth.amount.value == new_vesting_worth.amount.value );
      ...
      while( db->head_block_num() < WORTH_START_VESTING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( WORTH_MIN_CONTENT_REWARD, asset( ( WORTH_CONTENT_APR * gpo.virtual_supply.amount ) / ( WORTH_BLOCKS_PER_YEAR * 100 ), WORTH_SYMBOL ) )
            + std::max( WORTH_MIN_CURATE_REWARD, asset( ( WORTH_CURATE_APR * gpo.virtual_supply.amount ) / ( WORTH_BLOCKS_PER_YEAR * 100 ), WORTH_SYMBOL ) );
         witness_pay = std::max( WORTH_MIN_PRODUCER_REWARD, asset( ( WORTH_PRODUCER_APR * gpo.virtual_supply.amount ) / ( WORTH_BLOCKS_PER_YEAR * 100 ), WORTH_SYMBOL ) );
         new_vesting_worth = asset( 0, WORTH_SYMBOL );
         ...
         if ( db->get_account( witness_name ).vesting_shares.amount.value == 0 )
         {
            ...
            witness_pay = asset( 0, WORTH_SYMBOL );
         }

         new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_worth;
         new_rewards += gpo.total_reward_fund_worth;
         new_vesting_worth += gpo.total_vesting_fund_worth;
      ...
      while( db->head_block_num() < WORTH_START_MINER_VOTING_BLOCK - 1 )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db->get_scheduled_witness(1);
         old_witness_balance = db->get_account( witness_name ).balance;
         ...
         witness_pay = std::max( WORTH_MIN_PRODUCER_REWARD, asset( ( WORTH_PRODUCER_APR * gpo.virtual_supply.amount ) / ( WORTH_BLOCKS_PER_YEAR * 100 ), WORTH_SYMBOL ) );
         auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
         new_vesting_worth = asset( ( witness_pay + new_rewards ).amount * 9, WORTH_SYMBOL );
         new_vesting_shares = gpo.total_vesting_shares;
      ...
      for( int i = 0; i < WORTH_BLOCKS_PER_DAY; i++ )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db->get_scheduled_witness(1);
         old_witness_balance = db->get_account( witness_name ).balance;
         ...
         witness_pay = std::max( WORTH_MIN_PRODUCER_REWARD, asset( ( WORTH_PRODUCER_APR * gpo.virtual_supply.amount ) / ( WORTH_BLOCKS_PER_YEAR * 100 ), WORTH_SYMBOL ) );
         witness_pay_shares = witness_pay * gpo.get_vesting_share_price();
         new_vesting_worth = asset( ( witness_pay + new_rewards ).amount * 9, WORTH_SYMBOL ) + witness_pay;
         new_vesting_shares = gpo.total_vesting_shares + witness_pay_shares;
         new_supply = gpo.current_supply + new_rewards + new_vesting_worth;
         new_rewards += gpo.total_reward_fund_worth;
         new_vesting_worth += gpo.total_vesting_fund_worth;
      ...
      vesting_worth = gpo.total_vesting_fund_worth;
      reward_worth = gpo.total_reward_fund_worth;
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_vesting_fund_worth.amount.value,
         ( vesting_worth.amount.value
            + ( ( ( uint128_t( virtual_supply.amount.value ) / 10 ) / WORTH_BLOCKS_PER_YEAR ) * 9 )
            + ( uint128_t( virtual_supply.amount.value ) / 100 / WORTH_BLOCKS_PER_YEAR ) ).to_uint64() );
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_worth.amount.value,
         reward_worth.amount.value + virtual_supply.amount.value / 10 / WORTH_BLOCKS_PER_YEAR + virtual_supply.amount.value / 10 / WORTH_BLOCKS_PER_DAY );
      BOOST_REQUIRE_EQUAL( db->get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / WORTH_BLOCKS_PER_YEAR ) * WORTH_1_PERCENT ) / WORTH_100_PERCENT, WORTH_SYMBOL ) * ( vesting_shares / vesting_worth ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE( wbd_interest )
{
   try
   {
      ACTORS( (alice)(bob) )
      ...
      auto start_time = db->get_account( "alice" ).wbd_seconds_last_update;
      auto alice_wbd = db->get_account( "alice" ).wbd_balance;

      generate_blocks( db->head_block_time() + fc::seconds( WORTH_WBD_INTEREST_COMPOUND_INTERVAL_SEC ), true );
      ...
      BOOST_REQUIRE( gpo.wbd_interest_rate > 0 );
      BOOST_REQUIRE( static_cast<uint64_t>(db->get_account( "alice" ).wbd_balance.amount.value) == alice_wbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_wbd.amount.value ) * ( db->head_block_time() - start_time ).to_seconds() ) / WORTH_SECONDS_PER_YEAR ) * gpo.wbd_interest_rate ) / WORTH_100_PERCENT ).to_uint64() );
      BOOST_REQUIRE( interest_op.owner == "alice" );
      BOOST_REQUIRE( interest_op.interest.amount.value == db->get_account( "alice" ).wbd_balance.amount.value - ( alice_wbd.amount.value - ASSET( "1.000 TBD" ).amount.value ) );
      validate_database();

      BOOST_TEST_MESSAGE( "Testing interest under interest period" );

      start_time = db->get_account( "alice" ).wbd_seconds_last_update;
      alice_wbd = db->get_account( "alice" ).wbd_balance;

      generate_blocks( db->head_block_time() + fc::seconds( WORTH_WBD_INTEREST_COMPOUND_INTERVAL_SEC / 2 ), true );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance.amount.value == alice_wbd.amount.value - ASSET( "1.000 TBD" ).amount.value );
      validate_database();

      auto alice_coindays = uint128_t( alice_wbd.amount.value ) * ( db->head_block_time() - start_time ).to_seconds();
      alice_wbd = db->get_account( "alice" ).wbd_balance;
      start_time = db->get_account( "alice" ).wbd_seconds_last_update;

      BOOST_TEST_MESSAGE( "Testing longer interest period" );

      generate_blocks( db->head_block_time() + fc::seconds( ( WORTH_WBD_INTEREST_COMPOUND_INTERVAL_SEC * 7 ) / 3 ), true );
      ...
      BOOST_REQUIRE( static_cast<uint64_t>(db->get_account( "alice" ).wbd_balance.amount.value) == alice_wbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_wbd.amount.value ) * ( db->head_block_time() - start_time ).to_seconds() + alice_coindays ) / WORTH_SECONDS_PER_YEAR ) * gpo.wbd_interest_rate ) / WORTH_100_PERCENT ).to_uint64() );
      validate_database();
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( liquidity_rewards )
{
   ...
   try
   {
      ...
      op.owner = "alice";
      op.amount_to_sell = asset( alice_wbd.amount.value / 20, WBD_SYMBOL ) ;
      ...
      BOOST_TEST_MESSAGE( "Creating Limit Order for WBD that will be filled immediately." );
      ...
      alice_worth_volume += ( asset( alice_wbd.amount / 20, WBD_SYMBOL ) * exchange_rate ).amount.value;
      alice_reward_last_update = db->head_block_time();
      bob_worth_volume -= ( asset( alice_wbd.amount / 20, WBD_SYMBOL ) * exchange_rate ).amount.value;
      ...
      BOOST_REQUIRE( reward->wbd_volume == alice_wbd_volume );
      ...
      BOOST_REQUIRE( reward->wbd_volume == bob_wbd_volume );
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == asset( alice_wbd.amount.value / 20, WBD_SYMBOL ).amount.value );
      BOOST_REQUIRE( fill_order_op.current_owner == "bob" );
      BOOST_REQUIRE( fill_order_op.current_orderid == 2 );
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == ( asset( alice_wbd.amount.value / 20, WBD_SYMBOL ) * exchange_rate ).amount.value );
      ...
      BOOST_TEST_MESSAGE( "Creating Limit Order for WBD that will stay on the books for 60 minutes." );

      op.owner = "sam";
      op.amount_to_sell = asset( ( alice_wbd.amount.value / 20 ), WORTH_SYMBOL );
      op.min_to_receive = asset( ( alice_wbd.amount.value / 20 ), WBD_SYMBOL );
      op.orderid = 3;
      ...
      BOOST_TEST_MESSAGE( "Creating Limit Order for WBD that will stay on the books for 30 minutes." );

      op.owner = "bob";
      op.orderid = 4;
      op.amount_to_sell = asset( ( alice_wbd.amount.value / 10 ) * 3 - alice_wbd.amount.value / 20, WORTH_SYMBOL );
      op.min_to_receive = asset( ( alice_wbd.amount.value / 10 ) * 3 - alice_wbd.amount.value / 20, WBD_SYMBOL );
      ...
      op.owner = "alice";
      op.orderid = 5;
      op.amount_to_sell = asset( ( alice_wbd.amount.value / 10 ) * 3, WBD_SYMBOL );
      op.min_to_receive = asset( ( alice_wbd.amount.value / 10 ) * 3, WORTH_SYMBOL );
      ...
      alice_wbd_volume -= ( alice_wbd.amount.value / 10 ) * 3;
      alice_reward_last_update = db->head_block_time();
      sam_wbd_volume += alice_wbd.amount.value / 20;
      sam_reward_last_update = db->head_block_time();
      bob_wbd_volume += ( alice_wbd.amount.value / 10 ) * 3 - ( alice_wbd.amount.value / 20 );
      bob_reward_last_update = db->head_block_time();
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == asset( ( alice_wbd.amount.value / 10 ) * 3 - alice_wbd.amount.value / 20, WORTH_SYMBOL ).amount.value );
      BOOST_REQUIRE( fill_order_op.current_owner == "alice" );
      BOOST_REQUIRE( fill_order_op.current_orderid == 5 );
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == asset( ( alice_wbd.amount.value / 10 ) * 3 - alice_wbd.amount.value / 20, WBD_SYMBOL ).amount.value );
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == asset( alice_wbd.amount.value / 20, WORTH_SYMBOL ).amount.value );
      BOOST_REQUIRE( fill_order_op.current_owner == "alice" );
      BOOST_REQUIRE( fill_order_op.current_orderid == 5 );
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == asset( alice_wbd.amount.value / 20, WBD_SYMBOL ).amount.value );
      ...
      BOOST_REQUIRE( reward->wbd_volume == sam_wbd_volume );
      ...
      op.orderid = 6;
      op.amount_to_sell = asset( alice_wbd.amount.value / 20 * 2, WBD_SYMBOL );
      op.min_to_receive = asset( alice_wbd.amount.value / 20 * 2, WORTH_SYMBOL );
      ...
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == asset( alice_wbd.amount.value / 20, WORTH_SYMBOL ).amount.value );
      ...
      alice_worth_volume += alice_wbd.amount.value / 20;
      alice_reward_last_update = db->head_block_time();
      sam_worth_volume -= alice_wbd.amount.value / 20;
      sam_reward_last_update = db->head_block_time();
      ...
      transfer.to = "dave";
      transfer.from = "alice";
      transfer.amount = asset( alice_wbd.amount / 2, WBD_SYMBOL );
      ...
      op.owner = "alice";
      op.amount_to_sell = asset( 8 * ( alice_wbd.amount.value / 20 ), WORTH_SYMBOL );
      op.min_to_receive = asset( op.amount_to_sell.amount, WBD_SYMBOL );
      op.orderid = 9;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      ...
      op.owner = "dave";
      op.amount_to_sell = asset( 7 * ( alice_wbd.amount.value / 20 ), WBD_SYMBOL );;
      ...
      alice_wbd_volume += op.amount_to_sell.amount.value;
      alice_reward_last_update = db->head_block_time();
      dave_wbd_volume -= op.amount_to_sell.amount.value;
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == 7 * ( alice_wbd.amount.value / 20 ) );
      BOOST_REQUIRE( fill_order_op.current_owner == "dave" );
      BOOST_REQUIRE( fill_order_op.current_orderid == 10 );
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == 7 * ( alice_wbd.amount.value / 20 ) );
      ...
      alice_wbd_volume += op.amount_to_sell.amount.value;
      alice_reward_last_update = db->head_block_time();
      bob_wbd_volume -= op.amount_to_sell.amount.value;
      bob_reward_last_update = db->head_block_time();
      ...
      BOOST_REQUIRE( fill_order_op.open_owner == "alice" );
      BOOST_REQUIRE( fill_order_op.open_orderid == 9 );
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == alice_wbd.amount.value / 20 );
      BOOST_REQUIRE( fill_order_op.current_owner == "bob" );
      BOOST_REQUIRE( fill_order_op.current_orderid == 11 );
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == alice_wbd.amount.value / 20 );
      ...
      transfer.to = "bob";
      transfer.from = "alice";
      transfer.amount = asset( alice_wbd.amount / 5, WBD_SYMBOL );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( transfer );
      tx.set_expiration( db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION );
      ...
      op.owner = "bob";
      op.orderid = 12;
      op.amount_to_sell = asset( 3 * ( alice_wbd.amount / 40 ), WBD_SYMBOL );
      op.min_to_receive = asset( op.amount_to_sell.amount, WORTH_SYMBOL );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      ...
      op.min_to_receive.symbol = WBD_SYMBOL;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, dave_private_key );
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == 3 * ( alice_wbd.amount.value / 40 ) );
      BOOST_REQUIRE( fill_order_op.current_owner == "dave" );
      BOOST_REQUIRE( fill_order_op.current_orderid == 13 );
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == 3 * ( alice_wbd.amount.value / 40 ) );
      ...
      op.owner = "alice";
      op.orderid = 15;
      op.amount_to_sell.symbol = WBD_SYMBOL;
      ...
      sam_wbd_volume = ASSET( "1.000 TBD" ).amount.value;
      sam_worth_volume = 0;
      sam_reward_last_update = db->head_block_time();
...
BOOST_AUTO_TEST_CASE( post_rate_limit )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test1" ) ).reward_weight == WORTH_100_PERCENT );

      tx.operations.clear();
      tx.signatures.clear();

      generate_blocks( db->head_block_time() + WORTH_MIN_ROOT_COMMENT_INTERVAL + fc::seconds( WORTH_BLOCK_INTERVAL ), true );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test2" ) ).reward_weight == WORTH_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test3" ) ).reward_weight == WORTH_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test4" ) ).reward_weight == WORTH_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test5" ) ).reward_weight == WORTH_100_PERCENT );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_freeze )
{
   try
   {
      ...
      vote.voter = "bob";
      vote.weight = WORTH_100_PERCENT * -1;
...
BOOST_AUTO_TEST_CASE( wbd_stability )
{
   #ifndef DEBUG
   try
   {
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            gpo.wps_fund_percent = 0;
            gpo.content_reward_percent = 75 * WORTH_1_PERCENT;
         });
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().wbd_print_rate == WORTH_100_PERCENT );
      ...
      db_plugin->debug_generate_blocks_until( debug_key, fc::time_point_sec( db->get_comment( comment.author, comment.permlink ).cashout_time.sec_since_epoch() - 2 * WORTH_BLOCK_INTERVAL ), true, database::skip_witness_signature );

      auto& gpo = db->get_dynamic_global_properties();

      BOOST_TEST_MESSAGE( "Changing sam and gpo to set up market cap conditions" );

      asset wbd_balance = asset( ( gpo.virtual_supply.amount * ( gpo.wbd_stop_percent + 112 ) ) / WORTH_100_PERCENT, WORTH_SYMBOL ) * exchange_rate;
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( "sam" ), [&]( account_object& a )
         {
            a.wbd_balance = wbd_balance;
         });
      }, database::skip_witness_signature );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.current_wbd_supply = wbd_balance + db.get_account( WORTH_TREASURY_ACCOUNT ).wbd_balance;
            gpo.virtual_supply = gpo.virtual_supply + wbd_balance * exchange_rate;
         });
      }, database::skip_witness_signature );
      ...
      auto comment_reward = ( gpo.total_reward_fund_worth.amount + 2000 ) - ( ( gpo.total_reward_fund_worth.amount + 2000 ) * 25 * WORTH_1_PERCENT ) / WORTH_100_PERCENT ;
      comment_reward /= 2;
      auto wbd_reward = ( comment_reward * gpo.wbd_print_rate ) / WORTH_100_PERCENT;
      auto alice_wbd = db->get_account( "alice" ).wbd_balance + db->get_account( "alice" ).reward_wbd_balance + asset( wbd_reward, WORTH_SYMBOL ) * exchange_rate;
      auto alice_worth = db->get_account( "alice" ).balance + db->get_account( "alice" ).reward_worth_balance ;

      BOOST_TEST_MESSAGE( "Checking printing WBD has slowed" );
      BOOST_REQUIRE( db->get_dynamic_global_properties().wbd_print_rate < WORTH_100_PERCENT );

      BOOST_TEST_MESSAGE( "Pay out comment and check rewards are paid as WORTH" );
      db_plugin->debug_generate_blocks( debug_key, 1, database::skip_witness_signature );

      validate_database();

      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance + db->get_account( "alice" ).reward_wbd_balance == alice_wbd );
      BOOST_REQUIRE( db->get_account( "alice" ).balance + db->get_account( "alice" ).reward_worth_balance > alice_worth );

      BOOST_TEST_MESSAGE( "Letting percent market cap fall to wbd_start_percent to verify printing of WBD turns back on" );

      // Get close to wbd_start_percent for printing WBD to start again, but not all the way
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( "sam" ), [&]( account_object& a )
         {
            a.wbd_balance = asset( ( ( gpo.wbd_start_percent - 9 ) * wbd_balance.amount ) / gpo.wbd_stop_percent, WBD_SYMBOL );
         });
      }, database::skip_witness_signature );

      auto current_wbd_supply = alice_wbd + asset( ( ( gpo.wbd_start_percent - 9 ) * wbd_balance.amount ) / gpo.wbd_stop_percent, WBD_SYMBOL ) + db->get_account( WORTH_TREASURY_ACCOUNT ).wbd_balance;

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.current_wbd_supply = current_wbd_supply;
         });
      }, database::skip_witness_signature );
      ...
      auto last_print_rate = db->get_dynamic_global_properties().wbd_print_rate;

      // Keep producing blocks until printing WBD is back
      while( ( db->get_dynamic_global_properties().current_wbd_supply * exchange_rate ).amount >= ( db->get_dynamic_global_properties().virtual_supply.amount * db->get_dynamic_global_properties().wbd_start_percent ) / WORTH_100_PERCENT )
      {
         auto& gpo = db->get_dynamic_global_properties();
         BOOST_REQUIRE( gpo.wbd_print_rate >= last_print_rate );
         last_print_rate = gpo.wbd_print_rate;
         db_plugin->debug_generate_blocks( debug_key, 1, database::skip_witness_signature );
...
BOOST_AUTO_TEST_CASE( wbd_price_feed_limit )
{
   try
   {
      ACTORS( (alice) );
      ...
      BOOST_TEST_MESSAGE( "Setting WBD percent to greater than 10% market cap." );

      db->skip_price_feed_limit_check = false;
      const auto& gpo = db->get_dynamic_global_properties();
      auto new_exchange_rate = price( gpo.current_wbd_supply, asset( ( WORTH_100_PERCENT ) * gpo.current_supply.amount, WORTH_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( clear_null_account )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( WORTH_NULL_ACCOUNT ), [&]( account_object& a )
         {
            a.reward_worth_balance = ASSET( "1.000 TESTS" );
            a.reward_wbd_balance = ASSET( "1.000 TBD" );
            a.reward_vesting_balance = ASSET( "1.000000 VESTS" );
            a.reward_vesting_worth = ASSET( "1.000 TESTS" );
         });
         ...
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            ...
            gpo.current_wbd_supply += ASSET( "1.000 TBD" );
            gpo.pending_rewarded_vesting_shares += ASSET( "1.000000 VESTS" );
            gpo.pending_rewarded_vesting_worth += ASSET( "1.000 TESTS" );
         });
      });

      validate_database();

      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).wbd_balance == ASSET( "2.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).vesting_shares > ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).savings_balance == ASSET( "4.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).savings_wbd_balance == ASSET( "5.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).reward_wbd_balance == ASSET( "1.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).reward_worth_balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "1.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).reward_vesting_worth == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "2.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance == ASSET( "3.000 TBD" ) );

      BOOST_TEST_MESSAGE( "--- Generating block to clear balances" );
      generate_block();
      validate_database();

      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).wbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).vesting_shares == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).savings_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).savings_wbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).reward_wbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).reward_worth_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( WORTH_NULL_ACCOUNT ).reward_vesting_worth == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "2.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).wbd_balance == ASSET( "3.000 TBD" ) );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( generate_account_subsidies )
{
   try
   {
      ...
      const witness_schedule_object& wso = db->get_witness_schedule_object();
      BOOST_CHECK_EQUAL( wso.account_subsidy_rd.resource_unit, WORTH_ACCOUNT_SUBSIDY_PRECISION );
      ...
      while( true )
      {
         ...
         if( gpo.available_account_subsidies >= 100 * WORTH_ACCOUNT_SUBSIDY_PRECISION )
            break;
         generate_block();
         pool = pool + 5123 - ((249617279 * pool) >> WORTH_RD_DECAY_DENOM_SHIFT);
      }
...
BOOST_AUTO_TEST_CASE( account_subsidy_witness_limits )
{
   try
   {
      ...
      size_t n = size_t( db->get< witness_object, by_name >( db->get_scheduled_witness( 1 ) ).available_witness_account_subsidies / WORTH_ACCOUNT_SUBSIDY_PRECISION );
