...
./smoketest.sh ~/working/worths ~/reference/worths ~/working_blockchain_dir ~/reference_blockchain_dir 3000000 12
...
1. In that subdirectory create test_group.sh which
    - Accepts following arguments: JOBS TEST_ADDRESS TEST_PORT REF_ADDRESS REF_PORT BLOCK_LIMIT where:
      ...
      TEST_ADDRESS:TEST_PORT indicate where the tested worths instance listens to API requests,
      REF_ADDRESS:REF_PORT ditto for reference instance and
      BLOCK_LIMIT is the top number of block to be processed.
    - Returns 0 on success, non-zero on failure.
    - Create config.ini in test directory or config_ref.ini and config_test.ini for dedicated worths instances.
      If no config*.ini files you need manually create them in working directories of worths instances.
