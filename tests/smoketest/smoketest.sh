...
function print_help_and_quit {
   echo "Usage: path_to_tested_worths path_to_reference_worths path_to_test_blockchain_directory"
   echo "       path_to_reference_blockchain_directory number_of_blocks_to_replay [number_of_jobs [--dont-copy-config]]"
   echo "Example: ~/work/wortheum/wortheum/build/Release/programs/worths/worths ~/master/wortheum/wortheum/build/Release/programs/worths/worths ~/wortheum/wortheum/work1 ~/wortheum/wortheum/work2 5000000"
...
function run_test_group {
   ...
   echo Running ./$GROUP_TEST_SCRIPT $JOBS $TEST_WORTHS_PATH $REF_WORTHS_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   ./$GROUP_TEST_SCRIPT $JOBS $TEST_WORTHS_PATH $REF_WORTHS_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   EXIT_CODE=$?
   if [ $EXIT_CODE -ne 0 ]
   then
      EXIT_CODE=-1
...
check_worths_path $TEST_WORTHS_PATH
check_worths_path $REF_WORTHS_PATH
