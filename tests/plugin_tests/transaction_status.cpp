...
#include <worth/plugins/transaction_status/transaction_status_plugin.hpp>
#include <worth/plugins/transaction_status/transaction_status_objects.hpp>
#include <worth/plugins/transaction_status_api/transaction_status_api_plugin.hpp>
#include <worth/plugins/transaction_status_api/transaction_status_api.hpp>
...
{
   using namespace worth::plugins::transaction_status;
   ...
   try
   {
      ...
      appbase::app().register_plugin< worth::plugins::transaction_status_api::transaction_status_api_plugin >();
      ...
      appbase::app().initialize<
         worth::plugins::transaction_status_api::transaction_status_api_plugin,
         worth::plugins::debug_node::debug_node_plugin >( test_argc, (char**)test_argv );

      db = &appbase::app().get_plugin< worth::plugins::chain::chain_plugin >().db();
      BOOST_REQUIRE( db );

      auto tx_status_api = &appbase::app().get_plugin< worth::plugins::transaction_status_api::transaction_status_api_plugin >();
      BOOST_REQUIRE( tx_status_api );

      auto tx_status = &appbase::app().get_plugin< worth::plugins::transaction_status::transaction_status_plugin >();
      ...
      auto tx0_expiration = db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION;
      ...
      // Tracking should not be enabled until we have reached TRANSCATION_STATUS_TRACK_AFTER_BLOCK - ( WORTH_MAX_TIME_UNTIL_EXPIRATION / WORTH_BLOCK_INTERVAL ) blocks
      BOOST_REQUIRE( db->get_index< transaction_status_index >().indices().get< by_id >().empty() );
      BOOST_REQUIRE( db->get_index< transaction_status_index >().indices().get< by_trx_id >().empty() );
      BOOST_REQUIRE( db->get_index< transaction_status_index >().indices().get< by_block_num >().empty() );
      ...
      auto tx1_expiration = db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION;
      ...
      auto tx2_expiration = db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION;
      ...
      auto tx3_expiration = db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION;
      ...
      generate_blocks( WORTH_MAX_TIME_UNTIL_EXPIRATION / WORTH_BLOCK_INTERVAL );
      ...
      auto tx4_expiration = db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION;
      ...
      auto tx5_expiration = db->head_block_time() + WORTH_MAX_TIME_UNTIL_EXPIRATION;
      ...
      generate_blocks( TRANSACTION_STATUS_TEST_BLOCK_DEPTH + ( WORTH_MAX_TIME_UNTIL_EXPIRATION / WORTH_BLOCK_INTERVAL ) - 1 );
