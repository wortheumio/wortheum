...
MAIN_LOG_PATH = "./wps_rpc_api_test.log"

MODULE_NAME = "WPS API Tester"
...
def list_voter_proposals(start, order_by, order_direction, limit, status):
    ...
    "jsonrpc" : "2.0",
    "id" : get_random_id(),
    "method" : "wps_api.list_voter_proposals", 
    "params" : {
      "start" : start,
      "order_by" : order_by, 
...
if __name__ == '__main__':
  logger.info("Performing WPS RPC API plugin tests")
  import argparse
  parser = argparse.ArgumentParser()
  parser.add_argument("--node-ip", help = "IP address of wortheum node", default = "http://127.0.0.1", dest = "node_ip")
  parser.add_argument("--node-port", help = "Wortheum node port", default = 8751, dest = "node_port")
  ...
  logger.info("Running WPS RPC API tests")
  ...
  logger.info("Running WPS API tests via condeser API")
