...
#define WBD_INITIAL_TEST_SUPPLY (300000000ll)

extern uint32_t WORTH_TESTING_GENESIS_TIMESTAMP;

#define PUSH_TX \
   worth::chain::test::_push_transaction

#define PUSH_BLOCK \
   worth::chain::test::_push_block
...
/*#define WORTH_REQUIRE_THROW( expr, exc_type )          \
{                                                         \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "WORTH_REQUIRE_THROW begin "        \
         << req_throw_info << std::endl;                  \
   BOOST_REQUIRE_THROW( expr, exc_type );                 \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "WORTH_REQUIRE_THROW end "          \
         << req_throw_info << std::endl;                  \
}*/
...
#define WORTH_CHECK_THROW( expr, exc_type )            \
{                                                         \
   std::string req_throw_info = fc::json::to_string(      \
      fc::mutable_variant_object()                        \
      ("source_file", __FILE__)                           \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "WORTH_CHECK_THROW begin "          \
         << req_throw_info << std::endl;                  \
   BOOST_CHECK_THROW( expr, exc_type );                   \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "WORTH_CHECK_THROW end "            \
         << req_throw_info << std::endl;                  \
}
...
{ \
   const auto temp = op.field; \
   op.field = value; \
   WORTH_REQUIRE_THROW( op.validate(), exc_type ); \
   op.field = temp; \
}
...
{ \
   ...
   trx.operations.back() = op; \
   op.field = bak; \
   WORTH_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
#define WMT_SYMBOL( name, decimal_places, db ) \
   asset_symbol_type name ## _symbol = get_new_wmt_symbol( decimal_places, db );

#define ASSET( s ) \
   worth::plugins::condenser_api::legacy_asset::from_string( s ).to_asset()
...
// 'WMT' would be dropped from the name then.
#define FUND_WMT_REWARDS( account_name, amount ) \
...
{ \
   signed_transaction tx; \
   OP2TX(OP,tx,KEY) \
   WORTH_REQUIRE_THROW( db->push_transaction( tx, 0 ), EXCEPTION ); \
}
...
struct database_fixture {
   ...
   string debug_key = worth::utilities::key_to_wif( init_account_priv_key );
   ...
   static asset_symbol_type get_new_wmt_symbol( uint8_t token_decimal_places, chain::database* db );
#endif
...
#ifdef WORTHEUM_ENABLE_WMT
template< typename T >
struct t_wmt_database_fixture : public T
{
   ...
   t_wmt_database_fixture(){}
   virtual ~t_wmt_database_fixture(){}

   asset_symbol_type create_wmt( const string& account_name, const fc::ecc::private_key& key,
      uint8_t token_decimal_places );
   asset_symbol_type create_wmt_with_nai( const string& account_name, const fc::ecc::private_key& key,
      uint32_t nai, uint8_t token_decimal_places );

   /// Creates 3 different WMTs for provided control account, one with 0 precision, the other two with the same non-zero precision.
   std::array<asset_symbol_type, 3> create_wmt_3(const char* control_account_name, const fc::ecc::private_key& key);
   /// Tries to create WMTs with too big precision or invalid name.
   void create_invalid_wmt( const char* control_account_name, const fc::ecc::private_key& key );
   /// Tries to create WMTs matching existing one. First attempt with matching precision, second one with different (but valid) precision.
   void create_conflicting_wmt( const asset_symbol_type existing_wmt, const char* control_account_name, const fc::ecc::private_key& key );

   //wmt_setup_operation
   wmt_generation_unit get_generation_unit ( const units& worth_unit = units(), const units& token_unit = units() );
   wmt_capped_generation_policy get_capped_generation_policy
   (
      const wmt_generation_unit& pre_soft_cap_unit = wmt_generation_unit(),
      const wmt_generation_unit& post_soft_cap_unit = wmt_generation_unit(),
...
using wmt_database_fixture = t_wmt_database_fixture< clean_database_fixture >;
using wmt_database_fixture_for_plugin = t_wmt_database_fixture< database_fixture >;

#endif

struct wps_proposal_database_fixture : public clean_database_fixture
{
   wps_proposal_database_fixture( uint16_t shared_file_size_in_mb = 8 )
                           : clean_database_fixture( shared_file_size_in_mb ){}
   virtual ~wps_proposal_database_fixture(){}
...
struct wps_proposal_database_fixture_performance : public wps_proposal_database_fixture
{
   wps_proposal_database_fixture_performance( uint16_t shared_file_size_in_mb = 512 )
                           : wps_proposal_database_fixture( shared_file_size_in_mb )
   {
      db->get_benchmark_dumper().set_enabled( true );
      db->set_wps_remove_threshold( -1 );
   }
};
...
{
   private:
      worth::plugins::json_rpc::json_rpc_plugin* rpc_plugin;
