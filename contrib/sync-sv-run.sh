...
WORTHS_PID=`pgrep -f p2p-endpoint`
if [[ ! $? -eq 0 ]]; then
  echo NOTIFYALERT! worthssync has quit unexpectedly, checking for coredump and then starting a new instance..
  sleep 30
  SAVED_PID=`cat /tmp/worthspid`
  if [[ -e /tmp/core.$SAVED_PID ]]; then
    gdb --batch --quiet -ex "thread apply all bt full" -ex "quit" /usr/local/worths-full/bin/worths /tmp/core.$SAVED_PID >> /tmp/stacktrace
    STACKTRACE=`cat /tmp/stacktrace`
    echo NOTIFYALERT! worthssync stacktrace from coredump:
  ...
  RUN_SV_PID=`pgrep -f /etc/service/worths`
  kill -9 $RUN_SV_PID
fi
...
if [[ ! -z "$BLOCKCHAIN_TIME" ]]; then
  ...
  if [[ ${BLOCK_AGE} -le 10 ]]; then
    WORTHS_PID=`pgrep -f p2p-endpoint`
    kill -SIGINT $WORTHS_PID
    echo worthssync: waiting for worths to exit cleanly

    # loop while the process is still running
    let WAIT_TIME=0
    while kill -0 $WORTHS_PID 2> /dev/null; do
       sleep 1
       let WAIT_TIME++
    ...
    echo worthssync: starting a new blockchainstate upload operation
    cd ${COMPRESSPATH:-$HOME}
    echo worthssync: compressing blockchainstate...
    if [[ "$USE_RAMDISK" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! worthssync was unable to compress shared memory file, check the logs.
      exit 1
    fi
    ...
    echo worthssync: uploading $FILE_NAME to $S3_BUCKET
    aws s3 cp blockchain.tar.lz4 s3://$S3_BUCKET/$FILE_NAME
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! worthssync was unable to upload $FILE_NAME to s3://$S3_BUCKET
      exit 1
    fi
    echo worthssync: replacing current version of blockchain state with $FILE_NAME
    if [[ "$IS_BROADCAST_NODE" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! worthssync was unable to overwrite the current blockchainstate with $FILE_NAME
      exit 1
    fi
    ...
    echo worthssync: stopping the container after a sync operation
    if [[ -e /tmp/isnewsync ]]; then
      echo notifyworthssync: worthssync: successfully generated and uploaded new blockchain-$VERSION-latest.tar.lz4 to s3://$S3_BUCKET
    fi
    RUN_SV_PID=`pgrep -f /etc/service/worths`
    kill -9 $RUN_SV_PID
  fi
