...
#include <worth/utilities/words.hpp>

#include <worth/protocol/base.hpp>
#include <worth/protocol/wps_operations.hpp>
#include <worth/wallet/wallet.hpp>
#include <worth/wallet/api_documentation.hpp>
#include <worth/wallet/reflect_util.hpp>
#include <worth/wallet/remote_node_api.hpp>
...
namespace worth { namespace wallet {
...
public:
   wallet_api& self;
   wallet_api_impl( wallet_api& s, const wallet_data& initial_data, const worth::protocol::chain_id_type& _worth_chain_id, fc::api< remote_node_api > rapi )
      : self( s ),
        _remote_api( rapi )
   {
      init_prototype_ops();
      ...
      worth_chain_id = _worth_chain_id;
   }
   ...
   variant info() const
   {
      ...
      result["median_wbd_price"] = _remote_api->get_current_median_history_price();
      result["account_creation_fee"] = _remote_api->get_chain_properties().account_creation_fee;
      result["post_reward_fund"] = fc::variant(_remote_api->get_reward_fund( WORTH_POST_REWARD_FUND_NAME )).get_object();
      return result;
   }

   variant_object about() const
   {
      string client_version( worth::utilities::git_revision_description );
      const size_t pos = client_version.find( '/' );
      if( pos != string::npos && client_version.size() > pos )
         client_version = client_version.substr( pos + 1 );

      fc::mutable_variant_object result;
      result["blockchain_version"]       = WORTH_BLOCKCHAIN_VERSION;
      result["client_version"]           = client_version;
      result["worth_revision"]           = worth::utilities::git_revision_sha;
      result["worth_revision_age"]       = fc::get_approximate_relative_time_string( fc::time_point_sec( worth::utilities::git_revision_unix_timestamp ) );
      ...
      try
      {
         ...
         result["server_worth_revision"] = v.worth_revision;
   ...
   bool import_key(string wif_key)
   {
      ...
      worth::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         worth::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         worth::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         worth::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         worth::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds < WORTH_MAX_TIME_UNTIL_EXPIRATION );
      ...
      bool broadcast = false )
   {
      ...
      for( const public_key_type& k : minimal_signing_keys )
      {
         ...
         tx.sign( it->second, worth_chain_id, fc::ecc::fc_canonical );
      }
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
      ...
      m["list_my_accounts"] = [](variant result, const fc::variants& a ) {
         ...
         for( const auto& a : accounts ) {
            total_worth += a.balance.to_asset();
            total_vest  += a.vesting_shares.to_asset();
            total_wbd  += a.wbd_balance.to_asset();
            out << std::left << std::setw( 17 ) << std::string(a.name)
                ...
                << std::right << std::setw(16) << fc::variant(a.wbd_balance).as_string() <<"\n";
         }
         out << "-------------------------------------------------------------------------\n";
            out << std::left << std::setw( 17 ) << "TOTAL"
                << std::right << std::setw(18) << legacy_asset::from_asset(total_worth).to_string() <<" "
                << std::right << std::setw(26) << legacy_asset::from_asset(total_vest).to_string() <<" "
                << std::right << std::setw(16) << legacy_asset::from_asset(total_wbd).to_string() <<"\n";
         return out.str();
      };
      ...
      m["get_open_orders"] = []( variant result, const fc::variants& a ) {
          ...
          for( const auto& o : orders ) {
             ...
             ss << ' ' << setw( 10 ) << (o.sell_price.base.symbol == WORTH_SYMBOL ? "SELL" : "BUY");
             ss << "\n";
          }
          return ss.str();
      };
      m["get_order_book"] = []( variant result, const fc::variants& a ) {
         ...
         std::stringstream ss;
         asset bid_sum = asset( 0, WBD_SYMBOL );
         asset ask_sum = asset( 0, WBD_SYMBOL );
         int spacing = 24;
         ...
         ss << ' ' << setw( ( spacing * 4 ) + 6 ) << "Bids" << "Asks\n"
            << ' '
            << setw( spacing + 3 ) << "Sum(WBD)"
            << setw( spacing + 1) << "WBD"
            << setw( spacing + 1 ) << "WORTH"
            << setw( spacing + 1 ) << "Price"
            << setw( spacing + 1 ) << "Price"
            << setw( spacing + 1 ) << "WORTH "
            << setw( spacing + 1 ) << "WBD " << "Sum(WBD)"
            << "\n====================================================================================================="
            << "|=====================================================================================================\n";
         ...
         for( size_t i = 0; i < orders.bids.size() || i < orders.asks.size(); i++ )
         {
            ...
            {
               bid_sum += asset( orders.bids[i].wbd, WBD_SYMBOL );
               ss
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( bid_sum ).to_string()
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.bids[i].wbd, WBD_SYMBOL ) ).to_string()
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.bids[i].worth, WORTH_SYMBOL ) ).to_string()
            ...
            {
               ask_sum += asset( orders.asks[i].wbd, WBD_SYMBOL );
               ss << ' ' << setw( spacing ) << orders.asks[i].real_price
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.asks[i].worth, WORTH_SYMBOL ) ).to_string()
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.asks[i].wbd, WBD_SYMBOL ) ).to_string()
   ...
   wallet_data                             _wallet;
   worth::protocol::chain_id_type          worth_chain_id;
...
wallet_api::wallet_api(const wallet_data& initial_data, const worth::protocol::chain_id_type& _worth_chain_id, fc::api< remote_node_api > rapi)
   : my(new detail::wallet_api_impl(*this, initial_data, _worth_chain_id, rapi))
{}
...
brain_key_info wallet_api::suggest_brain_key()const
{
   ...
   {
      fc::bigint choice = entropy % worth::words::word_list_size;
      entropy /= worth::words::word_list_size;
      if( i > 0 )
         brain_key += " ";
      brain_key += worth::words::word_list[ choice.to_int64() ];
   }
...
condenser_api::legacy_signed_transaction wallet_api::create_account_with_keys_delegated(
   string creator,
   condenser_api::legacy_asset worth_fee,
...
{ try {
   ...
   import_key( posting.wif_priv_key );
   import_key( memo.wif_priv_key );
   return create_account_with_keys_delegated( creator, worth_fee, delegated_vests, new_account_name, json_meta,  owner.pub_key, active.pub_key, posting.pub_key, memo.pub_key, broadcast );
} FC_CAPTURE_AND_RETHROW( (creator)(new_account_name)(json_meta) ) }
...
condenser_api::legacy_signed_transaction wallet_api::escrow_transfer(
   ...
   string agent,
   uint32_t escrow_id,
   condenser_api::legacy_asset wbd_amount,
   condenser_api::legacy_asset worth_amount,
   ...
   bool broadcast )
{
   ...
   op.wbd_amount = wbd_amount.to_asset();
   op.worth_amount = worth_amount.to_asset();
   op.fee = fee.to_asset();
...
condenser_api::legacy_signed_transaction wallet_api::convert_wbd(
   string from,
   condenser_api::legacy_asset amount,
   bool broadcast )
{
...
condenser_api::legacy_signed_transaction wallet_api::claim_reward_balance(
   string account,
   condenser_api::legacy_asset reward_worth,
   condenser_api::legacy_asset reward_wbd,
   ...
   bool broadcast )
{
   ...
   op.account = account;
   op.reward_worth = reward_worth.to_asset();
   op.reward_wbd = reward_wbd.to_asset();
   ...
   bool broadcast )
{
   ...
   op.author = author;
   op.permlink = permlink;
   op.weight = weight * WORTH_1_PERCENT;
