...
#include <worth/plugins/condenser_api/condenser_api.hpp>
...
using namespace worth::utilities;
...
{
   public:
      wallet_api( const wallet_data& initial_data, const worth::protocol::chain_id_type& _worth_chain_id, fc::api< remote_node_api > rapi );
      virtual ~wallet_api();
       ...
       *  'info' wallet command.
       *
       *  These accounts are created with combination of WORTH and delegated SP
       *
       *  @param creator The account creating the new account
       *  @param worth_fee The amount of the fee to be paid with WORTH
      ...
      /**
       * Transfer funds from one account to another. WORTH and WBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param amount The funds being transferred. i.e. "100.000 WORTH"
      ...
      /**
       * Transfer funds from one account to another using escrow. WORTH and WBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param agent The account acting as the agent in case of dispute
       * @param escrow_id A unique id for the escrow transfer. (from, escrow_id) must be a unique pair
       * @param wbd_amount The amount of WBD to transfer
       * @param worth_amount The amount of WORTH to transfer
       ...
       * @param escrow_id A unique id for the escrow transfer
       * @param wbd_amount The amount of WBD that will be released
       * @param worth_amount The amount of WORTH that will be released
       * @param broadcast true if you wish to broadcast the transaction
       */
      ...
      /**
       * Transfer WORTH into a vesting fund represented by vesting shares (VESTS). VESTS are required to vesting
       * for a minimum of one coin year and can be withdrawn once a week over a two year withdraw period.
       * VESTS are protected against dilution up until 90% of WORTH is vesting.
       *
       * @param from The account the WORTH is coming from
       * @param to The account getting the VESTS
       * @param amount The amount of WORTH to vest i.e. "100.00 WORTH"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *    withdrawn and deposited back as WORTH. i.e. "10.000000 VESTS"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *
       * @param from The account the VESTS are withdrawn from.
       * @param to   The account receiving either VESTS or WORTH.
      ...
      /**
       *  This method will convert WBD to WORTH at the current_median_history price one
       *  week from the time it is executed. This method depends upon there being a valid price feed.
       *
       *  @param from The account requesting conversion of its WBD i.e. "1.000 WBD"
       *  @param amount The amount of WBD to convert
       *  @param broadcast true if you wish to broadcast the transaction
       */
      condenser_api::legacy_signed_transaction convert_wbd(
         string from,
         condenser_api::legacy_asset amount,
         bool broadcast = false );

      /**
       * A witness can public a price feed for the WORTH:WBD market. The median price feed is used
       * to process conversion requests from WBD to WORTH.
       *
       ...
       *                       operations defined in `worth/chain/operations.hpp`
      ...
      /**
       * Gets the current order book for WORTH:WBD
       *
       ...
       *  @param amount_to_sell The amount of either WBD or WORTH you wish to sell
      ...
      /**
       * Vote on a comment to be paid WORTH
       *
       ...
       * @param daily_pay  - the amount of WBD that is being requested to be paid out daily,
...
FC_REFLECT( worth::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( worth::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( worth::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( worth::wallet::authority_type, (owner)(active)(posting) )

FC_API( worth::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
...
FC_REFLECT( worth::wallet::memo_data, (from)(to)(nonce)(check)(encrypted) )
