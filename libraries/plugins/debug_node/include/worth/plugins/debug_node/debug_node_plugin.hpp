...
#define WORTH_DEBUG_NODE_PLUGIN_NAME "debug_node"
...
struct debug_generate_blocks_args
{
   ...
   uint32_t                                  count = 0;
   uint32_t                                  skip = worth::chain::database::skip_nothing;
...
class debug_node_plugin : public plugin< debug_node_plugin >
{
   public:
      ...
      static const std::string& name() { static std::string name = WORTH_DEBUG_NODE_PLUGIN_NAME; return name; }
      ...
      void debug_update( Lambda&& callback, uint32_t skip = worth::chain::database::skip_nothing )
      {
   ...
   private:
      void on_post_apply_block( const worth::chain::block_notification& note );
...
FC_REFLECT( worth::plugins::debug_node::debug_generate_blocks_args,
            (debug_key)
            (count)
            (skip)
            (miss_blocks)
...
FC_REFLECT( worth::plugins::debug_node::debug_generate_blocks_return,
            (blocks)
          )
