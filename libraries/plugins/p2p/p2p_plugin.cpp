...
#include <worth/plugins/p2p/p2p_default_seeds.hpp>
...
using worth::protocol::block_header;
...
public:
   ...
   virtual worth::protocol::chain_id_type get_chain_id() const override;
...
worth::protocol::chain_id_type p2p_plugin_impl::get_chain_id() const
{
...
void p2p_plugin::plugin_initialize(const boost::program_options::variables_map& options)
{
   ...
   my->user_agent = "Worth Reference Implementation";
...
void p2p_plugin::broadcast_block( const worth::protocol::signed_block& block )
{
   ulog("Broadcasting block #${n}", ("n", block.block_num()));
   my->node->broadcast( graphene::net::block_message( block ) );
}

void p2p_plugin::broadcast_transaction( const worth::protocol::signed_transaction& tx )
{
