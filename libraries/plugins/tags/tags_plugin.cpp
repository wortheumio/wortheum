...
struct operation_visitor
{
   ...
   {
      if( _my._db.head_block_time() >= _my._promoted_start_time && op.to == WORTH_NULL_ACCOUNT && op.amount.symbol == WBD_SYMBOL )
      {
...
void tags_plugin::plugin_initialize(const boost::program_options::variables_map& options)
{
   ...
   WORTH_ADD_PLUGIN_INDEX(my->_db, tag_index);
   WORTH_ADD_PLUGIN_INDEX(my->_db, tag_stats_index);
   WORTH_ADD_PLUGIN_INDEX(my->_db, author_tag_stats_index);
