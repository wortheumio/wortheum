...
struct post_operation_visitor
{
   ...
   void operator()( const custom_json_operation& op )const
   {
      try
      {
         if( op.id == WORTH_FOLLOW_PLUGIN_NAME )
         {
...
void follow_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   try
   {
      ...
      _custom_operation_interpreter = std::make_shared< generic_custom_operation_interpreter< worth::plugins::follow::follow_plugin_operation > >( my->_db, name() );
      ...
      WORTH_ADD_PLUGIN_INDEX(my->_db, follow_index);
      WORTH_ADD_PLUGIN_INDEX(my->_db, feed_index);
      WORTH_ADD_PLUGIN_INDEX(my->_db, blog_index);
      WORTH_ADD_PLUGIN_INDEX(my->_db, reputation_index);
      WORTH_ADD_PLUGIN_INDEX(my->_db, follow_count_index);
      WORTH_ADD_PLUGIN_INDEX(my->_db, blog_author_stats_index);
