...
#include <worth/plugins/rewards_api/rewards_api_plugin.hpp>
#include <worth/plugins/rewards_api/rewards_api.hpp>
...
DEFINE_API_IMPL( rewards_api_impl, simulate_curve_payouts )
{
   ...
   auto reward_fund_object = _db.get< chain::reward_fund_object, chain::by_name >( WORTH_POST_REWARD_FUND_NAME );
   ...
   for ( std::size_t i = 0; i < ret.payouts.size(); ++i )
   {
      ...
      ret.payouts[ i ].payout = protocol::asset( static_cast< uint64_t >( payout_u256 ), WORTH_SYMBOL );
   }
...
} // worth::plugins::rewards_api::detail

rewards_api::rewards_api() : my( std::make_unique< detail::rewards_api_impl >() )
{
   JSON_RPC_REGISTER_API( WORTH_REWARDS_API_PLUGIN_NAME );
}
