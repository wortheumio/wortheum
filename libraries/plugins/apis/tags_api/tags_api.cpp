...
{
   public:
      tags_api_impl() : _db( appbase::app().get_plugin< worth::plugins::chain::chain_plugin >().db() ) {}
      ...
      chain::database& _db;
      std::shared_ptr< worth::plugins::follow::follow_api > _follow_api;
};
...
DEFINE_API_IMPL( tags_api_impl, get_discussions_by_promoted )
{
   ...
   auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type( WORTH_MAX_SHARE_SUPPLY ) )  );
...
void tags_api_impl::set_pending_payout( discussion& d )
{
   ...
   if( itr != cidx.end() && itr->comment == d.id )  {
      d.promoted = asset( itr->promoted_balance, WBD_SYMBOL );
   }
                                                        ...
                                                        )
{
   ...
   while( count > 0 && tidx_itr != tidx.end() )
   {
      ...
      try
      {
         ...
         result.discussions.back().promoted = asset(tidx_itr->promoted_balance, WBD_SYMBOL );
...
{
   JSON_RPC_REGISTER_API( WORTH_TAGS_API_PLUGIN_NAME );
}
...
{
   auto follow_api_plugin = appbase::app().find_plugin< worth::plugins::follow::follow_api_plugin >();
