...
uint32_t string_to_asset_num( const char* p, uint8_t decimals )
{
   ...
   switch( *p )
   {
      ...
      case 'S': case 'T': case 'U': case 'V': case 'W': case 'X': case 'Y': case 'Z':
      {
         ...
         switch( name_u64 )
         {
            case WORTH_SYMBOL_U64:
               FC_ASSERT( decimals == 3, "Incorrect decimal places" );
               asset_num = WORTH_ASSET_NUM_WORTH;
               break;
            case WBD_SYMBOL_U64:
               FC_ASSERT( decimals == 3, "Incorrect decimal places" );
               asset_num = WORTH_ASSET_NUM_WBD;
               break;
            case VESTS_SYMBOL_U64:
               FC_ASSERT( decimals == 6, "Incorrect decimal places" );
               asset_num = WORTH_ASSET_NUM_VESTS;
               break;
            default:
...
std::string asset_num_to_string( uint32_t asset_num )
{
   switch( asset_num )
   {
      ...
      case WORTH_ASSET_NUM_VESTS:
         return "VESTS";
      default:
         return "UNKN"; // WMTs will return this symbol if returned as a legacy asset
   }
}
...
legacy_asset legacy_asset::from_string( const string& from )
{
   try
   {
      ...
      if( dot_pos != std::string::npos )
      {
         ...
         //Max amount = 9223372036854775.807 WORTH/WBD
