#include <worth/plugins/condenser_api/condenser_api_legacy_operations.hpp>
...
void to_variant( const worth::plugins::condenser_api::legacy_operation& var,  fc::variant& vo )
{
   var.visit( from_operation( vo ) );
}

void from_variant( const fc::variant& var, worth::plugins::condenser_api::legacy_operation& vo )
{
   static std::map<string,int64_t> to_tag = []()
   {
      std::map<string,int64_t> name_map;
      for( int i = 0; i < worth::plugins::condenser_api::legacy_operation::count(); ++i )
      {
         worth::plugins::condenser_api::legacy_operation tmp;
         tmp.set_which(i);
         string n;
...
void to_variant( const worth::plugins::condenser_api::legacy_comment_options_extensions& sv, fc::variant& v )
{
   old_sv_to_variant( sv, v );
}

void from_variant( const fc::variant& v, worth::plugins::condenser_api::legacy_comment_options_extensions& sv )
{
   old_sv_from_variant( v, sv );
}

void to_variant( const worth::plugins::condenser_api::legacy_pow2_work& sv, fc::variant& v )
{
   old_sv_to_variant( sv, v );
}

void from_variant( const fc::variant& v, worth::plugins::condenser_api::legacy_pow2_work& sv )
{
   old_sv_from_variant( v, sv );
}
