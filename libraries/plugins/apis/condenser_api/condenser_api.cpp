...
#include <worth/plugins/account_history_api/account_history_api_plugin.hpp>
#include <worth/plugins/account_by_key_api/account_by_key_api_plugin.hpp>
#include <worth/plugins/network_broadcast_api/network_broadcast_api_plugin.hpp>
#include <worth/plugins/tags_api/tags_api_plugin.hpp>
#include <worth/plugins/follow_api/follow_api_plugin.hpp>
#include <worth/plugins/reputation_api/reputation_api_plugin.hpp>
#include <worth/plugins/market_history_api/market_history_api_plugin.hpp>
...
namespace detail
{
   ...
   class condenser_api_impl
   {
      public:
         condenser_api_impl() :
            _chain( appbase::app().get_plugin< worth::plugins::chain::chain_plugin >() ),
            _db( _chain.db() )
         {
            _on_post_apply_block_conn = _db.add_post_apply_block_handler(
               [&]( const block_notification& note ){ on_post_apply_block( note.block ); },
               appbase::app().get_plugin< worth::plugins::condenser_api::condenser_api_plugin >(),
               0 );
         }
         ...
         worth::plugins::chain::chain_plugin&                              _chain;
   ...
   {
      FC_ASSERT( false, "condenser_api::get_account_references --- Needs to be refactored for Worth." );
   }
   ...
   {
      CHECK_ARG_SIZE( 1 )
      auto requests = _database_api->find_wbd_conversion_requests(
         {
   ...
   DEFINE_API_IMPL( condenser_api_impl, list_proposals )
   {
      ...
      worth::plugins::database_api::list_proposals_args list_args;
      list_args.start           = args[0];
      list_args.limit           = args[1].as< uint32_t >();
      list_args.order           = args[2].as< worth::plugins::database_api::sort_order_type >();
      list_args.order_direction = args.size() > 3 ?
         args[3].as< worth::plugins::database_api::order_direction_type >() : database_api::ascending;
      list_args.status          = args.size() > 4 ?
         args[4].as< worth::plugins::database_api::proposal_status >() : database_api::all;
   ...
   DEFINE_API_IMPL( condenser_api_impl, find_proposals )
   {
      ...
      const auto& proposals = _database_api->find_proposals( { args[0].as< vector< worth::plugins::database_api::api_id_type > >() } ).proposals;
   ...
   void condenser_api_impl::set_pending_payout( discussion& d )
   {
      if( _tags_api )
      {
         ...
         {
            d.promoted = legacy_asset::from_asset( asset( itr->promoted_balance, WBD_SYMBOL ) );
         }
      }
      ...
      if( _db.has_hardfork( WORTH_HARDFORK_0_17__774 ) )
         ...
         pot = props.total_reward_fund_worth;
      ...
      if( d.parent_author != WORTH_ROOT_POST_PARENT )
...
{
   if( a.voting_manabar.last_update_time < WORTH_HARDFORK_0_20_TIME )
   ...
   // vp_t2 = WORTH_100_PERCENT * current_mana / vests
   // vp_t1 = vp_t2 - WORTH_100_PERCENT * (t2 - t1) / WORTH_VOTING_MANA_REGENERATION_SECONDS
   //

   uint32_t t1 = a.last_vote_time.sec_since_epoch();
   uint32_t t2 = a.voting_manabar.last_update_time;
   uint64_t dt = (t2 > t1) ? (t2 - t1) : 0;
   uint64_t vp_dt = WORTH_100_PERCENT * dt / WORTH_VOTING_MANA_REGENERATION_SECONDS;

   uint128_t vp_t2 = WORTH_100_PERCENT;
   vp_t2 *= a.voting_manabar.current_mana;
   vp_t2 /= vests;

   uint64_t vp_t2u = vp_t2.to_uint64();
   if( vp_t2u >= WORTH_100_PERCENT )
   {
      wlog( "Truncated vp_t2u to WORTH_100_PERCENT for account ${a}", ("a", a.name) );
      vp_t2u = WORTH_100_PERCENT;
   }
...
{
   JSON_RPC_REGISTER_API( WORTH_CONDENSER_API_PLUGIN_NAME );
}
