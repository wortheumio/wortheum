...
#include <worth/protocol/transaction_util.hpp>
...
class database_api_impl
{
   public:
      ...
      (
         ...
         (list_wbd_conversion_requests)
         (find_wbd_conversion_requests)
         ...
         (get_nai_pool)
         (list_wmt_contributions)
         (find_wmt_contributions)
         (list_wmt_tokens)
         (find_wmt_tokens)
         (list_wmt_token_emissions)
         (find_wmt_token_emissions)
#endif
      )
...
{
   JSON_RPC_REGISTER_API( WORTH_DATABASE_API_PLUGIN_NAME );
}

database_api::~database_api() {}

database_api_impl::database_api_impl()
   : _db( appbase::app().get_plugin< worth::plugins::chain::chain_plugin >().db() ) {}
...
{
   return worth::protocol::get_config();
}
...
DEFINE_API_IMPL( database_api_impl, get_version )
{
   ...
   (
      fc::string( WORTH_BLOCKCHAIN_VERSION ),
      fc::string( worth::utilities::git_revision_sha ),
...
/* WBD Conversion Requests */

DEFINE_API_IMPL( database_api_impl, list_wbd_conversion_requests )
{
   FC_ASSERT( args.limit <= DATABASE_API_SINGLE_QUERY_LIMIT );

   list_wbd_conversion_requests_return result;
   result.requests.reserve( args.limit );
...
DEFINE_API_IMPL( database_api_impl, find_wbd_conversion_requests )
{
   find_wbd_conversion_requests_return result;
...
DEFINE_API_IMPL( database_api_impl, get_order_book )
{
   ...
   auto max_sell = price::max( WBD_SYMBOL, WORTH_SYMBOL );
   auto max_buy = price::max( WORTH_SYMBOL, WBD_SYMBOL );
   ...
   while( sell_itr != end && sell_itr->sell_price.base.symbol == WBD_SYMBOL && result.bids.size() < args.limit )
   {
      auto itr = sell_itr;
      order cur;
      ...
      cur.wbd = itr->for_sale;
      cur.worth = ( asset( itr->for_sale, WBD_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
      result.bids.push_back( cur );
      ++sell_itr;
   }
   while( buy_itr != end && buy_itr->sell_price.base.symbol == WORTH_SYMBOL && result.asks.size() < args.limit )
   {
      auto itr = buy_itr;
      order cur;
      cur.order_price = itr->sell_price;
      cur.real_price = 0.0;
      // cur.real_price  = (~cur.order_price).to_real();
      cur.worth   = itr->for_sale;
      cur.wbd     = ( asset( itr->for_sale, WORTH_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
...
DEFINE_API_IMPL( database_api_impl, list_proposals )
{
   ...
   switch( args.order )
   {
      case by_creator:
      {
         ...
         iterate_results< worth::chain::proposal_index, worth::chain::by_creator >(
            boost::make_tuple( key.first, key.second ),
            result.proposals,
            args.limit,
      ...
      case by_start_date:
      {
         ...
         iterate_results< worth::chain::proposal_index, worth::chain::by_start_date >(
            boost::make_tuple( key.first, key.second ),
            result.proposals,
            args.limit,
            [&]( const proposal_object& po ){ return api_proposal_object( po, current_time ); },
      ...
      {
         auto key = args.start.as< std::pair< time_point_sec, api_id_type > >();
         iterate_results< worth::chain::proposal_index, worth::chain::by_end_date >(
            boost::make_tuple( key.first, key.second ),
            result.proposals,
            args.limit,
            [&]( const proposal_object& po ){ return api_proposal_object( po, current_time ); },
      ...
      case by_total_votes:
      {
         ...
         iterate_results< worth::chain::proposal_index, worth::chain::by_total_votes >(
            boost::make_tuple( key.first, key.second ),
            result.proposals,
            args.limit,
            [&]( const proposal_object& po ){ return api_proposal_object( po, current_time ); },
...
DEFINE_API_IMPL( database_api_impl, find_proposals )
{
   ...
   {
      auto po = _db.find< worth::chain::proposal_object, worth::chain::by_proposal_id >( id );
...
DEFINE_API_IMPL( database_api_impl, list_proposal_votes )
{
   ...
   switch( args.order )
   {
      ...
      {
         auto key = args.start.as< std::pair< account_name_type, api_id_type > >();
         iterate_results< worth::chain::proposal_vote_index, worth::chain::by_voter_proposal >(
            boost::make_tuple( key.first, key.second ),
            ...
            {
               auto itr = _db.find< worth::chain::proposal_object, worth::chain::by_id >( po.proposal_id );
      ...
      case by_proposal_voter:
      {
         ...
         iterate_results< worth::chain::proposal_vote_index, worth::chain::by_proposal_voter >(
            boost::make_tuple( key.first, key.second ),
            result.proposal_votes,
            args.limit,
            [&]( const proposal_vote_object& po ){ return api_proposal_vote_object( po, _db ); },
...
DEFINE_API_IMPL( database_api_impl, get_required_signatures )
{
   ...
   result.keys = args.trx.get_required_signatures( _db.get_chain_id(),
                                                   ...
                                                   WORTH_MAX_SIG_CHECK_DEPTH,
                                                   _db.has_hardfork( WORTH_HARDFORK_0_20__1944 ) ? fc::ecc::canonical_signature_type::bip_0062 : fc::ecc::canonical_signature_type::fc_canonical );
...
DEFINE_API_IMPL( database_api_impl, verify_authority )
{
   args.trx.verify_authority(_db.get_chain_id(),
                           ...
                           [&]( string account_name ){ return authority( _db.get< chain::account_authority_object, chain::by_account >( account_name ).posting ); },
                           WORTH_MAX_SIG_CHECK_DEPTH,
                           WORTH_MAX_AUTHORITY_MEMBERSHIP,
                           WORTH_MAX_SIG_CHECK_ACCOUNTS,
                           _db.has_hardfork( WORTH_HARDFORK_0_20__1944 ) ? fc::ecc::canonical_signature_type::bip_0062 : fc::ecc::canonical_signature_type::fc_canonical );
...
DEFINE_API_IMPL( database_api_impl, verify_signatures )
{
   ...
   try
   {
      worth::protocol::verify_authority< verify_signatures_args >(
         { args },
         sig_keys,
...
DEFINE_API_IMPL( database_api_impl, list_wmt_contributions )
{
   FC_ASSERT( args.limit <= DATABASE_API_SINGLE_QUERY_LIMIT );

   list_wmt_contributions_return result;
   ...
   switch( args.order )
   {
      case( by_symbol_contributor ):
      {
         ...
         iterate_results< chain::wmt_contribution_index, chain::by_symbol_contributor >(
            start,
            result.contributions,
            args.limit,
            &database_api_impl::on_push_default< chain::wmt_contribution_object >,
            &database_api_impl::filter_default< chain::wmt_contribution_object > );
         break;
      }
      case( by_symbol_id ):
      {
         ...
         boost::tuple< asset_symbol_type, wmt_contribution_object_id_type > start;
         if ( key.size() == 0 )
            start = boost::make_tuple( asset_symbol_type(), 0 );
         else
            start = boost::make_tuple( key[ 0 ].as< asset_symbol_type >(), key[ 1 ].as< wmt_contribution_object_id_type >() );

         iterate_results< chain::wmt_contribution_index, chain::by_symbol_id >(
            start,
            result.contributions,
            args.limit,
            &database_api_impl::on_push_default< chain::wmt_contribution_object >,
      ...
      case ( by_contributor ):
      {
         ...
         iterate_results< chain::wmt_contribution_index, chain::by_contributor >(
            start,
            result.contributions,
            args.limit,
            &database_api_impl::on_push_default< chain::wmt_contribution_object >,
...
DEFINE_API_IMPL( database_api_impl, find_wmt_contributions )
{
   find_wmt_contributions_return result;

   const auto& idx = _db.get_index< chain::wmt_contribution_index, chain::by_symbol_contributor >();
...
DEFINE_API_IMPL( database_api_impl, list_wmt_tokens )
{
   FC_ASSERT( args.limit <= DATABASE_API_SINGLE_QUERY_LIMIT );

   list_wmt_tokens_return result;
   ...
   switch( args.order )
   {
      case( by_symbol ):
      {
         ...
         iterate_results< chain::wmt_token_index, chain::by_symbol >(
            start,
            result.tokens,
            args.limit,
            [&]( const wmt_token_object& t ) { return api_wmt_token_object( t, _db ); },
            &database_api_impl::filter_default< chain::wmt_token_object > );
      ...
      case( by_control_account ):
      {
         ...
         iterate_results< chain::wmt_token_index, chain::by_control_account >(
            start,
            result.tokens,
            args.limit,
            [&]( const wmt_token_object& t ) { return api_wmt_token_object( t, _db ); },
...
DEFINE_API_IMPL( database_api_impl, find_wmt_tokens )
{
   FC_ASSERT( args.symbols.size() <= DATABASE_API_SINGLE_QUERY_LIMIT );

   find_wmt_tokens_return result;
   result.tokens.reserve( args.symbols.size() );

   for( auto& symbol : args.symbols )
   {
      const auto token = chain::util::wmt::find_token( _db, symbol, args.ignore_precision );
      if( token != nullptr )
      {
         result.tokens.push_back( api_wmt_token_object( *token, _db ) );
      }
   }
...
DEFINE_API_IMPL( database_api_impl, list_wmt_token_emissions )
{
   FC_ASSERT( args.limit <= DATABASE_API_SINGLE_QUERY_LIMIT );

   list_wmt_token_emissions_return result;
   ...
   switch( args.order )
   {
      case( by_symbol_time ):
      {
         ...
         iterate_results< chain::wmt_token_emissions_index, chain::by_symbol_time >(
            start,
            result.token_emissions,
            args.limit,
            &database_api_impl::on_push_default< chain::wmt_token_emissions_object >,
            &database_api_impl::filter_default< chain::wmt_token_emissions_object > );
         break;
      }
      default:
         FC_ASSERT( false, "Unknown or unsupported sort order" );
...
DEFINE_API_IMPL( database_api_impl, find_wmt_token_emissions )
{
   find_wmt_token_emissions_return result;

   const auto& idx = _db.get_index< chain::wmt_token_emissions_index, chain::by_symbol_time >();
