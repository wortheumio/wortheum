...
{
   public:
      market_history_api_impl() : _db( appbase::app().get_plugin< worth::plugins::chain::chain_plugin >().db() ) {}
...
DEFINE_API_IMPL( market_history_api_impl, get_ticker )
{
   ...
   {
      auto open = ASSET_TO_REAL( asset( itr->non_worth.open, WBD_SYMBOL ) ) / ASSET_TO_REAL( asset( itr->worth.open, WORTH_SYMBOL ) );
      itr = bucket_idx.lower_bound( boost::make_tuple( 0, _db.head_block_time() ) );
      result.latest = ASSET_TO_REAL( asset( itr->non_worth.close, WBD_SYMBOL ) ) / ASSET_TO_REAL( asset( itr->worth.close, WORTH_SYMBOL ) );
   ...
   result.worth_volume = volume.worth_volume;
   result.wbd_volume = volume.wbd_volume;
...
DEFINE_API_IMPL( market_history_api_impl, get_volume )
{
   ...
   do
   {
      result.worth_volume.amount += itr->worth.volume;
      result.wbd_volume.amount += itr->non_worth.volume;
...
DEFINE_API_IMPL( market_history_api_impl, get_order_book )
{
   ...
   auto itr = order_idx.lower_bound( price::max( WBD_SYMBOL, WORTH_SYMBOL ) );

   get_order_book_return result;

   while( itr != order_idx.end() && itr->sell_price.base.symbol == WBD_SYMBOL && result.bids.size() < args.limit )
   {
      order cur;
      cur.order_price = itr->sell_price;
      cur.real_price = ASSET_TO_REAL( itr->sell_price.base ) / ASSET_TO_REAL( itr->sell_price.quote );
      cur.worth = ( asset( itr->for_sale, WBD_SYMBOL ) * itr->sell_price ).amount;
   ...
   itr = order_idx.lower_bound( price::max( WORTH_SYMBOL, WBD_SYMBOL ) );

   while( itr != order_idx.end() && itr->sell_price.base.symbol == WORTH_SYMBOL && result.asks.size() < args.limit )
   {
      order cur;
      cur.order_price = itr->sell_price;
      cur.real_price = ASSET_TO_REAL( itr->sell_price.quote ) / ASSET_TO_REAL( itr->sell_price.base );
      cur.worth = itr->for_sale;
      cur.wbd = ( asset( itr->for_sale, WORTH_SYMBOL ) * itr->sell_price ).amount;
      cur.created = itr->created;
...
DEFINE_API_IMPL( market_history_api_impl, get_market_history_buckets )
{
   ...
   result.bucket_sizes = appbase::app().get_plugin< worth::plugins::market_history::market_history_plugin >().get_tracked_buckets();
   return result;
}
...
{
   JSON_RPC_REGISTER_API( WORTH_MARKET_HISTORY_API_PLUGIN_NAME );
}
