...
public:
   transaction_status_api_impl() :
      ...
      _tsp( appbase::app().get_plugin< worth::plugins::transaction_status::transaction_status_plugin >() ) {}
...
DEFINE_API_IMPL( transaction_status_api_impl, find_transaction )
{
   ...
   if ( _db.head_block_num() >= earliest_tracked_block_num )
   {
      ...
      if ( args.expiration.valid() )
      {
         ...
         if ( expiration < earliest_tracked_block->timestamp + WORTH_MAX_TIME_UNTIL_EXPIRATION )
            return {
...
} // worth::plugins::transaction_status_api::detail

transaction_status_api::transaction_status_api() : my( std::make_unique< detail::transaction_status_api_impl >() )
{
   JSON_RPC_REGISTER_API( WORTH_TRANSACTION_STATUS_API_PLUGIN_NAME );
}
