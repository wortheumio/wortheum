...
#include <worth/plugins/stats_export/stats_export_plugin.hpp>
...
using worth::plugins::chain::chain_plugin;
...
FC_REFLECT( worth::plugins::stats_export::detail::api_stats_transaction_data_object, (user)(size) )
FC_REFLECT( worth::plugins::stats_export::detail::api_stats_export_data_object, (global_properties)(transaction_stats)(free_memory) )

namespace worth { namespace plugins { namespace stats_export { namespace detail {
...
{
   std::shared_ptr< api_stats_export_data_object > stats = _export_plugin.find_export_data< api_stats_export_data_object >( WORTH_STATS_EXPORT_PLUGIN_NAME );
   if( !stats )
      return;
...
void stats_export_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      my->_export_plugin.register_export_data_factory( WORTH_STATS_EXPORT_PLUGIN_NAME,
