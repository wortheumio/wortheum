...
// time.
//
#ifndef WORTH_MARKET_HISTORY_SPACE_ID
#define WORTH_MARKET_HISTORY_SPACE_ID 7
#endif

#ifndef WORTH_MARKET_HISTORY_PLUGIN_NAME
#define WORTH_MARKET_HISTORY_PLUGIN_NAME "market_history"
#endif
...
{
   bucket_object_type        = ( WORTH_MARKET_HISTORY_SPACE_ID << 8 ),
   order_history_object_type = ( WORTH_MARKET_HISTORY_SPACE_ID << 8 ) + 1
};
...
class market_history_plugin : public plugin< market_history_plugin >
{
   public:
      ...
      static const std::string& name() { static std::string name = WORTH_MARKET_HISTORY_PLUGIN_NAME; return name; }
...
struct bucket_object : public object< bucket_object_type, bucket_object >
{
   ...
   bucket_object_details worth;
   bucket_object_details non_worth;

#ifdef WORTHEUM_ENABLE_WMT
   asset_symbol_type symbol = WBD_SYMBOL;

   price high()const { return asset( non_worth.high, symbol ) / asset( worth.high, WORTH_SYMBOL ); }
   price low()const { return asset( non_worth.low, symbol ) / asset( worth.low, WORTH_SYMBOL ); }
#else
   price high()const { return asset( non_worth.high, WBD_SYMBOL ) / asset( worth.high, WORTH_SYMBOL ); }
   price low()const { return asset( non_worth.low, WBD_SYMBOL ) / asset( worth.low, WORTH_SYMBOL ); }
#endif
};
...
FC_REFLECT( worth::plugins::market_history::bucket_object_details,
            (high)
            (low)
            (open)
            (close)
...
FC_REFLECT( worth::plugins::market_history::bucket_object,
                     (id)
                     (open)(seconds)
                     (worth)
#ifdef WORTHEUM_ENABLE_WMT
...
CHAINBASE_SET_INDEX_TYPE( worth::plugins::market_history::bucket_object, worth::plugins::market_history::bucket_index )

FC_REFLECT( worth::plugins::market_history::order_history_object,
                     (id)
                     (time)
                     (op) )
CHAINBASE_SET_INDEX_TYPE( worth::plugins::market_history::order_history_object, worth::plugins::market_history::order_history_index )
