...
using worth::protocol::fill_order_operation;
      ...
      for( const auto& bucket : _tracked_buckets )
      {
         ...
         if( itr == bucket_idx.end() )
         {
            _db.create< bucket_object >( [&]( bucket_object& b )
            {
               ...
               b.worth.fill( ( op.open_pays.symbol == WORTH_SYMBOL ) ? op.open_pays.amount : op.current_pays.amount );
#ifdef WORTHEUM_ENABLE_WMT
                  b.symbol = ( op.open_pays.symbol == WORTH_SYMBOL ) ? op.current_pays.symbol : op.open_pays.symbol;
#endif
                  b.non_worth.fill( ( op.open_pays.symbol == WORTH_SYMBOL ) ? op.current_pays.amount : op.open_pays.amount );
            });
         }
         else
         {
            _db.modify( *itr, [&]( bucket_object& b )
            {
               ...
               b.symbol = ( op.open_pays.symbol == WORTH_SYMBOL ) ? op.current_pays.symbol : op.open_pays.symbol;
#endif
               if( op.open_pays.symbol == WORTH_SYMBOL )
               {
                  b.worth.volume += op.open_pays.amount;
                  b.worth.close = op.open_pays.amount;

                  b.non_worth.volume += op.current_pays.amount;
                  b.non_worth.close = op.current_pays.amount;

                  if( b.high() < price( op.current_pays, op.open_pays ) )
                  {
                     b.worth.high = op.open_pays.amount;

                     b.non_worth.high = op.current_pays.amount;
                  }

                  if( b.low() > price( op.current_pays, op.open_pays ) )
                  {
                     b.worth.low = op.open_pays.amount;

                     b.non_worth.low = op.current_pays.amount;
                  }
               }
               else
               {
                  b.worth.volume += op.current_pays.amount;
                  b.worth.close = op.current_pays.amount;

                  b.non_worth.volume += op.open_pays.amount;
                  b.non_worth.close = op.open_pays.amount;

                  if( b.high() < price( op.open_pays, op.current_pays ) )
                  {
                     b.worth.high = op.current_pays.amount;

                     b.non_worth.high = op.open_pays.amount;
                  }

                  if( b.low() > price( op.open_pays, op.current_pays ) )
                  {
                     b.worth.low = op.current_pays.amount;

                     b.non_worth.low = op.open_pays.amount;
                  }
               }
            });
...
void market_history_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   try
   {
      ...
      WORTH_ADD_PLUGIN_INDEX(my->_db, bucket_index);
      WORTH_ADD_PLUGIN_INDEX(my->_db, order_history_index);
