...
#include <worth/plugins/rc/rc_curve.hpp>
#include <worth/plugins/rc/rc_export_objects.hpp>
...
#include <worth/jsonball/jsonball.hpp>

#include <boost/algorithm/string.hpp>

#define WORTH_RC_REGEN_TIME   (60*60*24*5)
// 2020.748973 VESTS == 1.000 WORTH when HF20 occurred on mainnet
// TODO: What should this value be for testnet?
#define WORTH_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT      2020748973

#ifndef IS_TEST_NET
#define WORTH_HF20_BLOCK_NUM                              0
#endif

// 1.66% is ~2 hours of regen.
// 2 / ( 24 * 5 ) = 0.01666...
#define WORTH_RC_MAX_NEGATIVE_PERCENT 166

namespace worth { namespace plugins { namespace rc {

using worth::plugins::block_data_export::block_data_export_plugin;

namespace detail {

using chain::plugin_exception;
using worth::chain::util::manabar_params;
...
void create_rc_account( database& db, uint32_t now, const account_object& account, asset max_rc_creation_adjustment )
{
   ...
   if( max_rc_creation_adjustment.symbol == WORTH_SYMBOL )
   {
   ...
   )
{
   if( account_name == account_name_type() )
   {
      if( db.is_producing() )
      {
         WORTH_ASSERT( false, plugin_exception,
   ...
   mbparams.regen_time = WORTH_RC_REGEN_TIME;
   ...
   db.modify( rc_account, [&]( rc_account_object& rca )
   {
      ...
      if( (!skip.skip_reject_not_enough_rc) && db.has_hardfork( WORTH_HARDFORK_0_20 ) )
      {
         if( db.is_producing() )
         {
            WORTH_ASSERT( has_mana, plugin_exception,
               "Account: ${account} has ${rc_current} RC, needs ${rc_needed} RC. Please wait to transact, or power up WORTH.",
      ...
      int64_t min_mana = -1 * ( WORTH_RC_MAX_NEGATIVE_PERCENT * mbparams.max_mana ) / WORTH_100_PERCENT;
...
void rc_plugin_impl::on_post_apply_transaction( const transaction_notification& note )
{
   ...
   int64_t rc_regen = (gpo.total_vesting_shares.amount.value / (WORTH_RC_REGEN_TIME / WORTH_BLOCK_INTERVAL));
   ...
   if( rc_regen > 0 )
   {
      for( size_t i=0; i<WORTH_NUM_RESOURCE_TYPES; i++ )
      {
   ...
   std::shared_ptr< exp_rc_data > export_data =
      worth::plugins::block_data_export::find_export_data< exp_rc_data >( WORTH_RC_PLUGIN_NAME );
      ...
      [&]( rc_pool_object& pool_obj )
      {
         ...
         for( size_t i=0; i<WORTH_NUM_RESOURCE_TYPES; i++ )
         {
            ...
            if( debug_print )
            {
               ...
               a /= k*double(pool);
               dlog( "a=${a}   aR=${aR}", ("a", a)("aR", a*gpo.total_vesting_shares.amount.value/WORTH_RC_REGEN_TIME) );
            }
         }
         if( debug_print )
         {
...
void rc_plugin_impl::on_first_block()
{
   ...
   std::string resource_params_json = worth::jsonball::get_resource_parameters();
   ...
   for( auto it=idx.begin(); it!=idx.end(); ++it )
   {
      create_rc_account( _db, now.sec_since_epoch(), *it, asset( WORTH_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT, VESTS_SYMBOL ) );
   }
...
struct pre_apply_operation_visitor
{
   ...
   void regenerate( const account_object& account, const rc_account_object& rc_account )const
   {
      ...
      // TODO:  Issue number
      //
      static_assert( WORTH_RC_REGEN_TIME <= WORTH_VOTING_MANA_REGENERATION_SECONDS, "RC regen time must be smaller than vote regen time" );
   ...
   {
      if( op.hardfork_id == WORTH_HARDFORK_0_1 )
      {
         const auto& idx = _db.get_index< account_index >().indices().get< by_id >();
         for( auto it=idx.begin(); it!=idx.end(); ++it )
         {
...
struct post_apply_operation_visitor
{
   ...
   void operator()( const pow_operation& op )const
   {
      ...
      create_rc_account< true >( _db, _current_time, op.worker_account, asset( 0, WORTH_SYMBOL ) );
   ...
   void operator()( const pow2_operation& op )const
   {
      ...
      create_rc_account< true >( _db, _current_time, worker_name, asset( 0, WORTH_SYMBOL ) );
   ...
   void operator()( const hardfork_operation& op )const
   {
      ...
      if( op.hardfork_id == WORTH_HARDFORK_0_20 )
      {
   ...
   void operator()( const clear_null_account_balance_operation& op )const
   {
      _mod_accounts.emplace_back( WORTH_NULL_ACCOUNT );
   }
...
void rc_plugin_impl::on_pre_apply_operation( const operation_notification& note )
{
   ...
   if( _db.has_hardfork( WORTH_HARDFORK_0_20 ) )
...
void rc_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      if( export_plugin != nullptr )
      {
         ...
         export_plugin->register_export_data_factory( WORTH_RC_PLUGIN_NAME,
            []() -> std::shared_ptr< exportable_block_data > { return std::make_shared< exp_rc_data >(); } );
      }

      chain::database& db = appbase::app().get_plugin< worth::plugins::chain::chain_plugin >().db();
      ...
      WORTH_ADD_PLUGIN_INDEX(db, rc_resource_param_index);
      WORTH_ADD_PLUGIN_INDEX(db, rc_pool_index);
      WORTH_ADD_PLUGIN_INDEX(db, rc_account_index);
      ...
      {
         my->_enable_at_block = WORTH_HF20_BLOCK_NUM;
      }
#else
