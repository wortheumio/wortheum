...
#include <worth/plugins/rc/rc_utility.hpp>
...
#ifndef WORTH_RC_SPACE_ID
#define WORTH_RC_SPACE_ID 16
#endif

#define WORTH_RC_DRC_FLOAT_LEVEL   (20*WORTH_1_PERCENT)
#define WORTH_RC_MAX_DRC_RATE      1000

enum rc_object_types
{
   rc_resource_param_object_type   = ( WORTH_RC_SPACE_ID << 8 ),
   rc_pool_object_type             = ( WORTH_RC_SPACE_ID << 8 ) + 1,
   rc_account_object_type          = ( WORTH_RC_SPACE_ID << 8 ) + 2,
   rc_delegation_pool_object_type  = ( WORTH_RC_SPACE_ID << 8 ) + 3,
   rc_indel_edge_object_type       = ( WORTH_RC_SPACE_ID << 8 ) + 4,
   rc_outdel_drc_edge_object_type  = ( WORTH_RC_SPACE_ID << 8 ) + 5
};
...
class rc_resource_param_object : public object< rc_resource_param_object_type, rc_resource_param_object >
{
   public:
      ...
      id_type               id;
      fc::int_array< rc_resource_params, WORTH_NUM_RESOURCE_TYPES >
...
class rc_account_object : public object< rc_account_object_type, rc_account_object >
{
   public:
      ...
      worth::chain::util::manabar   rc_manabar;
...
class rc_delegation_pool_object : public object< rc_delegation_pool_object_type, rc_delegation_pool_object >
{
   public:
      ...
      account_name_type             account;
      worth::chain::util::manabar   rc_pool_manabar;
};
 ...
 *
 * However, if the pool drops below WORTH_RC_DRC_FLOAT_LEVEL, DRC:RC exchange rate starts
 * to rise according to `f(x) = 1/(a+b*x)` where `x` is the pool level, and coefficients `a`,
 * `b` are set such that `f(WORTH_RC_DRC_FLOAT_LEVEL) = 1` and `f(0) = WORTH_RC_MAX_DRC_RATE`.
 *
...
class rc_outdel_drc_edge_object : public object< rc_outdel_drc_edge_object_type, rc_outdel_drc_edge_object >
{
   public:
      ...
      worth::chain::util::manabar   drc_manabar;
      int64_t                       drc_max_mana = 0;
};

int64_t get_maximum_rc( const worth::chain::account_object& account, const rc_account_object& rc_account );
...
FC_REFLECT( worth::plugins::rc::rc_resource_param_object, (id)(resource_param_array) )
CHAINBASE_SET_INDEX_TYPE( worth::plugins::rc::rc_resource_param_object, worth::plugins::rc::rc_resource_param_index )

FC_REFLECT( worth::plugins::rc::rc_pool_object, (id)(pool_array) )
CHAINBASE_SET_INDEX_TYPE( worth::plugins::rc::rc_pool_object, worth::plugins::rc::rc_pool_index )

FC_REFLECT( worth::plugins::rc::rc_account_object,
   (id)
   (account)
   (rc_manabar)
...
CHAINBASE_SET_INDEX_TYPE( worth::plugins::rc::rc_account_object, worth::plugins::rc::rc_account_index )

FC_REFLECT( worth::plugins::rc::rc_delegation_pool_object,
   (id)
   (account)
   (rc_pool_manabar)
   )
CHAINBASE_SET_INDEX_TYPE( worth::plugins::rc::rc_delegation_pool_object, worth::plugins::rc::rc_delegation_pool_index )

FC_REFLECT( worth::plugins::rc::rc_indel_edge_object,
   (id)
   (from_account)
   (to_pool)
   (amount)
   )
CHAINBASE_SET_INDEX_TYPE( worth::plugins::rc::rc_indel_edge_object, worth::plugins::rc::rc_indel_edge_index )

FC_REFLECT( worth::plugins::rc::rc_outdel_drc_edge_object,
   (id)
   (from_pool)
   (to_account)
   (drc_manabar)
...
CHAINBASE_SET_INDEX_TYPE( worth::plugins::rc::rc_outdel_drc_edge_object, worth::plugins::rc::rc_outdel_drc_edge_index )
