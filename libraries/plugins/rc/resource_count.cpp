...
#include <worth/plugins/rc/resource_sizes.hpp>
...
struct count_operation_visitor
{
   ...
   {
      FC_TODO( "Change RC state bytes computation to take WMT's into account" )
   ...
   void operator()( const wmt_setup_operation& op )const
   {
      FC_TODO( "Change RC state bytes computation to take WMT's into account" )
      execution_time_count += _e.wmt_setup_operation_exec_time;
   }

   void operator()( const wmt_setup_emissions_operation& op )const
   {
      FC_TODO( "Change RC state bytes computation to take WMT's into account" )
      execution_time_count += _e.wmt_setup_emissions_operation_exec_time;
   }

   void operator()( const wmt_set_setup_parameters_operation& op )const
   {
      FC_TODO( "Change RC state bytes computation to take WMT's into account" )
      execution_time_count += _e.wmt_set_setup_parameters_operation_exec_time;
   }

   void operator()( const wmt_set_runtime_parameters_operation& op )const
   {
      FC_TODO( "Change RC state bytes computation to take WMT's into account" )
      execution_time_count += _e.wmt_set_runtime_parameters_operation_exec_time;
   }

   void operator()( const wmt_create_operation& op )const
   {
      FC_TODO( "Change RC state bytes computation to take WMT's into account" )
      execution_time_count += _e.wmt_create_operation_exec_time;
   }
   ...
   void operator()( const wmt_contribute_operation& op ) const
   {
      FC_TODO( "Change RC state bytes computation to take WMT's into account" )
      execution_time_count += _e.wmt_contribute_operation_exec_time;
   }
#endif
   ...
   void operator()( const wps_fund_operation& ) const {}
   ...
   // claim_reward_balance, delegate_vesting_shares, any WMT operations
};
