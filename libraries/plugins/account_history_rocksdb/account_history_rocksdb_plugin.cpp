...
#include <worth/plugins/account_history_rocksdb/account_history_rocksdb_plugin.hpp>
...
#define WORTH_NAMESPACE_PREFIX "worth::protocol::"
...
namespace worth { namespace plugins { namespace account_history_rocksdb {

using worth::protocol::account_name_type;
using worth::protocol::block_id_type;
using worth::protocol::operation;
using worth::protocol::signed_block;
using worth::protocol::signed_block_header;
using worth::protocol::signed_transaction;

using worth::chain::operation_notification;
using worth::chain::transaction_id_type;

using worth::utilities::benchmark_dumper;
...
public:
   impl( account_history_rocksdb_plugin& self, const bpo::variables_map& options, const bfs::path& storagePath) :
      _self(self),
      _mainDb(appbase::app().get_plugin<worth::plugins::chain::chain_plugin>().db()),
      _storagePath(storagePath),
      _writeBuffer(_storage, _columnHandles)
      {
      collectOptions(options);

      _mainDb.add_pre_reindex_handler([&]( const worth::chain::reindex_notification& note ) -> void
         {
            on_pre_reindex( note );
         }, _self, 0);

      _mainDb.add_post_reindex_handler([&]( const worth::chain::reindex_notification& note ) -> void
         {
            on_post_reindex( note );
         }, _self, 0);

      WORTH_ADD_PLUGIN_INDEX(_mainDb, volatile_operation_index);
      }
   ...
   void on_pre_reindex( const worth::chain::reindex_notification& note );
   void on_post_reindex( const worth::chain::reindex_notification& note );
...
void account_history_rocksdb_plugin::impl::collectOptions(const boost::program_options::variables_map& options)
{
   ...
   WORTH_LOAD_VALUE_SET(options, "account-history-rocksdb-track-account-range", _tracked_accounts, pairstring);
...
std::vector<account_name_type> account_history_rocksdb_plugin::impl::getImpactedAccounts(const operation& op) const
{
   ...
   worth::app::operation_get_impacted_accounts(op, impacted);
...
void account_history_rocksdb_plugin::impl::storeOpFilteringParameters(const std::vector<std::string>& opList,
   flat_set<std::string>* storage) const
   {
      for(const auto& arg : opList)
      {
         ...
         {
            if( op.empty() == false )
               storage->insert( WORTH_NAMESPACE_PREFIX + op );
         }
      }
   }
...
void account_history_rocksdb_plugin::impl::on_pre_reindex(const worth::chain::reindex_notification& note)
{
...
void account_history_rocksdb_plugin::impl::on_post_reindex(const worth::chain::reindex_notification& note)
{
...
FC_REFLECT( worth::plugins::account_history_rocksdb::account_history_info,
