...
chain::signed_block block_producer::generate_block(fc::time_point_sec when, const chain::account_name_type& witness_owner, const fc::ecc::private_key& block_signing_private_key, uint32_t skip)
{
   ...
   worth::chain::detail::with_skip_flags(
      _db,
      skip,
      [&]()
      {
...
chain::signed_block block_producer::_generate_block(fc::time_point_sec when, const chain::account_name_type& witness_owner, const fc::ecc::private_key& block_signing_private_key)
{
   ...
   if( !(skip & chain::database::skip_witness_signature) )
      pending_block.sign( block_signing_private_key, _db.has_hardfork( WORTH_HARDFORK_0_20__1944 ) ? fc::ecc::bip_0062 : fc::ecc::fc_canonical );

   // TODO:  Move this to _push_block() so session is restored.
   if( !(skip & chain::database::skip_block_size_check) )
   {
      FC_ASSERT( fc::raw::pack_size(pending_block) <= WORTH_MAX_BLOCK_SIZE );
   }
...
void block_producer::adjust_hardfork_version_vote(const chain::witness_object& witness, chain::signed_block& pending_block)
{
   ...
   if( witness.running_version != WORTH_BLOCKCHAIN_VERSION )
      pending_block.extensions.insert( block_header_extensions( WORTH_BLOCKCHAIN_VERSION ) );

   const auto& hfp = _db.get_hardfork_property_object();
   const auto& hf_versions = _db.get_hardfork_versions();

   if( hfp.current_hardfork_version < WORTH_BLOCKCHAIN_VERSION // Binary is newer hardfork than has been applied
   ...
   else if( hfp.current_hardfork_version == WORTH_BLOCKCHAIN_VERSION // Binary does not know of a new hardfork
            && witness.hardfork_version_vote > WORTH_BLOCKCHAIN_VERSION ) // Voting for hardfork in the future, that we do not know of...
   {
        ...
        chain::signed_block& pending_block)
{
   ...
   uint64_t maximum_block_size = gpo.maximum_block_size; //WORTH_MAX_BLOCK_SIZE;
   uint64_t maximum_transaction_partition_size = maximum_block_size -  ( maximum_block_size * gpo.required_actions_partition_percent ) / WORTH_100_PERCENT;
   ...
   for( const chain::signed_transaction& tx : _db._pending_tx )
   {
      ...
      if( postponed_tx_count > WORTH_BLOCK_GENERATION_POSTPONED_TX_LIMIT )
         break;
