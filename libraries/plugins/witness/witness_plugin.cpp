...
#include <worth/plugins/witness/witness_plugin_objects.hpp>
...
#include <worth/chain/util/impacted.hpp>

#include <worth/utilities/key_conversion.hpp>
#include <worth/utilities/plugin_utilities.hpp>
...
namespace detail {
   ...
   public:
      witness_plugin_impl( boost::asio::io_service& io ) :
         _timer(io),
         _chain_plugin( appbase::app().get_plugin< worth::plugins::chain::chain_plugin >() ),
      ...
      uint32_t _required_witness_participation  = 33 * WORTH_1_PERCENT;
      uint32_t _production_skip_flags           = chain::database::skip_nothing;

      std::map< worth::protocol::public_key_type, fc::ecc::private_key > _private_keys;
      std::set< worth::protocol::account_name_type >                     _witnesses;
   ...
   struct comment_options_extension_visitor
   {
      ...
      {
         WORTH_ASSERT( cpb.beneficiaries.size() <= 8,
            plugin_exception,
   ...
   void check_memo( const string& memo, const chain::account_object& account, const account_authority_object& auth )
   {
      ...
      {
         for( auto& key : keys )
            WORTH_ASSERT( key_weight_pair.first != key,  plugin_exception,
      ...
      for( auto& key : keys )
         WORTH_ASSERT( memo_key != key,  plugin_exception,
   ...
   struct operation_visitor
   {
      ...
      void operator()( const comment_operation& o )const
      {
         if( o.parent_author != WORTH_ROOT_POST_PARENT )
         {
            ...
            if( parent != nullptr )
            WORTH_ASSERT( parent->depth < WORTH_SOFT_MAX_COMMENT_DEPTH,
               plugin_exception,
               "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",WORTH_SOFT_MAX_COMMENT_DEPTH) );
         }
      }
         ...
         case operation::tag< custom_binary_operation >::value:
            if( _db.is_producing() )
            {
               ...
               for( const account_name_type& account : impacted )
               {
                  ...
                  {
                     WORTH_ASSERT( coo->count < WITNESS_CUSTOM_OP_BLOCK_LIMIT, plugin_exception,
   ...
   {
      if( fc::time_point::now() < fc::time_point(WORTH_GENESIS_TIME) )
      {
         wlog( "waiting until genesis time to produce block: ${t}", ("t",WORTH_GENESIS_TIME) );
   ...
   block_production_condition::block_production_condition_enum witness_plugin_impl::maybe_produce_block(fc::mutable_variant_object& capture)
   {
      ...
      {
         capture("pct", uint32_t(100*uint64_t(prate) / WORTH_1_PERCENT));
      ...
      appbase::app().get_plugin< worth::plugins::p2p::p2p_plugin >().broadcast_block( block );
...
{ try {
   ...
   WORTH_LOAD_VALUE_SET( options, "witness", my->_witnesses, worth::protocol::account_name_type )
   ...
   if( options.count("private-key") )
   {
      ...
      {
         fc::optional<fc::ecc::private_key> private_key = worth::utilities::wif_to_key(wif_key);
   ...
   {
      my->_required_witness_participation = WORTH_1_PERCENT * options.at( "required-participation" ).as< uint32_t >();
   }
   ...
   WORTH_ADD_PLUGIN_INDEX(my->_db, witness_custom_op_index);
...
{ try {
   ...
   chain::database& d = appbase::app().get_plugin< worth::plugins::chain::chain_plugin >().db();

   if( !my->_witnesses.empty() )
   {
      ilog( "Launching block production for ${n} witnesses.", ("n", my->_witnesses.size()) );
      appbase::app().get_plugin< worth::plugins::p2p::p2p_plugin >().set_block_production( true );
