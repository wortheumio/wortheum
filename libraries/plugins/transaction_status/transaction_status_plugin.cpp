...
{
public:
   transaction_status_impl() : _db( appbase::app().get_plugin< worth::plugins::chain::chain_plugin >().db() ) {}
...
void transaction_status_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   try
   {
      ...
      my->actual_track_after_block = std::max< int64_t >( 0, int64_t( my->nominal_track_after_block ) - int64_t( WORTH_MAX_TIME_UNTIL_EXPIRATION / WORTH_BLOCK_INTERVAL ) );

      // We need to track 1 hour of blocks in addition to the depth the user would like us to track
      my->actual_block_depth = my->nominal_block_depth + ( WORTH_MAX_TIME_UNTIL_EXPIRATION / WORTH_BLOCK_INTERVAL );
      ...
      WORTH_ADD_PLUGIN_INDEX(my->_db, transaction_status_index);
