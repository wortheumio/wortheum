...
#include <worth/plugins/chain/abstract_block_producer.hpp>
#include <worth/plugins/chain/chain_plugin.hpp>
#include <worth/plugins/statsd/utility.hpp>

#include <worth/utilities/benchmark_dumper.hpp>
...
using fc::flat_map;
using worth::chain::block_id_type;
...
class chain_plugin_impl
{
   public:
      ...
      int16_t                          wps_remove_threshold = -1;
...
void chain_plugin_impl::write_default_database_config( bfs::path &p )
{
   ...
   fc::json::save_to_file( worth::utilities::default_database_configuration(), p );
}
...
const worth::chain::database& chain_plugin::db() const { return my->db; }
...
{
   cfg.add_options()
         ("wps-remove-threshold", bpo::value<uint16_t>()->default_value( 200 ), "Maximum numbers of proposals/votes which can be removed in the same cycle")
...
#endif
#ifdef IS_TEST_NET
         ("chain-id", bpo::value< std::string >()->default_value( WORTH_CHAIN_ID ), "chain ID to connect to")
#endif
         ;
}
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   my->wps_remove_threshold = options.at( "wps-remove-threshold" ).as< uint16_t >();
...
void chain_plugin::plugin_startup()
{
   ...
   {
      auto statsd = appbase::app().find_plugin< worth::plugins::statsd::statsd_plugin >();
      if( statsd != nullptr )
      {
   ...
   worth::utilities::benchmark_dumper dumper;

   const auto& abstract_index_cntr = my->db.get_abstract_index_cntr();

   typedef worth::utilities::benchmark_dumper::index_memory_details_cntr_t index_memory_details_cntr_t;
   ...
   db_open_args.initial_supply = WORTH_INIT_SUPPLY;
   db_open_args.wbd_initial_supply = WORTH_WBD_INIT_SUPPLY;
   ...
   db_open_args.wps_remove_threshold = my->wps_remove_threshold;
      ...
      const chainbase::database::abstract_index_cntr_t& abstract_index_cntr )
   {
      ...
      {
         typedef worth::utilities::benchmark_dumper::database_object_sizeof_cntr_t database_object_sizeof_cntr_t;
      ...
      const worth::utilities::benchmark_dumper::measurement& measure =
   ...
   if(my->replay)
   {
      ...
      db_open_args.benchmark = worth::chain::database::TBenchmark(my->benchmark_interval, benchmark_lambda);
      last_block_number = my->db.reindex( db_open_args );

      if( my->benchmark_interval > 0 )
      {
         const worth::utilities::benchmark_dumper::measurement& total_data = dumper.dump(true, get_indexes_memory_details);
   ...
   {
      db_open_args.benchmark = worth::chain::database::TBenchmark(dump_memory_details, benchmark_lambda);
...
bool chain_plugin::accept_block( const worth::chain::signed_block& block, bool currently_syncing, uint32_t skip )
{
...
void chain_plugin::accept_transaction( const worth::chain::signed_transaction& trx )
{
   boost::promise< void > prom;
   write_context cxt;
   cxt.req_ptr = &trx;
...
worth::chain::signed_block chain_plugin::generate_block(
   const fc::time_point_sec when,
   const account_name_type& witness_owner,
   const fc::ecc::private_key& block_signing_private_key,
   uint32_t skip )
...
bool chain_plugin::block_is_on_preferred_chain(const worth::chain::block_id_type& block_id )
{
   ...
   return db().get_block_id_for_num( worth::chain::block_header::num_from_id( block_id ) ) == block_id;
}

void chain_plugin::check_time_in_block( const worth::chain::signed_block& block )
{
...
} } } // namespace worth::plugis::chain::chain_apis
