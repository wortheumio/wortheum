...
#define WORTH_CHAIN_PLUGIN_NAME "chain"
...
public:
   ...
   static const std::string& name() { static std::string name = WORTH_CHAIN_PLUGIN_NAME; return name; }
   ...
   bool accept_block( const worth::chain::signed_block& block, bool currently_syncing, uint32_t skip );
   void accept_transaction( const worth::chain::signed_transaction& trx );
   worth::chain::signed_block generate_block(
   ...
   bool block_is_on_preferred_chain( const worth::chain::block_id_type& block_id );

   void check_time_in_block( const worth::chain::signed_block& block );
