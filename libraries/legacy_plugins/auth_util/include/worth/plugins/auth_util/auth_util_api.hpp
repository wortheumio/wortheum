...
{
   public:
      auth_util_api( const worth::app::api_context& ctx );
...
FC_REFLECT( worth::plugin::auth_util::check_authority_signature_params,
   (account_name)
   (level)
   (dig)
   (sigs)
   )
FC_REFLECT( worth::plugin::auth_util::check_authority_signature_result,
   (keys)
   )

FC_API( worth::plugin::auth_util::auth_util_api,
