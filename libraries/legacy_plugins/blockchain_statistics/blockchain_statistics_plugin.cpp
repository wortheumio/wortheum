#include <worth/blockchain_statistics/blockchain_statistics_api.hpp>

#include <worth/app/impacted.hpp>
...
#include <worth/chain/operation_notification.hpp>
...
struct operation_process
{
   ...
   void operator()( const transfer_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         if( op.amount.symbol == WORTH_SYMBOL )
            b.worth_transferred += op.amount.amount;
         else
            b.wbd_transferred += op.amount.amount;
      });
   }
   ...
   void operator()( const interest_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         b.wbd_paid_as_interest += op.interest.amount;
      });
   }
   ...
   void operator()( const author_reward_operation& op )const
   {
      ...
      {
         b.payouts++;
         b.wbd_paid_to_authors += op.wbd_payout.amount;
   ...
   void operator()( const transfer_to_vesting_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         b.worth_vested += op.amount.amount;
      });
   }
   ...
   void operator()( const fill_vesting_withdraw_operation& op )const
   {
      ...
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         if( op.deposited.symbol == WORTH_SYMBOL )
   ...
   void operator()( const convert_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         b.wbd_conversion_requests_created++;
         b.wbd_to_be_converted += op.amount.amount;
      });
   }
   ...
   void operator()( const fill_convert_request_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         b.wbd_conversion_requests_filled++;
         b.worth_converted += op.amount_out.amount;
      });
   }
};
...
void blockchain_statistics_plugin_impl::pre_operation( const operation_notification& o )
{
   ...
   for( const auto& bucket_id : _current_buckets )
   {
      ...
      else if( o.op.which() == operation::tag< withdraw_vesting_operation >::value )
      {
         ...
         auto new_vesting_withdrawal_rate = op.vesting_shares.amount / WORTH_VESTING_WITHDRAW_INTERVALS;
         if( op.vesting_shares.amount > 0 && new_vesting_withdrawal_rate == 0 )
            new_vesting_withdrawal_rate = 1;

         if( !db.has_hardfork( WORTH_HARDFORK_0_1 ) )
...
WORTH_DEFINE_PLUGIN( blockchain_statistics, worth::blockchain_statistics::blockchain_statistics_plugin );
