...
// time.
//
#ifndef WORTH_BLOCKCHAIN_STATISTICS_SPACE_ID
#define WORTH_BLOCKCHAIN_STATISTICS_SPACE_ID 9
#endif

#ifndef WORTH_BLOCKCHAIN_STATISTICS_PLUGIN_NAME
#define WORTH_BLOCKCHAIN_STATISTICS_PLUGIN_NAME "chain_stats"
#endif

namespace worth { namespace blockchain_statistics {
...
{
   bucket_object_type = ( WORTH_BLOCKCHAIN_STATISTICS_SPACE_ID << 8 )
};
...
class blockchain_statistics_plugin : public worth::app::plugin
{
   public:
      blockchain_statistics_plugin( application* app );
      virtual ~blockchain_statistics_plugin();

      virtual std::string plugin_name()const override { return WORTH_BLOCKCHAIN_STATISTICS_PLUGIN_NAME; }
...
struct bucket_object : public object< bucket_object_type, bucket_object >
{
   ...
   share_type           worth_transferred = 0;                       ///< WORTH transferred from account to account
   share_type           wbd_transferred = 0;                         ///< WBD transferred from account to account
   share_type           wbd_paid_as_interest = 0;                    ///< WBD paid as interest
   ...
   share_type           wbd_paid_to_authors = 0;                     ///< Ammount of WBD paid to authors
   ...
   share_type           liquidity_rewards_paid = 0;                  ///< Ammount of WORTH paid to market makers
   uint32_t             transfers_to_vesting = 0;                    ///< Transfers of WORTH into VESTS
   share_type           worth_vested = 0;                            ///< Ammount of WORTH vested
   ...
   share_type           vests_withdrawn = 0;                         ///< Ammount of VESTS withdrawn to WORTH
...
} } // worth::blockchain_statistics

FC_REFLECT( worth::blockchain_statistics::bucket_object,
   (id)
   (open)
   (seconds)
   (blocks)
...
CHAINBASE_SET_INDEX_TYPE( worth::blockchain_statistics::bucket_object, worth::blockchain_statistics::bucket_index )
