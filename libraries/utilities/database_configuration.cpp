...
namespace worth { namespace utilities {
...
FC_REFLECT( worth::utilities::database::configuration::shared_cache,
   (capacity)
);

FC_REFLECT( worth::utilities::database::configuration::write_buffer_manager,
   (write_buffer_size)
);

FC_REFLECT( worth::utilities::database::configuration::global,
   (shared_cache)
...
FC_REFLECT( worth::utilities::database::configuration::bloom_filter_policy,
   (bits_per_key)
   (use_block_based_builder)
);

FC_REFLECT( worth::utilities::database::configuration::block_based_table_options,
   (block_size)
   (cache_index_and_filter_blocks)
   (bloom_filter_policy)
);

FC_REFLECT( worth::utilities::database::configuration::base_index,
   (optimize_level_style_compaction)
   (increase_parallelism)
   (block_based_table_options)
);

FC_REFLECT( worth::utilities::database::configuration::configuration,
   (global)
   (base)
);
