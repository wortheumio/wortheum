...
//
//     WORTH_DEFINE_FIXED_STRING_IMPL( 32, fc::erpair< fc::uint128_t, fc::uint128_t > )
//
...
//
//     WORTH_DEFINE_FIXED_STRING_IMPL( 32, BOOST_IDENTITY_TYPE((fc::erpair< fc::uint128_t, fc::uint128_t >)) )
//
#define WORTH_DEFINE_FIXED_STRING_IMPL( SIZE, STORAGE_TYPE )       \
template<>                                                         \
struct fixed_string_impl_for_size< SIZE >                          \
{                                                                  \
   typedef worth::protocol::fixed_string_impl< STORAGE_TYPE > t;   \
};                                                                 \
                                                                   \
template<>                                                         \
...
WORTH_DEFINE_FIXED_STRING_IMPL( 16, BOOST_IDENTITY_TYPE((fc::uint128_t)) )
WORTH_DEFINE_FIXED_STRING_IMPL( 24, BOOST_IDENTITY_TYPE((fc::erpair< fc::uint128_t, uint64_t >)) )
WORTH_DEFINE_FIXED_STRING_IMPL( 32, BOOST_IDENTITY_TYPE((fc::erpair< fc::uint128_t, fc::uint128_t >)) )
...
inline void pack( Stream& s, const worth::protocol::fixed_string_impl< Storage >& u )
{
   pack( s, std::string( u ) );
}

template< typename Stream, typename Storage >
inline void unpack( Stream& s, worth::protocol::fixed_string_impl< Storage >& u, uint32_t depth )
{
   depth++;
   std::string str;
   unpack( s, str, depth );
...
void to_variant(   const worth::protocol::fixed_string_impl< Storage >& s, variant& v )
{
   v = std::string( s );
}

template< typename Storage >
void from_variant( const variant& v, worth::protocol::fixed_string_impl< Storage >& s )
{
   s = v.as_string();
}

template< typename Storage >
struct get_typename< worth::protocol::fixed_string_impl< Storage > >
{
   static const char* name()
   {
      static const std::string n =
         std::string("worth::protocol::fixed_string<") +
         std::to_string( worth::protocol::fixed_string_size_for_impl<Storage>::size ) +
         std::string(">");
      return n.c_str();
   }
};
