...
worth::protocol::transaction_id_type worth::protocol::transaction::id() const
{
   auto h = digest();
...
const signature_type& worth::protocol::signed_transaction::sign( const private_key_type& key, const chain_id_type& chain_id, canonical_signature_type canon_type )
{
...
signature_type worth::protocol::signed_transaction::sign( const private_key_type& key, const chain_id_type& chain_id, canonical_signature_type canon_type )const
{
   digest_type::encoder enc;
   fc::raw::pack( enc, chain_id );
   fc::raw::pack( enc, *this );
   ...
   ) const
{
   ...
   for( const public_key_type& k : s )
   {
      ...
      try
      {
         worth::protocol::verify_authority(
            operations,
            result,
            get_active,
            get_owner,
