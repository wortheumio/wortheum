...
namespace worth { namespace protocol {
   ...
   void validate_auth_size( const authority& a )
   {
      ...
      FC_ASSERT( size <= WORTH_MAX_AUTHORITY_MEMBERSHIP,
         "Authority membership exceeded. Max: ${max} Current: ${n}", ("max", WORTH_MAX_AUTHORITY_MEMBERSHIP)("n", size) );
   }

   void account_create_operation::validate() const
   {
      validate_account_name( new_account_name );
      FC_ASSERT( is_asset_type( fee, WORTH_SYMBOL ), "Account creation fee must be WORTH" );
      owner.validate();
      active.validate();
      ...
      FC_ASSERT( fee >= asset( 0, WORTH_SYMBOL ), "Account creation fee cannot be negative" );
   }
   ...
   {
      FC_ASSERT( title.size() < WORTH_COMMENT_TITLE_LIMIT,
         "Title size limit exceeded. Max: ${max} Current: ${n}", ("max", WORTH_COMMENT_TITLE_LIMIT - 1)("n", title.size()) );
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries.size() < WORTH_BENEFICIARY_LIMIT,
         "Cannot specify more than ${max} beneficiaries.", ("max", WORTH_BENEFICIARY_LIMIT - 1) ); // Require size serializtion fits in one byte.

      validate_account_name( beneficiaries[0].account );
      FC_ASSERT( beneficiaries[0].weight <= WORTH_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= WORTH_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= WORTH_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   {
      validate_account_name( author );
      FC_ASSERT( percent_worth_dollars <= WORTH_100_PERCENT, "Percent cannot exceed 100%" );
      FC_ASSERT( max_accepted_payout.symbol == WBD_SYMBOL, "Max accepted payout must be in WBD" );
   ...
   void claim_account_operation::validate()const
   {
      ...
      FC_ASSERT( is_asset_type( fee, WORTH_SYMBOL ), "Account creation fee must be WORTH" );
      FC_ASSERT( fee >= asset( 0, WORTH_SYMBOL ), "Account creation fee cannot be negative" );
      FC_ASSERT( fee <= asset( WORTH_MAX_ACCOUNT_CREATION_FEE, WORTH_SYMBOL ), "Account creation fee cannot be too large" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= WORTH_100_PERCENT, "Weight is not a WORTHIT percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( memo.size() < WORTH_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   {
      validate_account_name( from );
      FC_ASSERT( amount.symbol == WORTH_SYMBOL ||
                 ( amount.symbol.space() == asset_symbol_type::wmt_nai_space && amount.symbol.is_vesting() == false ),
                 "Amount must be WORTH or WMT liquid" );
   ...
   void set_withdraw_vesting_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= WORTH_100_PERCENT, "Percent must be valid worth percent" );
   }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( url.size() <= WORTH_MAX_WITNESS_URL_LENGTH, "URL is too long" );

      FC_ASSERT( url.size() > 0, "URL size must be greater than 0" );
      FC_ASSERT( fc::is_utf8( url ), "URL is not valid UTF8" );
      FC_ASSERT( fee >= asset( 0, WORTH_SYMBOL ), "Fee cannot be negative" );
   ...
   void witness_set_properties_operation::validate() const
   {
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_creation_fee.symbol == WORTH_SYMBOL, "account_creation_fee must be in WORTH" );
         FC_ASSERT( account_creation_fee.amount >= WORTH_MIN_ACCOUNT_CREATION_FEE, "account_creation_fee smaller than minimum account creation fee" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( maximum_block_size >= WORTH_MIN_BLOCK_SIZE_LIMIT, "maximum_block_size smaller than minimum max block size" );
      }

      itr = props.find( "wbd_interest_rate" );
      if( itr != props.end() )
      {
         uint16_t wbd_interest_rate;
         fc::raw::unpack_from_vector( itr->second, wbd_interest_rate );
         FC_ASSERT( wbd_interest_rate >= 0, "wbd_interest_rate must be positive" );
         FC_ASSERT( wbd_interest_rate <= WORTH_100_PERCENT, "wbd_interest_rate must not exceed 100%" );
      }
      ...
      itr = props.find( "wbd_exchange_rate" );
      if( itr != props.end() )
      {
         price wbd_exchange_rate;
         fc::raw::unpack_from_vector( itr->second, wbd_exchange_rate );
         FC_ASSERT( ( is_asset_type( wbd_exchange_rate.base, WBD_SYMBOL ) && is_asset_type( wbd_exchange_rate.quote, WORTH_SYMBOL ) ),
            "Price feed must be a WORTH/WBD price" );
         wbd_exchange_rate.validate();
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_budget >= WORTH_RD_MIN_BUDGET, "Budget must be at least ${n}", ("n", WORTH_RD_MIN_BUDGET) );
         FC_ASSERT( account_subsidy_budget <= WORTH_RD_MAX_BUDGET, "Budget must be at most ${n}", ("n", WORTH_RD_MAX_BUDGET) );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_decay >= WORTH_RD_MIN_DECAY, "Decay must be at least ${n}", ("n", WORTH_RD_MIN_DECAY) );
         FC_ASSERT( account_subsidy_decay <= WORTH_RD_MAX_DECAY, "Decay must be at most ${n}", ("n", WORTH_RD_MAX_DECAY) );
      }
   }
   ...
   void custom_json_operation::validate() const {
      ...
      FC_ASSERT( id.size() <= WORTH_CUSTOM_OP_ID_MAX_LENGTH,
         "Operation ID length exceeded. Max: ${max} Current: ${n}", ("max", WORTH_CUSTOM_OP_ID_MAX_LENGTH)("n", id.size()) );
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( WORTH_EQUIHASH_N, WORTH_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == WORTH_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == WORTH_EQUIHASH_K, "proof of work 'k' value is incorrect" );
   ...
   void feed_publish_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( exchange_rate.base, WORTH_SYMBOL ) && is_asset_type( exchange_rate.quote, WBD_SYMBOL ) )
         || ( is_asset_type( exchange_rate.base, WBD_SYMBOL ) && is_asset_type( exchange_rate.quote, WORTH_SYMBOL ) ),
         "Price feed must be a WORTH/WBD price" );
   ...
   void limit_order_create_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, WORTH_SYMBOL ) && is_asset_type( min_to_receive, WBD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, WBD_SYMBOL ) && is_asset_type( min_to_receive, WORTH_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::wmt_nai_space
                     && is_asset_type( min_to_receive, WORTH_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, WORTH_SYMBOL )
                     && min_to_receive.symbol.space() == asset_symbol_type::wmt_nai_space
                  ),
               "Limit order must be for the WORTH:WBD or WMT:(WORTH/WBD) market" );
   ...
   void limit_order_create2_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, WORTH_SYMBOL ) && is_asset_type( exchange_rate.quote, WBD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, WBD_SYMBOL ) && is_asset_type( exchange_rate.quote, WORTH_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::wmt_nai_space
                     && is_asset_type( exchange_rate.quote, WORTH_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, WORTH_SYMBOL )
                     && exchange_rate.quote.symbol.space() == asset_symbol_type::wmt_nai_space
                  ),
               "Limit order must be for the WORTH:WBD or WMT:(WORTH/WBD) market" );
   ...
   {
      validate_account_name( owner );
      /// only allow conversion from WBD to WORTH, allowing the opposite can enable traders to abuse
      /// market fluxuations through converting large quantities without moving the price.
      FC_ASSERT( is_asset_type( amount, WBD_SYMBOL ), "Can only convert WBD to WORTH" );
      FC_ASSERT( amount.amount > 0, "Must convert some WBD" );
   }
   ...
   void escrow_transfer_operation::validate()const
   {
      ...
      FC_ASSERT( wbd_amount.amount >= 0, "wbd amount cannot be negative" );
      FC_ASSERT( worth_amount.amount >= 0, "worth amount cannot be negative" );
      FC_ASSERT( wbd_amount.amount > 0 || worth_amount.amount > 0, "escrow must transfer a non-zero amount" );
      FC_ASSERT( from != agent && to != agent, "agent must be a third party" );
      FC_ASSERT( (fee.symbol == WORTH_SYMBOL) || (fee.symbol == WBD_SYMBOL), "fee must be WORTH or WBD" );
      FC_ASSERT( wbd_amount.symbol == WBD_SYMBOL, "wbd amount must contain WBD" );
      FC_ASSERT( worth_amount.symbol == WORTH_SYMBOL, "worth amount must contain WORTH" );
   ...
   void escrow_release_operation::validate()const
   {
      ...
      FC_ASSERT( wbd_amount.amount >= 0, "wbd amount cannot be negative" );
      FC_ASSERT( worth_amount.amount >= 0, "worth amount cannot be negative" );
      FC_ASSERT( wbd_amount.amount > 0 || worth_amount.amount > 0, "escrow must release a non-zero amount" );
      FC_ASSERT( wbd_amount.symbol == WBD_SYMBOL, "wbd amount must contain WBD" );
      FC_ASSERT( worth_amount.symbol == WORTH_SYMBOL, "worth amount must contain WORTH" );
   }
   ...
   void transfer_to_savings_operation::validate()const {
      ...
      FC_ASSERT( amount.symbol == WORTH_SYMBOL || amount.symbol == WBD_SYMBOL );
      FC_ASSERT( memo.size() < WORTH_MAX_MEMO_SIZE, "Memo is too large" );
      FC_ASSERT( fc::is_utf8( memo ), "Memo is not UTF8" );
   }
   ...
   {
      validate_account_name( account );
      FC_ASSERT( is_asset_type( reward_worth, WORTH_SYMBOL ), "Reward Worth must be WORTH" );
      FC_ASSERT( is_asset_type( reward_wbd, WBD_SYMBOL ), "Reward Worth must be WBD" );
      FC_ASSERT( is_asset_type( reward_vests, VESTS_SYMBOL ), "Reward Worth must be VESTS" );
      FC_ASSERT( reward_worth.amount >= 0, "Cannot claim a negative amount" );
      FC_ASSERT( reward_wbd.amount >= 0, "Cannot claim a negative amount" );
      FC_ASSERT( reward_vests.amount >= 0, "Cannot claim a negative amount" );
      FC_ASSERT( reward_worth.amount > 0 || reward_wbd.amount > 0 || reward_vests.amount > 0, "Must claim something." );
   }
