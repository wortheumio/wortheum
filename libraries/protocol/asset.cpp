...
{
   static_assert( WORTH_ASSET_SYMBOL_NAI_STRING_LENGTH >= 12, "This code will overflow a short buffer" );
   uint32_t x = to_nai();
   buf[11] = '\0';
   buf[10] = ((x%10)+'0');  x /= 10;
   buf[ 9] = ((x%10)+'0');  x /= 10;
...
asset_symbol_type asset_symbol_type::from_nai_string( const char* p, uint8_t decimal_places )
{
   try
   {
      ...
      FC_ASSERT( std::strlen( p ) == WORTH_ASSET_SYMBOL_NAI_STRING_LENGTH - 1, "Incorrect NAI string length" );
...
uint32_t asset_symbol_type::asset_num_from_nai( uint32_t nai, uint8_t decimal_places )
{
   ...
   FC_ASSERT( (nai_data_digits >= WMT_MIN_NAI) & (nai_data_digits <= WMT_MAX_NAI), "NAI out of range" );
   FC_ASSERT( nai_check_digit == damm_checksum_8digit(nai_data_digits), "Invalid check digit" );

   switch( nai_data_digits )
   {
      case WORTH_NAI_WORTH:
         FC_ASSERT( decimal_places == WORTH_PRECISION_WORTH );
         return WORTH_ASSET_NUM_WORTH;
      case WORTH_NAI_WBD:
         FC_ASSERT( decimal_places == WORTH_PRECISION_WBD );
         return WORTH_ASSET_NUM_WBD;
      case WORTH_NAI_VESTS:
         FC_ASSERT( decimal_places == WORTH_PRECISION_VESTS );
         return WORTH_ASSET_NUM_VESTS;
      default:
         FC_ASSERT( decimal_places <= WORTH_ASSET_MAX_DECIMALS, "Invalid decimal_places" );
         return (nai_data_digits << WORTH_NAI_SHIFT) | WMT_ASSET_NUM_CONTROL_MASK | decimal_places;
   }
}
...
uint32_t asset_symbol_type::to_nai()const
{
   ...
   switch( asset_num )
   {
      case WORTH_ASSET_NUM_WORTH:
         nai_data_digits = WORTH_NAI_WORTH;
         break;
      case WORTH_ASSET_NUM_WBD:
         nai_data_digits = WORTH_NAI_WBD;
         break;
      case WORTH_ASSET_NUM_VESTS:
         nai_data_digits = WORTH_NAI_VESTS;
         break;
      default:
         FC_ASSERT( space() == wmt_nai_space );
         nai_data_digits = (asset_num >> WORTH_NAI_SHIFT);
   }
...
bool asset_symbol_type::is_vesting() const
{
   switch( space() )
   {
      ...
      case wmt_nai_space:
         ...
         return asset_num & WMT_ASSET_NUM_VESTING_MASK;
      default:
         FC_ASSERT( false, "Unknown asset symbol" );
   }
}
...
asset_symbol_type asset_symbol_type::get_paired_symbol() const
{
   switch( space() )
   {
      case legacy_space:
      {
         ...
         {
            case WORTH_ASSET_NUM_WORTH:
               return from_asset_num( WORTH_ASSET_NUM_VESTS );
            case WORTH_ASSET_NUM_WBD:
               return *this;
            case WORTH_ASSET_NUM_VESTS:
               return from_asset_num( WORTH_ASSET_NUM_WORTH );
            default:
               FC_ASSERT( false, "Unknown asset symbol" );
         }
      }
      case wmt_nai_space:
         ...
         auto paired_asset_num = asset_num ^ ( WMT_ASSET_NUM_VESTING_MASK );
...
void asset_symbol_type::validate()const
{
   switch( asset_num )
   {
      ...
      default:
      {
         uint32_t nai_data_digits = (asset_num >> WORTH_NAI_SHIFT);
         uint32_t nai_1bit = (asset_num & WMT_ASSET_NUM_CONTROL_MASK);
         uint32_t nai_decimal_places = (asset_num & WMT_ASSET_NUM_PRECISION_MASK);
         FC_ASSERT( (nai_data_digits >= WMT_MIN_NAI) &
                    (nai_data_digits <= WMT_MAX_NAI) &
                    (nai_1bit == WMT_ASSET_NUM_CONTROL_MASK) &
                    (nai_decimal_places <= WORTH_ASSET_MAX_DECIMALS),
                    "Cannot determine space for asset ${n}", ("n", asset_num) );
      }
   }
   // this assert is duplicated by above code in all cases
   // FC_ASSERT( decimals() <= WORTH_ASSET_MAX_DECIMALS );
}
...
void asset::validate()const
{
   ...
   FC_ASSERT( amount.value <= WORTH_MAX_SATOSHIS );
}
...
DEFINE_PRICE_COMPARISON_OPERATOR( >= )
      ...
      price price::max( asset_symbol_type base, asset_symbol_type quote ) { return asset( share_type(WORTH_MAX_SATOSHIS), base ) / asset( share_type(1), quote); }
      price price::min( asset_symbol_type base, asset_symbol_type quote ) { return asset( 1, base ) / asset( WORTH_MAX_SATOSHIS, quote); }
...
namespace fc {
   void to_variant( const worth::protocol::asset& var, fc::variant& vo )
   {
      try
      {
   ...
   void from_variant( const fc::variant& var, worth::protocol::asset& vo )
   {
      try
      {
         ...
         vo.symbol = worth::protocol::asset_symbol_type::from_nai_string( v_object[ ASSET_NAI_KEY ].as< std::string >().c_str(), v_object[ ASSET_PRECISION_KEY ].as< uint8_t >() );
      } FC_CAPTURE_AND_RETHROW()
   }
}
