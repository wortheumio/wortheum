 ...
 * the pool is full (WMT_MAX_NAI_POOL_COUNT) or we encounter the maximum
 * acceptable collisions (WMT_MAX_NAI_GENERATION_TRIES). If we hit the
...
void replenish_nai_pool( database& db )
{
   try
   {
       ...
       */
      if ( collisions_per_block >= WMT_MAX_NAI_GENERATION_TRIES )
         return;

      const nai_pool_object& npo = db.get< nai_pool_object >();
      while ( npo.num_available_nais < WMT_MAX_NAI_POOL_COUNT )
      {
         ...
         for (;;)
         {
            ...
            // We must ensure the NAI is not an WMT, and it is not already contained within the NAI pool
            if ( !util::wmt::find_token( db, next_sym, true ) && !npo.contains( next_sym ) )
               break;
...
void remove_from_nai_pool( database &db, const asset_symbol_type& a )
{
   ...
   auto it = std::find( nais.begin(), end, asset_symbol_type::from_asset_num( a.get_stripped_precision_wmt_num() ) );
   if ( it != end )
   {
