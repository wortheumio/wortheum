...
using worth::protocol::operation;

using worth::chain::proposal_object;
using worth::chain::by_start_date;
using worth::chain::by_end_date;
using worth::chain::proposal_index;
using worth::chain::proposal_id_type;
using worth::chain::proposal_vote_index;
using worth::chain::by_proposal_voter;
using worth::chain::by_voter_proposal;
using worth::protocol::proposal_pay_operation;
using worth::chain::wps_helper;
using worth::chain::dynamic_global_property_object;
using worth::chain::block_notification;

const std::string wps_processor::removing_name = "wps_processor_remove";
const std::string wps_processor::calculating_name = "wps_processor_calculate";

bool wps_processor::is_maintenance_period( const time_point_sec& head_time ) const
{
   return db.get_dynamic_global_properties().next_maintenance_time <= head_time;
}

void wps_processor::remove_proposals( const time_point_sec& head_time )
{
   ...
   while( itr != found )
   {
      itr = wps_helper::remove_proposal< by_end_date >( itr, proposalIndex, votesIndex, byVoterIdx, obj_perf );
      if( obj_perf.done )
         break;
   }
}

void wps_processor::find_active_proposals( const time_point_sec& head_time, t_proposals& proposals )
{
...
uint64_t wps_processor::calculate_votes( const proposal_id_type& id )
{
   uint64_t ret = 0;
   ...
   while( found != pvidx.end() && found->proposal_id == id )
   {
      ...
      if( _voter.proxy == WORTH_PROXY_TO_SELF_ACCOUNT )
      {
...
void wps_processor::calculate_votes( const t_proposals& proposals )
{
...
void wps_processor::sort_by_votes( t_proposals& proposals )
{
...
asset wps_processor::get_treasury_fund()
{
   auto& treasury_account = db.get_account( WORTH_TREASURY_ACCOUNT );

   return treasury_account.wbd_balance;
}

asset wps_processor::get_daily_inflation()
{
   FC_TODO( "to invent how to get inflation needed for WORTH_TREASURY_ACCOUNT" )
   return asset( 0, WBD_SYMBOL );
}

asset wps_processor::calculate_maintenance_budget( const time_point_sec& head_time )
{
...
void wps_processor::transfer_daily_inflation_to_treasury( const asset& daily_inflation )
{
   /*
   ...
   */
   FC_TODO( "to choose how to transfer inflation into WORTH_TREASURY_ACCOUNT" )
   // Ifdeffing this out so that no inflation is accidentally created on main net.
#ifdef IS_TEST_NET
   if( daily_inflation.amount.value > 0 )
   {
      const auto& treasury_account = db.get_account( WORTH_TREASURY_ACCOUNT );
      db.adjust_balance( treasury_account, daily_inflation );
   }
#endif
}

void wps_processor::transfer_payments( const time_point_sec& head_time, asset& maintenance_budget_limit, const t_proposals& proposals )
{
   ...
   uint32_t passed_time_seconds = ( head_time - db.get_dynamic_global_properties().last_budget_time ).to_seconds();
   uint128_t ratio = ( passed_time_seconds * WORTH_100_PERCENT ) / daily_seconds;
   ...
   for( auto& item : proposals )
   {
      ...
      asset period_pay = asset( ( ratio * _item.daily_pay.amount.value ).to_uint64() / WORTH_100_PERCENT, _item.daily_pay.symbol );
...
void wps_processor::update_settings( const time_point_sec& head_time )
{
   db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& _dgpo )
                                          {
                                             _dgpo.next_maintenance_time = head_time + fc::seconds( WORTH_PROPOSAL_MAINTENANCE_PERIOD );
                                             _dgpo.last_budget_time = head_time;
                                          } );
}

void wps_processor::remove_old_proposals( const block_notification& note )
{
   ...
   if( db.get_benchmark_dumper().is_enabled() )
      db.get_benchmark_dumper().end( wps_processor::removing_name );
}

void wps_processor::make_payments( const block_notification& note )
{
   auto head_time = note.block.timestamp;
   ...
   {
      if( db.get_benchmark_dumper().is_enabled() )
         db.get_benchmark_dumper().end( wps_processor::calculating_name );
...
const std::string& wps_processor::get_removing_name()
{
   return removing_name;
}

const std::string& wps_processor::get_calculating_name()
{
   return calculating_name;
}

void wps_processor::run( const block_notification& note )
{
...
void wps_processor::record_funding( const block_notification& note )
{
   ...
   if ( props.wps_interval_ledger.amount.value <= 0 )
      return;

   operation vop = wps_fund_operation( props.wps_interval_ledger );
   db.push_virtual_operation( vop );

   db.modify( props, []( dynamic_global_property_object& dgpo )
   {
      dgpo.wps_interval_ledger = asset( 0, WBD_SYMBOL );
   });
}
