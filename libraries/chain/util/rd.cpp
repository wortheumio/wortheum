...
{
   FC_ASSERT( user_params.budget_per_time_unit >= WORTH_RD_MIN_BUDGET );
   FC_ASSERT( user_params.budget_per_time_unit <= WORTH_RD_MAX_BUDGET );
   FC_ASSERT( user_params.decay_per_time_unit >= WORTH_RD_MIN_DECAY );
   FC_ASSERT( user_params.decay_per_time_unit <= WORTH_RD_MAX_DECAY );
}
   ...
   rd_dynamics_params& dparams_out )
{
   ...
   //   WORTH_RD_DECAY_DENOM_SHIFT.  The interface is designed to support other values of this
   ...
   //   code so that it sets decay_per_time_unit_denom_shift = WORTH_RD_DECAY_DENOM_SHIFT,
   ...
   //
   FC_ASSERT( system_params.decay_per_time_unit_denom_shift == WORTH_RD_DECAY_DENOM_SHIFT );
   ...
   static_assert( WORTH_RD_MAX_BUDGET < (uint64_t(1) << (64-WORTH_RD_DECAY_DENOM_SHIFT)),
      "Computation of temp could overflow here, set smaller WORTH_RD_MAX_BUDGET" );
