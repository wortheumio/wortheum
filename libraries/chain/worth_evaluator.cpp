...
{
   FC_ASSERT( permlink.size() > WORTH_MIN_PERMLINK_LENGTH && permlink.size() < WORTH_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
...
void copy_legacy_chain_properties( chain_properties& dest, const legacy_chain_properties& src )
{
   ...
   dest.wbd_interest_rate = src.wbd_interest_rate;
}
...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
   ...
   if ( _db.has_hardfork( WORTH_HARDFORK_0_14__410 ) )
   {
      FC_ASSERT( o.props.account_creation_fee.symbol.is_canon() );
      if( _db.has_hardfork( WORTH_HARDFORK_0_20__2651 ) || _db.is_producing() )
      {
         FC_TODO( "Move to validate() after HF20" );
         FC_ASSERT( o.props.account_creation_fee.amount <= WORTH_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
      }
   }
   ...
   if( _db.is_producing() || _db.has_hardfork( WORTH_HARDFORK_0_20__2642 ))
   {
      FC_ASSERT( o.props.maximum_block_size <= WORTH_SOFT_MAX_BLOCK_SIZE, "Max block size cannot be more than 2MiB" );
   }
...
void witness_set_properties_evaluator::do_apply( const witness_set_properties_operation& o )
{
   ...
   price             wbd_exchange_rate;
   time_point_sec    last_wbd_exchange_update;
   string            url;
   ...
   if( flags.account_creation_changed )
   {
      ...
      {
         FC_TODO( "Move to validate() after HF20" );
         FC_ASSERT( props.account_creation_fee.amount <= WORTH_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
      }
   }
   ...
   itr = o.props.find( "wbd_interest_rate" );
   flags.wbd_interest_changed = itr != o.props.end();
   if( flags.wbd_interest_changed )
   {
      fc::raw::unpack_from_vector( itr->second, props.wbd_interest_rate );
   }
   ...
   itr = o.props.find( "wbd_exchange_rate" );
   flags.wbd_exchange_changed = itr != o.props.end();
   if( flags.wbd_exchange_changed )
   {
      fc::raw::unpack_from_vector( itr->second, wbd_exchange_rate );
      last_wbd_exchange_update = _db.head_block_time();
   }
   ...
   _db.modify( witness, [&]( witness_object& w )
   {
      ...
      if( flags.wbd_interest_changed )
      {
         w.props.wbd_interest_rate = props.wbd_interest_rate;
      }
      ...
      if( flags.wbd_exchange_changed )
      {
         w.wbd_exchange_rate = wbd_exchange_rate;
         w.last_wbd_exchange_update = last_wbd_exchange_update;
      }
   ...
   const dynamic_global_property_object& props, bool mined, const account_name_type& recovery_account, uint32_t hardfork )
{
   ...
   if( hardfork < WORTH_HARDFORK_0_20__2539 )
   {
      acc.voting_manabar.current_mana = WORTH_100_PERCENT;
   }

   if( hardfork >= WORTH_HARDFORK_0_11 )
   {
      FC_TODO( "If after HF 20, there are no temp account creations, the HF check can be removed." )
      if( ( hardfork < WORTH_HARDFORK_0_20__1782 ) || ( recovery_account != WORTH_TEMP_ACCOUNT ) )
      {
...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   {
      FC_TODO( "Move to validate() after HF20" );
      FC_ASSERT( o.fee <= asset( WORTH_MAX_ACCOUNT_CREATION_FEE, WORTH_SYMBOL ), "Account creation fee cannot be too large" );
   }

   if( _db.has_hardfork( WORTH_HARDFORK_0_20__1771 ) )
   {
      FC_ASSERT( o.fee == wso.median_props.account_creation_fee, "Must pay the exact account creation fee. paid: ${p} fee: ${f}",
                  ("p", o.fee)
                  ("f", wso.median_props.account_creation_fee) );
   }
   else if( !_db.has_hardfork( WORTH_HARDFORK_0_20__1761 ) && _db.has_hardfork( WORTH_HARDFORK_0_19__987 ) )
   {
      FC_ASSERT( o.fee >= asset( wso.median_props.account_creation_fee.amount * WORTH_CREATE_ACCOUNT_WITH_WORTH_MODIFIER, WORTH_SYMBOL ), "Insufficient Fee: ${f} required, ${p} provided.",
                 ("f", wso.median_props.account_creation_fee * asset( WORTH_CREATE_ACCOUNT_WITH_WORTH_MODIFIER, WORTH_SYMBOL ) )
                 ("p", o.fee) );
   }
   else if( _db.has_hardfork( WORTH_HARDFORK_0_1 ) )
   {
   ...
   if( _db.is_producing() || _db.has_hardfork( WORTH_HARDFORK_0_20 ) )
   {
   ...
   if( _db.has_hardfork( WORTH_HARDFORK_0_15__465 ) )
   {
   ...
   if( _db.has_hardfork( WORTH_HARDFORK_0_20__1762 ) )
   {
      _db.adjust_balance( _db.get< account_object, by_name >( WORTH_NULL_ACCOUNT ), o.fee );
   }
   ...
   if( !_db.has_hardfork( WORTH_HARDFORK_0_20__1762 ) && o.fee.amount > 0 )
   {
...
{
   FC_ASSERT( !_db.has_hardfork( WORTH_HARDFORK_0_20__1760 ), "Account creation with delegation is deprecated as of Hardfork 20" );
   ...
   auto target_delegation = asset( wso.median_props.account_creation_fee.amount * WORTH_CREATE_ACCOUNT_WITH_WORTH_MODIFIER * WORTH_CREATE_ACCOUNT_DELEGATION_RATIO, WORTH_SYMBOL ) * props.get_vesting_share_price();

   auto current_delegation = asset( o.fee.amount * WORTH_CREATE_ACCOUNT_DELEGATION_RATIO, WORTH_SYMBOL ) * props.get_vesting_share_price() + o.delegation;
   ...
   if( o.delegation.amount > 0 || !_db.has_hardfork( WORTH_HARDFORK_0_19__997 ) )
   {
      _db.create< vesting_delegation_object >( [&]( vesting_delegation_object& vdo )
      {
         ...
         vdo.min_delegation_time = _db.head_block_time() + WORTH_CREATE_ACCOUNT_DELEGATION_TIME;
      });
   }
...
{
   if( _db.has_hardfork( WORTH_HARDFORK_0_1 ) ) FC_ASSERT( o.account != WORTH_TEMP_ACCOUNT, "Cannot update temp account." );

   if( ( _db.has_hardfork( WORTH_HARDFORK_0_15__465 ) ) && o.posting )
      o.posting->validate();
   ...
   {
#ifndef IS_TEST_NET
      if( _db.has_hardfork( WORTH_HARDFORK_0_11 ) )
         FC_ASSERT( _db.head_block_time() - account_auth.last_owner_update > WORTH_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
#endif
   ...
   if( o.active && ( _db.has_hardfork( WORTH_HARDFORK_0_15__465 ) ) )
      verify_authority_accounts_exist( _db, *o.active, o.account, authority::active );
   if( o.posting && ( _db.has_hardfork( WORTH_HARDFORK_0_15__465 ) ) )
   ...
   if( o.json_metadata.size() > 0 )
   {
      ...
      {
         from_string( meta.json_metadata, o.json_metadata );
         if ( !_db.has_hardfork( WORTH_HARDFORK_0_21__3274 ) )
         {
...
{
   FC_ASSERT( _db.has_hardfork( WORTH_HARDFORK_0_21__3274 ), "Operation 'account_update2' is not enabled until HF 21" );
   FC_ASSERT( o.account != WORTH_TEMP_ACCOUNT, "Cannot update temp account." );
...
void delete_comment_evaluator::do_apply( const delete_comment_operation& o )
{
   ...
   if( _db.has_hardfork( WORTH_HARDFORK_0_19__876 ) )
      FC_ASSERT( comment.cashout_time != fc::time_point_sec::maximum() );

   if( _db.has_hardfork( WORTH_HARDFORK_0_19__977 ) )
   ...
   if( _db.has_hardfork( WORTH_HARDFORK_0_6__80 ) && comment.parent_author != WORTH_ROOT_POST_PARENT )
   {
...
void comment_options_evaluator::do_apply( const comment_options_operation& o )
{
   ...
   FC_ASSERT( comment.percent_worth_dollars >= o.percent_worth_dollars, "A comment cannot accept a greater percent WBD." );

   _db.modify( comment, [&]( comment_object& c ) {
       c.max_accepted_payout   = o.max_accepted_payout;
       c.percent_worth_dollars = o.percent_worth_dollars;
...
{ try {
   if( _db.has_hardfork( WORTH_HARDFORK_0_5__55 ) )
   ...
   if( o.parent_author != WORTH_ROOT_POST_PARENT )
   {
      parent = &_db.get_comment( o.parent_author, o.parent_permlink );
      if( !_db.has_hardfork( WORTH_HARDFORK_0_17__767 ) )
         FC_ASSERT( parent->depth < WORTH_MAX_COMMENT_DEPTH_PRE_HF17, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",WORTH_MAX_COMMENT_DEPTH_PRE_HF17) );
      else
         FC_ASSERT( parent->depth < WORTH_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",WORTH_MAX_COMMENT_DEPTH) );
   }
   ...
   if ( itr == by_permlink_idx.end() )
   {
      if( o.parent_author != WORTH_ROOT_POST_PARENT )
      {
         ...
         if( _db.has_hardfork( WORTH_HARDFORK_0_12__177 ) && !_db.has_hardfork( WORTH_HARDFORK_0_17__869 ) )
      ...
      if( _db.has_hardfork( WORTH_HARDFORK_0_20__2019 ) )
      {
         if( o.parent_author == WORTH_ROOT_POST_PARENT )
             FC_ASSERT( ( now - auth.last_root_post ) > WORTH_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
         else
             FC_ASSERT( (now - auth.last_post) >= WORTH_MIN_REPLY_INTERVAL_HF20, "You may only comment once every 3 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else if( _db.has_hardfork( WORTH_HARDFORK_0_12__176 ) )
      {
         if( o.parent_author == WORTH_ROOT_POST_PARENT )
             FC_ASSERT( ( now - auth.last_root_post ) > WORTH_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
         else
             FC_ASSERT( (now - auth.last_post) > WORTH_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else if( _db.has_hardfork( WORTH_HARDFORK_0_6__113 ) )
      {
         if( o.parent_author == WORTH_ROOT_POST_PARENT )
             FC_ASSERT( (now - auth.last_post) > WORTH_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("auth.last_post",auth.last_post) );
         else
             FC_ASSERT( (now - auth.last_post) > WORTH_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else
      ...
      uint16_t reward_weight = WORTH_100_PERCENT;
      uint64_t post_bandwidth = auth.post_bandwidth;

      if( _db.has_hardfork( WORTH_HARDFORK_0_12__176 ) && !_db.has_hardfork( WORTH_HARDFORK_0_17__733 ) && o.parent_author == WORTH_ROOT_POST_PARENT )
      {
         uint64_t post_delta_time = std::min( _db.head_block_time().sec_since_epoch() - auth.last_root_post.sec_since_epoch(), WORTH_POST_AVERAGE_WINDOW );
         uint32_t old_weight = uint32_t( ( post_bandwidth * ( WORTH_POST_AVERAGE_WINDOW - post_delta_time ) ) / WORTH_POST_AVERAGE_WINDOW );
         post_bandwidth = ( old_weight + WORTH_100_PERCENT );
         reward_weight = uint16_t( std::min( ( WORTH_POST_WEIGHT_CONSTANT * WORTH_100_PERCENT ) / ( post_bandwidth * post_bandwidth ), uint64_t( WORTH_100_PERCENT ) ) );
      }
      ...
      {
         if( _db.has_hardfork( WORTH_HARDFORK_0_1 ) )
         {
         ...
         if ( o.parent_author == WORTH_ROOT_POST_PARENT )
         {
            ...
            com.cashout_time = _db.has_hardfork( WORTH_HARDFORK_0_12__177 ) ?
               _db.head_block_time() + WORTH_CASHOUT_WINDOW_SECONDS_PRE_HF17 :
         ...
         if( _db.has_hardfork( WORTH_HARDFORK_0_17__769 ) )
         {
            com.cashout_time = com.created + WORTH_CASHOUT_WINDOW_SECONDS;
         }
      });
...
/// this loop can be skiped for validate-only nodes as it is merely gathering stats for indicies
   ...
   else // start edit case
   {
      ...
      if( _db.is_producing() || _db.has_hardfork( WORTH_HARDFORK_0_21__3313 ) )
      {
         FC_ASSERT( now - auth.last_post_edit >= WORTH_MIN_COMMENT_EDIT_INTERVAL, "Can only perform one comment edit per block." );
      }

      if( !_db.has_hardfork( WORTH_HARDFORK_0_17__772 ) )
      {
         if( _db.has_hardfork( WORTH_HARDFORK_0_14__306 ) )
            FC_ASSERT( _db.calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum(), "The comment is archived." );
         else if( _db.has_hardfork( WORTH_HARDFORK_0_10 ) )
      ...
      _db.modify( comment, [&]( comment_object& com )
      {
         ...
         if ( _db.has_hardfork( WORTH_HARDFORK_0_21__2203 ) )
...
void escrow_transfer_evaluator::do_apply( const escrow_transfer_operation& o )
{
   try
   {
      ...
      asset worth_spent = o.worth_amount;
      asset wbd_spent = o.wbd_amount;
      if( o.fee.symbol == WORTH_SYMBOL )
         worth_spent += o.fee;
      else
         wbd_spent += o.fee;

      _db.adjust_balance( from_account, -worth_spent );
      _db.adjust_balance( from_account, -wbd_spent );
      ...
      _db.create<escrow_object>([&]( escrow_object& esc )
      {
         ...
         esc.wbd_balance            = o.wbd_amount;
         esc.worth_balance          = o.worth_amount;
...
void escrow_approve_evaluator::do_apply( const escrow_approve_operation& o )
{
   try
   {
      ...
      if( reject_escrow )
      {
         _db.adjust_balance( o.from, escrow.worth_balance );
         _db.adjust_balance( o.from, escrow.wbd_balance );
...
void escrow_release_evaluator::do_apply( const escrow_release_operation& o )
{
   try
   {
      ...
      const auto& e = _db.get_escrow( o.from, o.escrow_id );
      FC_ASSERT( e.worth_balance >= o.worth_amount, "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}", ("a", o.worth_amount)("b", e.worth_balance) );
      FC_ASSERT( e.wbd_balance >= o.wbd_amount, "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}", ("a", o.wbd_amount)("b", e.wbd_balance) );
      FC_ASSERT( e.to == o.to, "Operation 'to' (${o}) does not match escrow 'to' (${e}).", ("o", o.to)("e", e.to) );
      FC_ASSERT( e.agent == o.agent, "Operation 'agent' (${a}) does not match escrow 'agent' (${e}).", ("o", o.agent)("e", e.agent) );
      ...
      _db.adjust_balance( o.receiver, o.worth_amount );
      _db.adjust_balance( o.receiver, o.wbd_amount );
      ...
      if( e.worth_balance.amount == 0 && e.wbd_balance.amount == 0 )
      {
         _db.remove( e );
      }
   }
...
void transfer_evaluator::do_apply( const transfer_operation& o )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( WORTH_HARDFORK_0_21__3343 ) )
   {
      FC_ASSERT( o.amount.symbol == WBD_SYMBOL || o.to != WORTH_TREASURY_ACCOUNT,
         "Can only transfer WBD to ${s}", ("s", WORTH_TREASURY_ACCOUNT) );
   }
...
void withdraw_vesting_evaluator::do_apply( const withdraw_vesting_operation& o )
{
   ...
   FC_ASSERT( account.vesting_shares >= asset( 0, VESTS_SYMBOL ), "Account does not have sufficient Worth Power for withdraw." );
   FC_ASSERT( account.vesting_shares - account.delegated_vesting_shares >= o.vesting_shares, "Account does not have sufficient Worth Power for withdraw." );

   FC_TODO( "Remove this entire block after HF 20" )
   if( !_db.has_hardfork( WORTH_HARDFORK_0_20__1860 ) && !account.mined && _db.has_hardfork( WORTH_HARDFORK_0_1 ) )
   {
      const auto& props = _db.get_dynamic_global_properties();
      const witness_schedule_object& wso = _db.get_witness_schedule_object();
      ...
      FC_ASSERT( account.vesting_shares > min_vests || ( _db.has_hardfork( WORTH_HARDFORK_0_16__562 ) && o.vesting_shares.amount == 0 ),
                 "Account registered by another account requires 10x account creation fee worth of Worth Power before it can be powered down." );
   }

   if( o.vesting_shares.amount == 0 )
   {
      if( _db.has_hardfork( WORTH_HARDFORK_0_5__57 ) )
   ...
   {
      int vesting_withdraw_intervals = WORTH_VESTING_WITHDRAW_INTERVALS_PRE_HF_16;
      if( _db.has_hardfork( WORTH_HARDFORK_0_16__551 ) )
         vesting_withdraw_intervals = WORTH_VESTING_WITHDRAW_INTERVALS; /// 13 weeks = 1 quarter of a year
      ...
      _db.modify( account, [&]( account_object& a )
      {
         ...
         if( _db.has_hardfork( WORTH_HARDFORK_0_21 ) && new_vesting_withdraw_rate.amount * vesting_withdraw_intervals < o.vesting_shares.amount )
         {
            new_vesting_withdraw_rate.amount += 1;
         }
         ...
         a.next_vesting_withdrawal = _db.head_block_time() + fc::seconds(WORTH_VESTING_WITHDRAW_INTERVAL_SECONDS);
...
void set_withdraw_vesting_route_evaluator::do_apply( const set_withdraw_vesting_route_operation& o )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( WORTH_HARDFORK_0_21__3343 ) )
   {
      FC_ASSERT( o.to_account != WORTH_TREASURY_ACCOUNT,
         "Cannot withdraw vesting to ${s}", ("s", WORTH_TREASURY_ACCOUNT) );
   }

   if( itr == wd_idx.end() )
   {
      FC_ASSERT( o.percent != 0, "Cannot create a 0% destination." );
      FC_ASSERT( from_account.withdraw_routes < WORTH_MAX_WITHDRAW_ROUTES, "Account already has the maximum number of routes." );
   ...
   FC_ASSERT( total_percent <= WORTH_100_PERCENT, "More than 100% of vesting withdrawals allocated to destinations." );
   }
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
   ...
   if( o.proxy.size() ) {
      ...
      proxy_chain.reserve( WORTH_MAX_PROXY_RECURSION_DEPTH + 1 );
      ...
      while( cprox->proxy.size() != 0 ) {
         ...
         cprox = &next_proxy;
         FC_ASSERT( proxy_chain.size() <= WORTH_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long." );
      }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      ...
      if ( _db.has_hardfork( WORTH_HARDFORK_0_2 ) )
      {
         FC_ASSERT( voter.witnesses_voted_for < WORTH_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." ); // TODO: Remove after hardfork 2
         ...
         if( _db.has_hardfork( WORTH_HARDFORK_0_3 ) ) {
...
void pre_hf20_vote_evaluator( const vote_operation& o, database& _db )
{
   ...
   if( _db.has_hardfork( WORTH_HARDFORK_0_12__177 ) && _db.calculate_discussion_payout_time( comment ) == fc::time_point_sec::maximum() )
   {
#ifndef CLEAR_VOTES
   ...
   if( _db.has_hardfork( WORTH_HARDFORK_0_11 ) )
      FC_ASSERT( elapsed_seconds >= WORTH_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

   int64_t regenerated_power = (WORTH_100_PERCENT * elapsed_seconds) / WORTH_VOTING_MANA_REGENERATION_SECONDS;
   int64_t current_power     = std::min( int64_t(voter.voting_manabar.current_mana) + regenerated_power, int64_t(WORTH_100_PERCENT) );
   ...
   int64_t  used_power  = ((current_power * abs_weight) / WORTH_100_PERCENT) * (60*60*24);

   const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();

   // The second multiplication is rounded up as of HF 259
   int64_t max_vote_denom = dgpo.vote_power_reserve_rate * WORTH_VOTING_MANA_REGENERATION_SECONDS;
   FC_ASSERT( max_vote_denom > 0 );

   if( !_db.has_hardfork( WORTH_HARDFORK_0_14__259 ) )
   {
   ...
   int64_t abs_rshares    = ((uint128_t( _db.get_effective_vesting_shares( voter, VESTS_SYMBOL ).amount.value ) * used_power) / (WORTH_100_PERCENT)).to_uint64();
   if( !_db.has_hardfork( WORTH_HARDFORK_0_14__259 ) && abs_rshares == 0 ) abs_rshares = 1;

   if( _db.has_hardfork( WORTH_HARDFORK_0_20__1764 ) )
   {
      abs_rshares -= WORTH_VOTE_DUST_THRESHOLD;
      abs_rshares = std::max( int64_t(0), abs_rshares );
   }
   else if( _db.has_hardfork( WORTH_HARDFORK_0_14__259 ) )
   {
      FC_ASSERT( abs_rshares > WORTH_VOTE_DUST_THRESHOLD || o.weight == 0, "Voting weight is too small, please accumulate more voting power or worth power." );
   }
   else if( _db.has_hardfork( WORTH_HARDFORK_0_13__248 ) )
   {
      FC_ASSERT( abs_rshares > WORTH_VOTE_DUST_THRESHOLD || abs_rshares == 1, "Voting weight is too small, please accumulate more voting power or worth power." );
   }
   ...
   {
      if( _db.has_hardfork( WORTH_HARDFORK_0_12__177 ) )
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      if( rshares > 0 )
      {
         if( _db.has_hardfork( WORTH_HARDFORK_0_17__900 ) )
            FC_ASSERT( _db.head_block_time() < comment.cashout_time - WORTH_UPVOTE_LOCKOUT_HF17, "Cannot increase payout within last twelve hours before payout." );
         else if( _db.has_hardfork( WORTH_HARDFORK_0_7 ) )
            FC_ASSERT( _db.head_block_time() < _db.calculate_discussion_payout_time( comment ) - WORTH_UPVOTE_LOCKOUT_HF7, "Cannot increase payout within last minute before payout." );
      }
      ...
      if( !_db.has_hardfork( WORTH_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( WORTH_HARDFORK_0_12__177 ) && !_db.has_hardfork( WORTH_HARDFORK_0_13__257)  )
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + WORTH_CASHOUT_WINDOW_SECONDS_PRE_HF17;
         else
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + WORTH_CASHOUT_WINDOW_SECONDS_PRE_HF12;
      ...
      _db.modify( comment, [&]( comment_object& c ){
         ...
         if( rshares > 0 )
            ...
            c.net_votes--;
         if( !_db.has_hardfork( WORTH_HARDFORK_0_6__114 ) && c.net_rshares == -c.abs_rshares) FC_ASSERT( c.net_votes < 0, "Comment has negative net votes?" );
      });
      ...
      _db.modify( root, [&]( comment_object& c )
      {
         ...
         if( !_db.has_hardfork( WORTH_HARDFORK_0_17__769 ) )
         {
            if( _db.has_hardfork( WORTH_HARDFORK_0_12__177 ) && c.last_payout > fc::time_point_sec::min() )
            ...
            if( c.max_cashout_time == fc::time_point_sec::maximum() )
               c.max_cashout_time = _db.head_block_time() + fc::seconds( WORTH_MAX_CASHOUT_WINDOW_SECONDS );
         }
      });
      ...
      **/
      _db.create<comment_vote_object>( [&]( comment_vote_object& cv ){
         ...
         if( curation_reward_eligible && _db.has_hardfork( WORTH_HARDFORK_0_17__774 ) )
            curation_reward_eligible = _db.get_curation_rewards_percent( comment ) > 0;

         if( curation_reward_eligible )
         {
            if( comment.created < fc::time_point_sec(WORTH_HARDFORK_0_6_REVERSE_AUCTION_TIME) ) {
               ...
               if( _db.has_hardfork( WORTH_HARDFORK_0_17__774 ) )
               {
                  const auto& reward_fund = _db.get_reward_fund( comment );
                  auto curve = !_db.has_hardfork( WORTH_HARDFORK_0_19__1052 ) && comment.created > WORTH_HF_19_SQRT_PRE_CALC
            ...
            if( _db.head_block_time() > fc::time_point_sec(WORTH_HARDFORK_0_6_REVERSE_AUCTION_TIME) )  /// start enforcing this prior to the hardfork
            {
   ...
   {
      FC_ASSERT( itr->num_changes < WORTH_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );

      if( _db.has_hardfork( WORTH_HARDFORK_0_6__112 ) )
      ...
      if( !_db.has_hardfork( WORTH_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( WORTH_HARDFORK_0_14__259 ) && abs_rshares == 0 )
...
void hf20_vote_evaluator( const vote_operation& o, database& _db )
{
   ...
   auto now = _db.head_block_time();
   FC_ASSERT( ( now - voter.last_vote_time ).to_seconds() >= WORTH_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

   _db.modify( voter, [&]( account_object& a )
   {
      util::update_manabar( _db.get_dynamic_global_properties(), a, _db.has_hardfork( WORTH_HARDFORK_0_21__3336 ) );
   });

   if ( _db.has_hardfork( WORTH_HARDFORK_0_21__3004 ) )
   {
   ...
   if( _db.has_hardfork( WORTH_HARDFORK_0_21__3336 ) && dgpo.downvote_pool_percent && o.weight < 0 )
   {
      used_mana = ( std::max( ( uint128_t( voter.downvote_manabar.current_mana * WORTH_100_PERCENT ) / dgpo.downvote_pool_percent ),
                                uint128_t( voter.voting_manabar.current_mana ) )
                  * abs_weight * 60 * 60 * 24 ) / WORTH_100_PERCENT;
   }
   else
   {
      used_mana = ( uint128_t( voter.voting_manabar.current_mana ) * abs_weight * 60 * 60 * 24 ) / WORTH_100_PERCENT;
   }
   ...
   if( cashout_delta < WORTH_UPVOTE_LOCKOUT_SECONDS )
   {
      abs_rshares = (int64_t) ( ( uint128_t( abs_rshares ) * cashout_delta ) / WORTH_UPVOTE_LOCKOUT_SECONDS ).to_uint64();
   }
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      _db.modify( voter, [&]( account_object& a )
      {
         if( _db.has_hardfork( WORTH_HARDFORK_0_21__3336 ) && dgpo.downvote_pool_percent > 0 && o.weight < 0 )
         {
...
{ try {
   if( _db.has_hardfork( WORTH_HARDFORK_0_20__2539 ) )
   {
...
void custom_evaluator::do_apply( const custom_operation& o )
{
   database& d = db();
   if( d.is_producing() )
      FC_ASSERT( o.data.size() <= WORTH_CUSTOM_OP_DATA_MAX_LENGTH,
         "Operation data must be less than ${bytes} bytes.", ("bytes", WORTH_CUSTOM_OP_DATA_MAX_LENGTH) );

   if( _db.is_producing() || _db.has_hardfork( WORTH_HARDFORK_0_20 ) )
   {
      FC_ASSERT( o.required_auths.size() <= WORTH_MAX_AUTHORITY_MEMBERSHIP,
         "Authority membership exceeded. Max: ${max} Current: ${n}", ("max", WORTH_MAX_AUTHORITY_MEMBERSHIP)("n", o.required_auths.size()) );
   }
}
...
void custom_json_evaluator::do_apply( const custom_json_operation& o )
{
   ...
   if( d.is_producing() )
      FC_ASSERT( o.json.length() <= WORTH_CUSTOM_OP_DATA_MAX_LENGTH,
         "Operation JSON must be less than ${bytes} bytes.", ("bytes", WORTH_CUSTOM_OP_DATA_MAX_LENGTH) );

   if( _db.is_producing() || _db.has_hardfork( WORTH_HARDFORK_0_20 ) )
   {
      size_t num_auths = o.required_auths.size() + o.required_posting_auths.size();
      FC_ASSERT( num_auths <= WORTH_MAX_AUTHORITY_MEMBERSHIP,
         "Authority membership exceeded. Max: ${max} Current: ${n}", ("max", WORTH_MAX_AUTHORITY_MEMBERSHIP)("n", num_auths) );
   }
...
void custom_binary_evaluator::do_apply( const custom_binary_operation& o )
{
   ...
   FC_ASSERT( d.has_hardfork( WORTH_HARDFORK_0_14__317 ) );
...
void pow_apply( database& db, Operation o )
{
   ...
   if( db.has_hardfork( WORTH_HARDFORK_0_5__59 ) )
   {
   ...
   if( db.has_hardfork( WORTH_HARDFORK_0_13__256 ) )
   ...
   if( db.head_block_num() < WORTH_START_MINER_VOTING_BLOCK )
      pow_reward.amount *= WORTH_MAX_WITNESSES;
...
void pow_evaluator::do_apply( const pow_operation& o ) {
   FC_ASSERT( !db().has_hardfork( WORTH_HARDFORK_0_13__256 ), "pow is deprecated. Use pow2 instead" );
   pow_apply( db(), o );
}
...
{
   database& db = this->db();
   FC_ASSERT( !db.has_hardfork( WORTH_HARDFORK_0_17__770 ), "mining is now disabled" );

   const auto& dgp = db.get_dynamic_global_properties();
   uint32_t target_pow = db.get_pow_summary_target();
   account_name_type worker_account;

   if( db.has_hardfork( WORTH_HARDFORK_0_16__551 ) )
   {
   ...
   FC_ASSERT( o.props.maximum_block_size >= WORTH_MIN_BLOCK_SIZE_LIMIT * 2, "Voted maximum block size is too small." );
...
{
   if( _db.has_hardfork( WORTH_HARDFORK_0_20__409 ) )
      FC_ASSERT( is_asset_type( o.exchange_rate.base, WBD_SYMBOL ) && is_asset_type( o.exchange_rate.quote, WORTH_SYMBOL ),
            "Price feed must be a WBD/WORTH price" );

   const auto& witness = _db.get_witness( o.publisher );
   _db.modify( witness, [&]( witness_object& w )
   {
      w.wbd_exchange_rate = o.exchange_rate;
      w.last_wbd_exchange_update = _db.head_block_time();
   });
}
...
void convert_evaluator::do_apply( const convert_operation& o )
{
  ...
  FC_ASSERT( !fhistory.current_median_history.is_null(), "Cannot convert WBD because there is no price feed." );

  auto worth_conversion_delay = WORTH_CONVERSION_DELAY_PRE_HF_16;
  if( _db.has_hardfork( WORTH_HARDFORK_0_16__551) )
     worth_conversion_delay = WORTH_CONVERSION_DELAY;
  ...
  _db.create<convert_request_object>( [&]( convert_request_object& obj )
  {
      ...
      obj.amount          = o.amount;
      obj.conversion_date = _db.head_block_time() + worth_conversion_delay;
  });
...
void limit_order_create_evaluator::do_apply( const limit_order_create_operation& o )
{
   ...
   if( _db.has_hardfork( WORTH_HARDFORK_0_20__1449) )
   {
      FC_ASSERT( o.expiration <= _db.head_block_time() + WORTH_MAX_LIMIT_ORDER_EXPIRATION, "Limit Order Expiration must not be more than 28 days in the future" );
   }
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       else
       {
          ...
          obj.expiration = std::min( o.expiration, fc::time_point_sec( WORTH_HARDFORK_0_20_TIME + WORTH_MAX_LIMIT_ORDER_EXPIRATION + rand_offset ) );
       }
   });
...
void limit_order_create2_evaluator::do_apply( const limit_order_create2_operation& o )
{
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       {
          obj.expiration = std::min( o.expiration, fc::time_point_sec( WORTH_HARDFORK_0_20_TIME + WORTH_MAX_LIMIT_ORDER_EXPIRATION ) );
       }
   });
...
{
   FC_ASSERT( !_db.has_hardfork( WORTH_HARDFORK_0_4 ), "report_over_production_operation is disabled." );
}

void claim_account_evaluator::do_apply( const claim_account_operation& o )
{
   FC_ASSERT( _db.has_hardfork( WORTH_HARDFORK_0_20__1771 ), "claim_account_operation is not enabled until hardfork 20." );
   ...
   if( o.fee.amount == 0 )
   {
      ...
      if( !_db.is_pending_tx() )
      {
         ...
         FC_ASSERT( current_witness.available_witness_account_subsidies >= WORTH_ACCOUNT_SUBSIDY_PRECISION, "Witness ${w} does not have enough subsidized accounts to claim",
            ("w", current_witness.owner) );

         _db.modify( current_witness, [&]( witness_object& w )
         {
            w.available_witness_account_subsidies -= WORTH_ACCOUNT_SUBSIDY_PRECISION;
         });
      }

      FC_ASSERT( gpo.available_account_subsidies >= WORTH_ACCOUNT_SUBSIDY_PRECISION, "There are not enough subsidized accounts to claim" );

      _db.modify( gpo, [&]( dynamic_global_property_object& gpo )
      {
         gpo.available_account_subsidies -= WORTH_ACCOUNT_SUBSIDY_PRECISION;
      });
   }
   else
   {
   ...
   _db.adjust_balance( _db.get_account( WORTH_NULL_ACCOUNT ), o.fee );
...
{
   FC_ASSERT( _db.has_hardfork( WORTH_HARDFORK_0_20__1771 ), "create_claimed_account_operation is not enabled until hardfork 20." );
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
   ...
   if ( account_to_recover.recovery_account.length() )   // Make sure recovery matches expected recovery account
   {
      ...
      if( o.recovery_account == WORTH_TEMP_ACCOUNT )
   ...
   if( request == recovery_request_idx.end() ) // New Request
   {
      ...
      _db.create< account_recovery_request_object >( [&]( account_recovery_request_object& req )
      {
         ...
         req.expires = _db.head_block_time() + WORTH_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
      });
   }
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
   ...
   if( _db.has_hardfork( WORTH_HARDFORK_0_12 ) )
      FC_ASSERT( _db.head_block_time() - account.last_account_recovery > WORTH_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
   ...
   if( request == change_recovery_idx.end() ) // New request
   {
      _db.create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
      {
         ...
         req.effective_on = _db.head_block_time() + WORTH_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
...
void transfer_to_savings_evaluator::do_apply( const transfer_to_savings_operation& op )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( WORTH_HARDFORK_0_21__3343 ) )
   {
      FC_ASSERT( op.to != WORTH_TREASURY_ACCOUNT,
         "Cannot transfer savings to ${s}", ("s", WORTH_TREASURY_ACCOUNT) );
   }
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
   ...
   FC_ASSERT( from.savings_withdraw_requests < WORTH_SAVINGS_WITHDRAW_REQUEST_LIMIT, "Account has reached limit for pending withdraw requests." );

   FC_TODO( "Remove is producing after HF 21" );
   if( _db.is_producing() || _db.has_hardfork( WORTH_HARDFORK_0_21__3343 ) )
   {
      FC_ASSERT( op.amount.symbol == WBD_SYMBOL || op.to != WORTH_TREASURY_ACCOUNT,
         "Can only transfer WBD to ${s}", ("s", WORTH_TREASURY_ACCOUNT) );
   }
   ...
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      ...
      s.complete = _db.head_block_time() + WORTH_SAVINGS_WITHDRAW_TIME;
   });
...
{
   FC_ASSERT( _db.has_hardfork( WORTH_HARDFORK_0_14__324 ) );
   ...
   if( o.decline )
   {
      ...
      _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
      {
         ...
         req.effective_date = _db.head_block_time() + WORTH_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
   else
   {
...
void claim_reward_balance_evaluator::do_apply( const claim_reward_balance_operation& op )
{
   ...
   FC_ASSERT( op.reward_worth <= acnt.reward_worth_balance, "Cannot claim that much WORTH. Claim: ${c} Actual: ${a}",
      ("c", op.reward_worth)("a", acnt.reward_worth_balance) );
   FC_ASSERT( op.reward_wbd <= acnt.reward_wbd_balance, "Cannot claim that much WBD. Claim: ${c} Actual: ${a}",
      ("c", op.reward_wbd)("a", acnt.reward_wbd_balance) );
   ...
   asset reward_vesting_worth_to_move = asset( 0, WORTH_SYMBOL );
   if( op.reward_vests == acnt.reward_vesting_balance )
      reward_vesting_worth_to_move = acnt.reward_vesting_worth;
   else
      reward_vesting_worth_to_move = asset( ( ( uint128_t( op.reward_vests.amount.value ) * uint128_t( acnt.reward_vesting_worth.amount.value ) )
         / uint128_t( acnt.reward_vesting_balance.amount.value ) ).to_uint64(), WORTH_SYMBOL );

   _db.adjust_reward_balance( acnt, -op.reward_worth );
   _db.adjust_reward_balance( acnt, -op.reward_wbd );
   _db.adjust_balance( acnt, op.reward_worth );
   _db.adjust_balance( acnt, op.reward_wbd );

   _db.modify( acnt, [&]( account_object& a )
   {
      if( _db.has_hardfork( WORTH_HARDFORK_0_20__2539 ) )
      {
         util::update_manabar( _db.get_dynamic_global_properties(), a, _db.has_hardfork( WORTH_HARDFORK_0_21__3336 ), op.reward_vests.amount.value );
      }

      a.vesting_shares += op.reward_vests;
      a.reward_vesting_balance -= op.reward_vests;
      a.reward_vesting_worth -= reward_vesting_worth_to_move;
   });

   _db.modify( _db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
   {
      gpo.total_vesting_shares += op.reward_vests;
      gpo.total_vesting_fund_worth += reward_vesting_worth_to_move;

      gpo.pending_rewarded_vesting_shares -= op.reward_vests;
      gpo.pending_rewarded_vesting_worth -= reward_vesting_worth_to_move;
   });
...
void claim_reward_balance2_evaluator::do_apply( const claim_reward_balance2_operation& op )
{
   ...
   for( const asset& token : op.reward_tokens )
   {
      ...
      if( token.symbol.space() == asset_symbol_type::wmt_nai_space )
      {
      ...
      else
      {
         ...
         if( token.symbol == VESTS_SYMBOL)
         {
            ...
            if( token == a->reward_vesting_balance )
               reward_vesting_worth_to_move = a->reward_vesting_worth;
            else
               reward_vesting_worth_to_move = asset( ( ( uint128_t( token.amount.value ) * uint128_t( a->reward_vesting_worth.amount.value ) )
                  / uint128_t( a->reward_vesting_balance.amount.value ) ).to_uint64(), WORTH_SYMBOL );
         ...
         else if( token.symbol == WORTH_SYMBOL || token.symbol == WBD_SYMBOL )
         {
            FC_ASSERT( is_asset_type( token, WORTH_SYMBOL ) == false || token <= a->reward_worth_balance,
                       "Cannot claim that much WORTH. Claim: ${c} Actual: ${a}", ("c", token)("a", a->reward_worth_balance) );
            FC_ASSERT( is_asset_type( token, WBD_SYMBOL ) == false || token <= a->reward_wbd_balance,
                       "Cannot claim that much WBD. Claim: ${c} Actual: ${a}", ("c", token)("a", a->reward_wbd_balance) );
...
{
#pragma message( "TODO: Update get_effective_vesting_shares when modifying this operation to support WMTs." )
   ...
   if( _db.has_hardfork( WORTH_HARDFORK_0_20__2539 ) )
   {
      ...
      {
         util::update_manabar( gpo, a, _db.has_hardfork( WORTH_HARDFORK_0_21__3336 ) );
      });
      ...
      if( gpo.downvote_pool_percent )
      {
         available_downvote_shares = asset(
            ( delegator.downvote_manabar.current_mana * WORTH_100_PERCENT ) / gpo.downvote_pool_percent
            + ( WORTH_100_PERCENT / gpo.downvote_pool_percent ) - 1, VESTS_SYMBOL );
      }
      else
      {
   ...
   auto min_delegation = _db.has_hardfork( WORTH_HARDFORK_0_20__1761 ) ?
      asset( wso.median_props.account_creation_fee.amount / 3, WORTH_SYMBOL ) * gpo.get_vesting_share_price() :
      asset( wso.median_props.account_creation_fee.amount * 10, WORTH_SYMBOL ) * gpo.get_vesting_share_price();
   auto min_update = _db.has_hardfork( WORTH_HARDFORK_0_20__1761 ) ?
      asset( wso.median_props.account_creation_fee.amount / 30, WORTH_SYMBOL ) * gpo.get_vesting_share_price() :
   ...
   if( delegation == nullptr )
   {
      ...
      FC_TODO( "This hardfork check should not be needed. Remove after HF21 if that is the case." );
      if( _db.has_hardfork( WORTH_HARDFORK_0_21__3336 ) )
      ...
      _db.modify( delegatee, [&]( account_object& a )
      {
         if( _db.has_hardfork( WORTH_HARDFORK_0_20__2539 ) )
         {
            util::update_manabar( gpo, a, _db.has_hardfork( WORTH_HARDFORK_0_21__3336 ), op.vesting_shares.amount.value );
         }
   ...
   else if( op.vesting_shares >= delegation->vesting_shares )
   {
      ...
      FC_ASSERT( delta >= min_update, "Worth Power increase is not enough of a difference. min_update: ${min}", ("min", min_update) );
      ...
      _db.modify( delegatee, [&]( account_object& a )
      {
         if( _db.has_hardfork( WORTH_HARDFORK_0_20__2539 ) )
         {
            util::update_manabar( gpo, a, _db.has_hardfork( WORTH_HARDFORK_0_21__3336 ), delta.amount.value );
         }
   ...
   else /* delegation->vesting_shares > op.vesting_shares */
   {
      ...
      {
         FC_ASSERT( delta >= min_update, "Worth Power decrease is not enough of a difference. min_update: ${min}", ("min", min_update) );
