 ...
 */
#pragma once
#include <worth/chain/block_log.hpp>
#include <worth/chain/fork_database.hpp>
#include <worth/chain/global_property_object.hpp>
#include <worth/chain/hardfork_property_object.hpp>
#include <worth/chain/node_property_object.hpp>
#include <worth/chain/notifications.hpp>

#include <worth/chain/util/advanced_benchmark_dumper.hpp>
#include <worth/chain/util/signal.hpp>

#include <worth/protocol/protocol.hpp>
...
namespace worth { namespace chain {
   ...
   {
      fc::time_point_sec         times[ WORTH_NUM_HARDFORKS + 1 ];
      protocol::hardfork_version versions[ WORTH_NUM_HARDFORKS + 1 ];
   };
   ...
   class database : public chainbase::database
   {
      public:
         ...
         struct open_args
         {
            ...
            uint64_t initial_supply = WORTH_INIT_SUPPLY;
            uint64_t wbd_initial_supply = WORTH_WBD_INIT_SUPPLY;
         ...
         chain_id_type worth_chain_id = WORTH_CHAIN_ID;
      ...
      public:
          ...
          *
          * Passing slot_num == 0 returns WORTH_NULL_WITNESS
          */
         ...
         /** @return the wbd created and deposited to_account, may return WORTH if there is no median feed */
         std::pair< asset, asset > create_wbd( const account_object& to_account, asset worth, bool to_reward_balance=false );
         asset create_vesting( const account_object& to_account, asset worth, bool to_reward_balance=false );
         void adjust_total_payout( const comment_object& a, const asset& wbd, const asset& curator_wbd_value, const asset& beneficiary_value );
         ...
         /**
          * Helper method to return the current wbd value of a given amount of
          * WORTH.  Return 0 WBD if there isn't a current_median_history
          */
         asset to_wbd( const asset& worth )const;
         asset to_worth( const asset& wbd )const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = WORTH_INIT_SUPPLY, uint64_t wbd_initial_supply = WORTH_WBD_INIT_SUPPLY );
         ...
         ///@{
         void validate_wmt_invariants()const;
         ///@}
#endif
   ...
   protected:
      ...
      private:
         ...
         void adjust_wmt_balance( const account_name_type& name, const asset& delta, bool check_account,
      ...
      public:
         ...
         int16_t get_wps_remove_threshold() const
         {
            return _wps_remove_threshold;
         }

         void set_wps_remove_threshold( int16_t val )
         {
            _wps_remove_threshold = val;
         }
      ...
      private:
         ...
         int16_t                       _wps_remove_threshold = -1;
