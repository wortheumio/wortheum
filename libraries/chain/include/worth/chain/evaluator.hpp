...
template< typename OperationType=worth::protocol::operation >
class evaluator
{
   public:
      virtual ~evaluator() {}
...
template< typename EvaluatorType, typename OperationType=worth::protocol::operation >
...
#define WORTH_DEFINE_EVALUATOR( X ) \
class X ## _evaluator : public worth::chain::evaluator_impl< X ## _evaluator > \
{                                                                           \
   public:                                                                  \
      typedef X ## _operation operation_type;                               \
                                                                            \
      X ## _evaluator( database& db )                                       \
         : worth::chain::evaluator_impl< X ## _evaluator >( db )          \
      {}                                                                    \
                                                                            \
      void do_apply( const X ## _operation& o );                            \
};

#define WORTH_DEFINE_ACTION_EVALUATOR( X, ACTION )                               \
class X ## _evaluator : public worth::chain::evaluator_impl< X ## _evaluator, ACTION > \
{                                                                                \
   public:                                                                       \
      typedef X ## _action operation_type;                                       \
                                                                                 \
      X ## _evaluator( database& db )                                            \
         : worth::chain::evaluator_impl< X ## _evaluator, ACTION >( db )        \
      {}                                                                         \
                                                                                 \
      void do_apply( const X ## _action& o );                                    \
};

#define WORTH_DEFINE_PLUGIN_EVALUATOR( PLUGIN, OPERATION, X )               \
class X ## _evaluator : public worth::chain::evaluator_impl< X ## _evaluator, OPERATION > \
{                                                                           \
   public:                                                                  \
      typedef X ## _operation operation_type;                               \
                                                                            \
      X ## _evaluator( worth::chain::database& db, PLUGIN* plugin )       \
         : worth::chain::evaluator_impl< X ## _evaluator, OPERATION >( db ), \
           _plugin( plugin )                                                \
      {}                                                                    \
                                                                            \
      void do_apply( const X ## _operation& o );                            \
