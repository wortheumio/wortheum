...
void update_manabar( const PropType& gpo, AccountType& account, bool downvote_mana = false, int64_t new_mana = 0 )
{
   ...
   manabar_params params( effective_vests, WORTH_VOTING_MANA_REGENERATION_SECONDS );
   ...
   if( downvote_mana )
   {
      util::manabar_params params( ( effective_vests * gpo.downvote_pool_percent ) / WORTH_100_PERCENT, WORTH_VOTING_MANA_REGENERATION_SECONDS );
      account.downvote_manabar.regenerate_mana( params, gpo.time );
      account.downvote_manabar.use_mana( ( -new_mana * gpo.downvote_pool_percent ) / WORTH_100_PERCENT );
   }
}

} } } // worth::chain::util

FC_REFLECT( worth::chain::util::manabar,
   (current_mana)
   (last_update_time)
   )
