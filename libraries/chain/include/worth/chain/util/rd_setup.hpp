...
// but we can't keep the WORTH_RD_* constants in this header because they're used for validation and
// need to be available to worth::protocol.
...
struct rd_system_params
{
   ...
   uint32_t        decay_per_time_unit_denom_shift = WORTH_RD_DECAY_DENOM_SHIFT;
};
...
FC_REFLECT( worth::chain::util::rd_user_params,
   (budget_per_time_unit)
   (decay_per_time_unit)
   )

FC_REFLECT( worth::chain::util::rd_system_params,
   (resource_unit)
