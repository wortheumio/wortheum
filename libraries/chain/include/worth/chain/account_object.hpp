...
#include <worth/chain/shared_authority.hpp>
...
namespace worth { namespace chain {
   ...
   using worth::protocol::authority;

   class account_object : public object< account_object_type, account_object >
   {
         WORTH_STD_ALLOCATOR_CONSTRUCTOR( account_object )
         ...
         account_name_type reset_account = WORTH_NULL_ACCOUNT;
         ...
         asset             balance = asset( 0, WORTH_SYMBOL );  ///< total liquid shares held by this account
         asset             savings_balance = asset( 0, WORTH_SYMBOL );  ///< total liquid shares held by this account

         /**
          *  WBD Deposits pay interest based upon the interest rate set by witnesses. The purpose of these
          *  fields is to track the total (time * wbd_balance) that it is held. Then at the appointed time
          *  interest can be paid using the following equation:
          *
          *  interest = interest_rate * wbd_seconds / seconds_per_year
          *
          *  Every time the wbd_balance is updated the wbd_seconds is also updated. If at least
          *  WORTH_MIN_COMPOUNDING_INTERVAL_SECONDS has past since wbd_last_interest_payment then
          *  interest is added to wbd_balance.
          *
          *  @defgroup wbd_data wbd Balance Data
          */
         ///@{
         asset             wbd_balance = asset( 0, WBD_SYMBOL ); /// total wbd balance
         uint128_t         wbd_seconds; ///< total wbd * how long it has been hel
         time_point_sec    wbd_seconds_last_update; ///< the last time the wbd_seconds was updated
         time_point_sec    wbd_last_interest_payment; ///< used to pay interest at most once per month


         asset             savings_wbd_balance = asset( 0, WBD_SYMBOL ); /// total wbd balance
         uint128_t         savings_wbd_seconds; ///< total wbd * how long it has been hel
         time_point_sec    savings_wbd_seconds_last_update; ///< the last time the wbd_seconds was updated
         time_point_sec    savings_wbd_last_interest_payment; ///< used to pay interest at most once per month

         uint8_t           savings_withdraw_requests = 0;
         ///@}

         asset             reward_wbd_balance = asset( 0, WBD_SYMBOL );
         asset             reward_worth_balance = asset( 0, WORTH_SYMBOL );
         asset             reward_vesting_balance = asset( 0, VESTS_SYMBOL );
         asset             reward_vesting_worth = asset( 0, WORTH_SYMBOL );
         ...
         fc::array<share_type, WORTH_MAX_PROXY_RECURSION_DEPTH> proxied_vsf_votes;// = std::vector<share_type>( WORTH_MAX_PROXY_RECURSION_DEPTH, 0 ); ///< the total VFS votes proxied to this account
   ...
   {
      WORTH_STD_ALLOCATOR_CONSTRUCTOR( account_metadata_object )
   ...
   {
      WORTH_STD_ALLOCATOR_CONSTRUCTOR( account_authority_object )
   ...
   {
      WORTH_STD_ALLOCATOR_CONSTRUCTOR( owner_authority_history_object )
   ...
   {
      WORTH_STD_ALLOCATOR_CONSTRUCTOR( account_recovery_request_object )
   ...
   {
      WORTH_STD_ALLOCATOR_CONSTRUCTOR( change_recovery_account_request_object )
...
template<> struct is_static_length< worth::chain::account_object > : public boost::true_type {};
template<> struct is_static_length< worth::chain::vesting_delegation_object > : public boost::true_type {};
template<> struct is_static_length< worth::chain::vesting_delegation_expiration_object > : public boost::true_type {};
template<> struct is_static_length< worth::chain::change_recovery_account_request_object > : public boost::true_type {};

} // mira
#endif

FC_REFLECT( worth::chain::account_object,
             ...
             (reward_worth_balance)(reward_wbd_balance)(reward_vesting_balance)(reward_vesting_worth)
...
CHAINBASE_SET_INDEX_TYPE( worth::chain::account_object, worth::chain::account_index )

FC_REFLECT( worth::chain::account_metadata_object,
             (id)(account)(json_metadata)(posting_json_metadata) )
CHAINBASE_SET_INDEX_TYPE( worth::chain::account_metadata_object, worth::chain::account_metadata_index )

FC_REFLECT( worth::chain::account_authority_object,
             (id)(account)(owner)(active)(posting)(last_owner_update)
)
CHAINBASE_SET_INDEX_TYPE( worth::chain::account_authority_object, worth::chain::account_authority_index )

FC_REFLECT( worth::chain::vesting_delegation_object,
            (id)(delegator)(delegatee)(vesting_shares)(min_delegation_time) )
CHAINBASE_SET_INDEX_TYPE( worth::chain::vesting_delegation_object, worth::chain::vesting_delegation_index )

FC_REFLECT( worth::chain::vesting_delegation_expiration_object,
            (id)(delegator)(vesting_shares)(expiration) )
CHAINBASE_SET_INDEX_TYPE( worth::chain::vesting_delegation_expiration_object, worth::chain::vesting_delegation_expiration_index )

FC_REFLECT( worth::chain::owner_authority_history_object,
             (id)(account)(previous_owner_authority)(last_valid_time)
          )
CHAINBASE_SET_INDEX_TYPE( worth::chain::owner_authority_history_object, worth::chain::owner_authority_history_index )

FC_REFLECT( worth::chain::account_recovery_request_object,
             (id)(account_to_recover)(new_owner_authority)(expires)
          )
CHAINBASE_SET_INDEX_TYPE( worth::chain::account_recovery_request_object, worth::chain::account_recovery_request_index )

FC_REFLECT( worth::chain::change_recovery_account_request_object,
             (id)(account_to_recover)(recovery_account)(effective_on)
          )
CHAINBASE_SET_INDEX_TYPE( worth::chain::change_recovery_account_request_object, worth::chain::change_recovery_account_request_index )
