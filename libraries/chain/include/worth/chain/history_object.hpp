...
namespace worth { namespace chain {
   ...
   {
      WORTH_STD_ALLOCATOR_CONSTRUCTOR( operation_object )
...
template<> struct is_static_length< worth::chain::account_history_object > : public boost::true_type {};

} // mira
#endif

FC_REFLECT( worth::chain::operation_object, (id)(trx_id)(block)(trx_in_block)(op_in_trx)(virtual_op)(timestamp)(serialized_op) )
CHAINBASE_SET_INDEX_TYPE( worth::chain::operation_object, worth::chain::operation_index )

FC_REFLECT( worth::chain::account_history_object, (id)(account)(sequence)(op) )

CHAINBASE_SET_INDEX_TYPE( worth::chain::account_history_object, worth::chain::account_history_index )

namespace helpers
{
   template <>
   class index_statistic_provider<worth::chain::operation_index>
   {
   public:
      typedef worth::chain::operation_index IndexType;
         ...
         if(onlyStaticInfo == false)
         {
            for(const auto& o : index)
               info._item_additional_allocation +=
                  o.serialized_op.capacity()*sizeof(worth::chain::buffer_type::value_type);
         }
   ...
   template <>
   class index_statistic_provider<worth::chain::account_history_index>
   {
   public:
      typedef worth::chain::account_history_index IndexType;
      ...
      index_statistic_info gather_statistics(const IndexType& index, bool onlyStaticInfo) const
      {
         ...
         if(onlyStaticInfo == false)
         {
            ...
            //      sizeof(worth::chain::account_history_object::operation_container::value_type);
         }
