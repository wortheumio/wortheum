...
{
   WORTH_STD_ALLOCATOR_CONSTRUCTOR( proposal_object )
   ...
   public:
      ...
      //daily_pay (the amount of WBD that is being requested to be paid out daily)
      asset daily_pay;

      //subject (a very brief description or title for the proposal)
      shared_string subject;

      //permlink (a link to a page describing the work proposal in depth, generally this will probably be to a Wortheum post).
      ...
      time_point_sec get_end_date_with_delay() const
      {
         ...
         ret += WORTH_PROPOSAL_MAINTENANCE_CLEANUP;
...
{
   WORTH_STD_ALLOCATOR_CONSTRUCTOR( proposal_vote_object )
...
template<> struct is_static_length< worth::chain::proposal_vote_object > : public boost::true_type {};

} // mira
#endif

FC_REFLECT( worth::chain::proposal_object, (id)(proposal_id)(creator)(receiver)(start_date)(end_date)(daily_pay)(subject)(permlink)(total_votes)(removed) )
CHAINBASE_SET_INDEX_TYPE( worth::chain::proposal_object, worth::chain::proposal_index )

FC_REFLECT( worth::chain::proposal_vote_object, (id)(voter)(proposal_id) )
CHAINBASE_SET_INDEX_TYPE( worth::chain::proposal_vote_object, worth::chain::proposal_vote_index )
