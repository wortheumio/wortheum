...
#include <worth/chain/buffer_type.hpp>

#include <worth/chain/multi_index_types.hpp>

#ifndef ENABLE_MIRA
#define WORTH_STD_ALLOCATOR_CONSTRUCTOR( object_type )   \
      object_type () = delete;                           \
   public:
#else
#define WORTH_STD_ALLOCATOR_CONSTRUCTOR( object_type )   \
...
using worth::protocol::chain_id_type;
...
#ifdef WORTHEUM_ENABLE_WMT
class wmt_token_object;
class account_regular_balance_object;
class account_rewards_balance_object;
class nai_pool_object;
class wmt_token_emissions_object;
class wmt_contribution_object;
class wmt_ico_object;
#endif
...
#ifdef WORTHEUM_ENABLE_WMT
typedef oid< wmt_token_object                       > wmt_token_id_type;
...
typedef oid< wmt_token_emissions_object             > wmt_token_emissions_object_id_type;
typedef oid< wmt_contribution_object                > wmt_contribution_object_id_type;
typedef oid< wmt_ico_object                         > wmt_ico_object_id_type;
#endif
...
template<> struct is_static_length< worth::protocol::account_name_type > : public boost::true_type {};
template<> struct is_static_length< worth::protocol::asset_symbol_type > : public boost::true_type {};
template<> struct is_static_length< worth::protocol::asset > : public boost::true_type {};
template<> struct is_static_length< worth::protocol::price > : public boost::true_type {};
...
#ifndef ENABLE_MIRA
inline void to_variant( const worth::chain::shared_string& s, variant& var )
{
   var = fc::string( worth::chain::to_string( s ) );
}

inline void from_variant( const variant& var, worth::chain::shared_string& s )
{
...
{
   std::string str = worth::chain::to_string( ss );
   fc::raw::pack( s, str );
}
...
void unpack( Stream& s, chainbase::shared_string& ss, uint32_t depth )
{
   ...
   worth::chain::from_string( ss, str );
}
#endif
...
#ifndef ENABLE_MIRA
template< typename T >
T unpack_from_vector( const worth::chain::buffer_type& s )
{
   try
   {
      T tmp;
...
FC_REFLECT_ENUM( worth::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_metadata_object_type)
                 (account_authority_object_type)
...
#ifndef ENABLE_MIRA
FC_REFLECT_TYPENAME( worth::chain::shared_string )
#endif

FC_REFLECT_ENUM( worth::chain::bandwidth_type, (post)(forum)(market) )
