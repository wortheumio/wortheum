...
#include <worth/chain/compound.hpp>
#include <worth/chain/custom_operation_interpreter.hpp>
#include <worth/chain/database.hpp>
#include <worth/chain/database_exceptions.hpp>
#include <worth/chain/db_with.hpp>
#include <worth/chain/evaluator_registry.hpp>
#include <worth/chain/global_property_object.hpp>
#include <worth/chain/history_object.hpp>
#include <worth/chain/optional_action_evaluator.hpp>
...
#include <worth/chain/shared_db_merkle.hpp>
#include <worth/chain/witness_schedule.hpp>

#include <worth/chain/util/asset.hpp>
#include <worth/chain/util/reward.hpp>
#include <worth/chain/util/uint256.hpp>
#include <worth/chain/util/reward.hpp>
#include <worth/chain/util/manabar.hpp>
#include <worth/chain/util/rd_setup.hpp>
#include <worth/chain/util/nai_generator.hpp>
#include <worth/chain/util/wps_processor.hpp>
...
FC_REFLECT( worth::chain::object_schema_repr, (space_type)(type) )
FC_REFLECT( worth::chain::operation_schema_repr, (id)(type) )
FC_REFLECT( worth::chain::db_schema, (types)(object_types)(operation_type)(custom_operation_types) )
...
struct reward_fund_context
{
   ...
   asset       reward_balance = asset( 0, WORTH_SYMBOL );
   share_type  worth_awarded = 0;
};
...
void database::open( const open_args& args )
{
   try
   {
      ...
      if( !find< dynamic_global_property_object >() )
         with_write_lock( [&]()
         {
            init_genesis( args.initial_supply, args.wbd_initial_supply );
         });
      ...
      _wps_remove_threshold = args.wps_remove_threshold;
...
uint32_t database::reindex( const open_args& args )
{
   ...
   BOOST_SCOPE_EXIT(this_,&note) {
      WORTH_TRY_NOTIFY(this_->_post_reindex_signal, note);
   ...
   try
   {
      ...
      WORTH_TRY_NOTIFY(_pre_reindex_signal, note);
      ...
      WORTH_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{
   return worth_chain_id;
}

void database::set_chain_id( const chain_id_type& chain_id )
{
   worth_chain_id = chain_id;

   idump( (worth_chain_id) );
}
...
{ try {
   if( !has_hardfork( WORTH_HARDFORK_0_6__127 ) )
...
{
   if( has_hardfork( WORTH_HARDFORK_0_17__769 ) || comment.parent_author == WORTH_ROOT_POST_PARENT )
...
{
   return get< reward_fund_object, by_name >( WORTH_POST_REWARD_FUND_NAME );
}
...
asset database::get_effective_vesting_shares( const account_object& account, asset_symbol_type vested_symbol )const
{
   ...
   FC_ASSERT( vested_symbol.space() == asset_symbol_type::wmt_nai_space );
   FC_ASSERT( vested_symbol.is_vesting() );

#pragma message( "TODO: Update the code below when delegation is modified to support WMTs." )
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(WORTH_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
...
void database::pop_block()
{
   try
   {
      ...
      WORTH_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
{
   WORTH_TRY_NOTIFY( _pre_apply_operation_signal, note )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = WORTH_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / WORTH_BLOCK_INTERVAL + 1;
}

/**
 *  Converts WORTH into wbd and adds it to to_account while reducing the WORTH supply
 *  by WORTH and increasing the wbd supply by the specified amount.
 */
std::pair< asset, asset > database::create_wbd( const account_object& to_account, asset worth, bool to_reward_balance )
{
   std::pair< asset, asset > assets( asset( 0, WBD_SYMBOL ), asset( 0, WORTH_SYMBOL ) );
   ...
   try
   {
      ...
      {
         auto to_wbd = ( gpo.wbd_print_rate * worth.amount ) / WORTH_100_PERCENT;
         auto to_worth = worth.amount - to_wbd;

         auto wbd = asset( to_wbd, WORTH_SYMBOL ) * median_price;

         if( to_reward_balance )
         {
            adjust_reward_balance( to_account, wbd );
            adjust_reward_balance( to_account, asset( to_worth, WORTH_SYMBOL ) );
         }
         else
         {
            adjust_balance( to_account, wbd );
            adjust_balance( to_account, asset( to_worth, WORTH_SYMBOL ) );
         }

         adjust_supply( asset( -to_wbd, WORTH_SYMBOL ) );
         adjust_supply( wbd );
         assets.first = wbd;
         assets.second = asset( to_worth, WORTH_SYMBOL );
      }
      else
      {
         adjust_balance( to_account, worth );
         assets.second = worth;
      }
   }
   FC_CAPTURE_LOG_AND_RETHROW( (to_account.name)(worth) )
...
asset create_vesting2( database& db, const account_object& to_account, asset liquid, bool to_reward_balance, Before&& before_vesting_callback )
{
   try
   {
      auto calculate_new_vesting = [ liquid ] ( price vesting_share_price ) -> asset
         {
         /**
          *  The ratio of total_vesting_shares / total_vesting_fund_worth should not
...
#ifdef WORTHEUM_ENABLE_WMT
      if( liquid.symbol.space() == asset_symbol_type::wmt_nai_space )
      {
         FC_ASSERT( liquid.symbol.is_vesting() == false );
         // Get share price.
         const auto& wmt = db.get< wmt_token_object, by_symbol >( liquid.symbol );
         FC_ASSERT( wmt.allow_voting == to_reward_balance, "No voting - no rewards" );
         price vesting_share_price = to_reward_balance ? wmt.get_reward_vesting_share_price() : wmt.get_vesting_share_price();
         ...
         db.modify( wmt, [&]( wmt_token_object& wmt_object )
         {
            if( to_reward_balance )
            {
               wmt_object.pending_rewarded_vesting_shares += new_vesting.amount;
               wmt_object.pending_rewarded_vesting_wmt += liquid.amount;
            }
            else
            {
               wmt_object.total_vesting_fund_wmt += liquid.amount;
               wmt_object.total_vesting_shares += new_vesting.amount;
            }
         } );

         // NOTE that WMT vesting does not impact witness voting.

         return new_vesting;
      }
#endif

      FC_ASSERT( liquid.symbol == WORTH_SYMBOL );
      // ^ A novelty, needed but risky in case someone managed to slip WBD/TESTS here in blockchain history.
      // Get share price.
      ...
      {
         if( db.has_hardfork( WORTH_HARDFORK_0_20__2539 ) )
         {
            db.modify( to_account, [&]( account_object& a )
            {
               util::update_manabar(
                  cprops,
                  a,
                  db.has_hardfork( WORTH_HARDFORK_0_21__3336 ),
      ...
      db.modify( cprops, [&]( dynamic_global_property_object& props )
      {
         if( to_reward_balance )
         {
            ...
            props.pending_rewarded_vesting_worth += liquid;
         }
         else
         {
            props.total_vesting_fund_worth += liquid;
 ...
 * @param liquid     - WORTH or liquid WMT to be converted to vesting shares
 */
...
uint32_t database::get_pow_summary_target()const
{
   ...
   if( has_hardfork( WORTH_HARDFORK_0_16__551 ) )
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   const std::array< share_type, WORTH_MAX_PROXY_RECURSION_DEPTH+1 >& delta,
                                   int depth )
{
   if( a.proxy != WORTH_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= WORTH_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = WORTH_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = WORTH_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
void database::adjust_witness_vote( const witness_object& witness, share_type delta )
{
   ...
   modify( witness, [&]( witness_object& w )
   {
      ...
      if( has_hardfork( WORTH_HARDFORK_0_2 ) )
         w.virtual_scheduled_time = w.virtual_last_update + (WORTH_VIRTUAL_SCHEDULE_LAP_LENGTH2 - w.virtual_position)/(w.votes.value+1);
      else
         w.virtual_scheduled_time = w.virtual_last_update + (WORTH_VIRTUAL_SCHEDULE_LAP_LENGTH - w.virtual_position)/(w.votes.value+1);

      /** witnesses with a low number of votes could overflow the time field and end up with a scheduled time in the past */
      if( has_hardfork( WORTH_HARDFORK_0_4 ) )
      {
...
void database::clear_witness_votes( const account_object& a )
{
   ...
   if( has_hardfork( WORTH_HARDFORK_0_6__104 ) )
...
{
   if( !has_hardfork( WORTH_HARDFORK_0_14__327 ) ) return;

   const auto& null_account = get_account( WORTH_NULL_ACCOUNT );
   asset total_worth( 0, WORTH_SYMBOL );
   asset total_wbd( 0, WBD_SYMBOL );
   asset total_vests( 0, VESTS_SYMBOL );

   asset vesting_shares_worth_value = asset( 0, WORTH_SYMBOL );

   if( null_account.balance.amount > 0 )
   {
      total_worth += null_account.balance;
   }

   if( null_account.savings_balance.amount > 0 )
   {
      total_worth += null_account.savings_balance;
   }

   if( null_account.wbd_balance.amount > 0 )
   {
      total_wbd += null_account.wbd_balance;
   }

   if( null_account.savings_wbd_balance.amount > 0 )
   {
      total_wbd += null_account.savings_wbd_balance;
   }

   if( null_account.vesting_shares.amount > 0 )
   {
      const auto& gpo = get_dynamic_global_properties();
      vesting_shares_worth_value = null_account.vesting_shares * gpo.get_vesting_share_price();
      total_worth += vesting_shares_worth_value;
      total_vests += null_account.vesting_shares;
   }

   if( null_account.reward_worth_balance.amount > 0 )
   {
      total_worth += null_account.reward_worth_balance;
   }

   if( null_account.reward_wbd_balance.amount > 0 )
   {
      total_wbd += null_account.reward_wbd_balance;
   }

   if( null_account.reward_vesting_balance.amount > 0 )
   {
      total_worth += null_account.reward_vesting_worth;
      total_vests += null_account.reward_vesting_balance;
   }

   if( (total_worth.amount.value == 0) && (total_wbd.amount.value == 0) && (total_vests.amount.value == 0) )
      return;
   ...
   if( total_worth.amount.value > 0 )
      vop.total_cleared.push_back( total_worth );
   if( total_vests.amount.value > 0 )
      vop.total_cleared.push_back( total_vests );
   if( total_wbd.amount.value > 0 )
      vop.total_cleared.push_back( total_wbd );
   ...
   if( null_account.wbd_balance.amount > 0 )
   {
      adjust_balance( null_account, -null_account.wbd_balance );
   }

   if( null_account.savings_wbd_balance.amount > 0 )
   {
      adjust_savings_balance( null_account, -null_account.savings_wbd_balance );
   }
   ...
   if( null_account.vesting_shares.amount > 0 )
   {
      ...
      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         ...
         g.total_vesting_fund_worth -= vesting_shares_worth_value;
      });
   ...
   if( null_account.reward_worth_balance.amount > 0 )
   {
      adjust_reward_balance( null_account, -null_account.reward_worth_balance );
   }

   if( null_account.reward_wbd_balance.amount > 0 )
   {
      adjust_reward_balance( null_account, -null_account.reward_wbd_balance );
   }
   ...
   if( null_account.reward_vesting_balance.amount > 0 )
   {
      ...
      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         ...
         g.pending_rewarded_vesting_worth -= null_account.reward_vesting_worth;
      });

      modify( null_account, [&]( account_object& a )
      {
         a.reward_vesting_worth.amount = 0;
   ...
   if( total_worth.amount > 0 )
      adjust_supply( -total_worth );

   if( total_wbd.amount > 0 )
      adjust_supply( -total_wbd );
...
{
   if( has_hardfork( WORTH_PROPOSALS_HARDFORK ) )
   {
      wps_processor wps( *this );
      wps.run( note );
   }
}
...
{
   if( head_block_num() >= WORTH_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
...
void database::process_vesting_withdrawals()
{
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
      ...
      share_type vests_deposited_as_worth = 0;
      share_type vests_deposited_as_vests = 0;
      asset total_worth_converted = asset( 0, WORTH_SYMBOL );

      // Do two passes, the first for vests, the second for worth. Try to maintain as much accuracy for vests as possible.
           ...
           ++itr )
      {
         if( itr->auto_vest )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / WORTH_100_PERCENT ).to_uint64();
           ...
           ++itr )
      {
         if( !itr->auto_vest )
         {
            ...
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / WORTH_100_PERCENT ).to_uint64();
            vests_deposited_as_worth += to_deposit;
            auto converted_worth = asset( to_deposit, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
            total_worth_converted += converted_worth;

            if( to_deposit > 0 )
            {
               operation vop = fill_vesting_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, VESTS_SYMBOL), converted_worth );

               pre_push_virtual_operation( vop );

               modify( to_account, [&]( account_object& a )
               {
                  a.balance += converted_worth;
               });

               modify( cprops, [&]( dynamic_global_property_object& o )
               {
                  o.total_vesting_fund_worth -= converted_worth;
      ...
      share_type to_convert = to_withdraw - vests_deposited_as_worth - vests_deposited_as_vests;
      FC_ASSERT( to_convert >= 0, "Deposited more vests than were supposed to be withdrawn" );

      auto converted_worth = asset( to_convert, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
      operation vop = fill_vesting_withdraw_operation( from_account.name, from_account.name, asset( to_convert, VESTS_SYMBOL ), converted_worth );
      pre_push_virtual_operation( vop );
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( WORTH_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
...
void database::adjust_total_payout( const comment_object& cur, const asset& wbd_created, const asset& curator_wbd_value, const asset& beneficiary_value )
{
   modify( cur, [&]( comment_object& c )
   {
      // input assets should be in wbd
      c.total_payout_value += wbd_created;
      c.curator_payout_value += curator_wbd_value;
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         for( auto& item : proxy_set )
         {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               create_vesting2( *this, voter, asset( claim, WORTH_SYMBOL ), has_hardfork( WORTH_HARDFORK_0_17__659 ),
                  [&]( const asset& reward )
                  {
...
void fill_comment_reward_context_local_state( util::comment_reward_context& ctx, const comment_object& comment )
{
   ...
   ctx.max_wbd = comment.max_accepted_payout;
}
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment, bool forward_curation_remainder )
{
   try
   {
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         if( has_hardfork( WORTH_HARDFORK_0_17__774 ) )
         {
         ...
         if( reward_tokens > 0 )
         {
            share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / WORTH_100_PERCENT ).to_uint64();
            ...
            {
               auto benefactor_tokens = ( author_tokens * b.weight ) / WORTH_100_PERCENT;
               auto benefactor_vesting_worth = benefactor_tokens;
               auto vop = comment_benefactor_reward_operation( b.account, comment.author, to_string( comment.permlink ), asset( 0, WBD_SYMBOL ), asset( 0, WORTH_SYMBOL ), asset( 0, VESTS_SYMBOL ) );

               if( has_hardfork( WORTH_HARDFORK_0_21__3343 ) && b.account == WORTH_TREASURY_ACCOUNT )
               {
                  benefactor_vesting_worth = 0;
                  vop.wbd_payout = asset( benefactor_tokens, WORTH_SYMBOL ) * get_feed_history().current_median_history;
                  adjust_balance( get_account( WORTH_TREASURY_ACCOUNT ), vop.wbd_payout );
                  adjust_supply( asset( -benefactor_tokens, WORTH_SYMBOL ) );
                  adjust_supply( vop.wbd_payout );
               }
               else if( has_hardfork( WORTH_HARDFORK_0_20__2022 ) )
               {
                  auto benefactor_wbd_worth = ( benefactor_tokens * comment.percent_worth_dollars ) / ( 2 * WORTH_100_PERCENT ) ;
                  benefactor_vesting_worth  = benefactor_tokens - benefactor_wbd_worth;
                  auto wbd_payout           = create_wbd( get_account( b.account ), asset( benefactor_wbd_worth, WORTH_SYMBOL ), true );

                  vop.wbd_payout   = wbd_payout.first; // WBD portion
                  vop.worth_payout = wbd_payout.second; // WORTH portion
               }

               create_vesting2( *this, get_account( b.account ), asset( benefactor_vesting_worth, WORTH_SYMBOL ), has_hardfork( WORTH_HARDFORK_0_17__659 ),
               [&]( const asset& reward )
               {
            ...
            auto wbd_worth     = ( author_tokens * comment.percent_worth_dollars ) / ( 2 * WORTH_100_PERCENT ) ;
            auto vesting_worth = author_tokens - wbd_worth;

            const auto& author = get_account( comment.author );
            auto wbd_payout = create_wbd( author, asset( wbd_worth, WORTH_SYMBOL ), has_hardfork( WORTH_HARDFORK_0_17__659 ) );
            operation vop = author_reward_operation( comment.author, to_string( comment.permlink ), wbd_payout.first, wbd_payout.second, asset( 0, VESTS_SYMBOL ) );

            create_vesting2( *this, author, asset( vesting_worth, WORTH_SYMBOL ), has_hardfork( WORTH_HARDFORK_0_17__659 ),
            ...
            adjust_total_payout( comment, wbd_payout.first + to_wbd( wbd_payout.second + asset( vesting_worth, WORTH_SYMBOL ) ), to_wbd( asset( curation_tokens, WORTH_SYMBOL ) ), to_wbd( asset( total_beneficiary, WORTH_SYMBOL ) ) );

            post_push_virtual_operation( vop );
            vop = comment_reward_operation( comment.author, to_string( comment.permlink ), to_wbd( asset( claimed_reward, WORTH_SYMBOL ) ) );
            pre_push_virtual_operation( vop );
            post_push_virtual_operation( vop );
      ...
      modify( comment, [&]( comment_object& c )
      {
         ...
         */
         ...
         if( has_hardfork( WORTH_HARDFORK_0_17__769 ) )
         {
            c.cashout_time = fc::time_point_sec::maximum();
         }
         else if( c.parent_author == WORTH_ROOT_POST_PARENT )
         {
            if( has_hardfork( WORTH_HARDFORK_0_12__177 ) && c.last_payout == fc::time_point_sec::min() )
               c.cashout_time = head_block_time() + WORTH_SECOND_CASHOUT_WINDOW;
            else
               c.cashout_time = fc::time_point_sec::maximum();
         }
      ...
      while( vote_itr != vote_idx.end() && vote_itr->comment == comment.id )
      {
         ...
         ++vote_itr;
         if( !has_hardfork( WORTH_HARDFORK_0_12__177 ) || calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum() )
         {
...
void database::process_comment_cashout()
{
   ...
   if( !has_hardfork( WORTH_FIRST_CASHOUT_TIME ) )
      return;

   const auto& gpo = get_dynamic_global_properties();
   util::comment_reward_context ctx;
   ctx.current_worth_price = get_feed_history().current_median_history;

   vector< reward_fund_context > funds;
   vector< share_type > worth_awarded;
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      modify( *itr, [&]( reward_fund_object& rfo )
      {
         ...
         if( has_hardfork( WORTH_HARDFORK_0_19__1051 ) )
            decay_time = WORTH_RECENT_RSHARES_DECAY_TIME_HF19;
         else
            decay_time = WORTH_RECENT_RSHARES_DECAY_TIME_HF17;
   ...
   if( has_hardfork( WORTH_HARDFORK_0_17__771 ) )
   {
    ...
    * The helper only does token allocation based on curation rewards and the WBD
    * global %, etc.
    *
   ...
   while( current != cidx.end() && current->cashout_time <= head_block_time() )
   {
      if( has_hardfork( WORTH_HARDFORK_0_17__771 ) )
      {
         ...
         ctx.total_reward_fund_worth = funds[ fund_id ].reward_balance;

         bool forward_curation_remainder = !has_hardfork( WORTH_HARDFORK_0_20__1877 );

         funds[ fund_id ].worth_awarded += cashout_comment_helper( ctx, *current, forward_curation_remainder );
      }
      else
      {
         ...
         while( itr != com_by_root.end() && itr->root_comment == current->root_comment )
         {
            ...
            ctx.total_reward_fund_worth = gpo.total_reward_fund_worth;
            ...
            if( reward > 0 )
            {
               ...
               {
                  p.total_reward_fund_worth.amount -= reward;
               });
            }
         }
      }
   ...
   if( funds.size() )
   {
      for( size_t i = 0; i < funds.size(); i++ )
      {
         modify( get< reward_fund_object, by_id >( reward_fund_id_type( i ) ), [&]( reward_fund_object& rfo )
         {
            ...
            rfo.reward_balance -= asset( funds[ i ].worth_awarded, WORTH_SYMBOL );
         });
      }
   }
}
...
/**
 *  Overall the network has an inflation rate of 102% of virtual worth per year
...
void database::process_funds()
{
   ...
   if( has_hardfork( WORTH_HARDFORK_0_16__551) )
   {
       ...
       */
      int64_t start_inflation_rate = int64_t( WORTH_INFLATION_RATE_START_PERCENT );
      int64_t inflation_rate_adjustment = int64_t( head_block_num() / WORTH_INFLATION_NARROWING_PERIOD );
      int64_t inflation_rate_floor = int64_t( WORTH_INFLATION_RATE_STOP_PERCENT );

      // below subtraction cannot underflow int64_t because inflation_rate_adjustment is <2^32
      int64_t current_inflation_rate = std::max( start_inflation_rate - inflation_rate_adjustment, inflation_rate_floor );

      auto new_worth = ( props.virtual_supply.amount * current_inflation_rate ) / ( int64_t( WORTH_100_PERCENT ) * int64_t( WORTH_BLOCKS_PER_YEAR ) );
      auto content_reward = ( new_worth * props.content_reward_percent ) / WORTH_100_PERCENT;
      if( has_hardfork( WORTH_HARDFORK_0_17__774 ) )
         content_reward = pay_reward_funds( content_reward );
      auto vesting_reward = ( new_worth * props.vesting_reward_percent ) / WORTH_100_PERCENT;
      auto wps_fund = ( new_worth * props.wps_fund_percent ) / WORTH_100_PERCENT;
      auto witness_reward = new_worth - content_reward - vesting_reward - wps_fund;

      const auto& cwit = get_witness( props.current_witness );
      witness_reward *= WORTH_MAX_WITNESSES;
      ...
      auto new_wbd = asset( 0, WBD_SYMBOL );

      if( wps_fund.value )
      {
         new_wbd = asset( wps_fund, WORTH_SYMBOL ) * feed.current_median_history;
         adjust_balance( WORTH_TREASURY_ACCOUNT, new_wbd );
      }

      new_worth = content_reward + vesting_reward + witness_reward;

      modify( props, [&]( dynamic_global_property_object& p )
      {
         p.total_vesting_fund_worth += asset( vesting_reward, WORTH_SYMBOL );
         if( !has_hardfork( WORTH_HARDFORK_0_17__774 ) )
            p.total_reward_fund_worth  += asset( content_reward, WORTH_SYMBOL );
         p.current_supply      += asset( new_worth, WORTH_SYMBOL );
         p.current_wbd_supply  += new_wbd;
         p.virtual_supply      += asset( new_worth + wps_fund, WORTH_SYMBOL );
         p.wps_interval_ledger += new_wbd;
      });

      operation vop = producer_reward_operation( cwit.owner, asset( 0, VESTS_SYMBOL ) );
      create_vesting2( *this, get_account( cwit.owner ), asset( witness_reward, WORTH_SYMBOL ), false,
   ...
   else
   {
      ...
      if( props.head_block_number < WORTH_START_VESTING_BLOCK )
      ...
      modify( props, [&]( dynamic_global_property_object& p )
      {
          p.total_vesting_fund_worth += vesting_reward;
          p.total_reward_fund_worth  += content_reward;
...
{
   if( has_hardfork( WORTH_HARDFORK_0_12__178 ) )
      return asset( 0, WORTH_SYMBOL );

   const auto& props = get_dynamic_global_properties();
   static_assert( WORTH_LIQUIDITY_REWARD_PERIOD_SEC == 60*60, "this code assumes a 1 hour time interval" );
   asset percent( protocol::calc_percent_reward_per_hour< WORTH_LIQUIDITY_APR_PERCENT >( props.virtual_supply.amount ), WORTH_SYMBOL );
   return std::max( percent, WORTH_MIN_LIQUIDITY_REWARD );
}

asset database::get_content_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( WORTH_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< WORTH_CONTENT_APR_PERCENT >( props.virtual_supply.amount ), WORTH_SYMBOL );
   return std::max( percent, WORTH_MIN_CONTENT_REWARD );
}
...
asset database::get_curation_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( WORTH_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< WORTH_CURATE_APR_PERCENT >( props.virtual_supply.amount ), WORTH_SYMBOL);
   return std::max( percent, WORTH_MIN_CURATE_REWARD );
}
...
asset database::get_producer_reward()
{
   const auto& props = get_dynamic_global_properties();
   static_assert( WORTH_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< WORTH_PRODUCER_APR_PERCENT >( props.virtual_supply.amount ), WORTH_SYMBOL);
   auto pay = std::max( percent, WORTH_MIN_PRODUCER_REWARD );
   const auto& witness_account = get_account( props.current_witness );

   /// pay witness in vesting shares
   if( props.head_block_number >= WORTH_START_MINER_VOTING_BLOCK || (witness_account.vesting_shares.amount.value == 0) )
   {
...
#ifndef IS_TEST_NET
   /// 0 block rewards until at least WORTH_MAX_WITNESSES have produced a POW
   if( props.num_pow_witnesses < WORTH_MAX_WITNESSES && props.head_block_number < WORTH_START_VESTING_BLOCK )
      return asset( 0, WORTH_SYMBOL );
#endif

   static_assert( WORTH_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   static_assert( WORTH_MAX_WITNESSES == 21, "this code assumes 21 per round" );
   asset percent( calc_percent_reward_per_round< WORTH_POW_APR_PERCENT >( props.virtual_supply.amount ), WORTH_SYMBOL);
   return std::max( percent, WORTH_MIN_POW_REWARD );
}
...
void database::pay_liquidity_reward()
{
   ...
   if( (head_block_num() % WORTH_LIQUIDITY_REWARD_BLOCKS) == 0 )
   {
...
uint16_t database::get_curation_rewards_percent( const comment_object& c ) const
{
   ...
   else if( has_hardfork( WORTH_HARDFORK_0_8__116 ) )
      return WORTH_1_PERCENT * 25;
   else
      return WORTH_1_PERCENT * 50;
}
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / WORTH_100_PERCENT;

      modify( *itr, [&]( reward_fund_object& rfo )
      {
         rfo.reward_balance += asset( r, WORTH_SYMBOL );
      });

      used_rewards += r;

      // Sanity check to ensure we aren't printing more WORTH than has been allocated through inflation
 ...
 *  the head block time and then converts them to/from worth/wbd at the
...
void database::process_conversions()
{
   ...
   asset net_wbd( 0, WBD_SYMBOL );
   asset net_worth( 0, WORTH_SYMBOL );
   ...
   modify( props, [&]( dynamic_global_property_object& p )
   {
       p.current_supply += net_worth;
       p.current_wbd_supply -= net_wbd;
       p.virtual_supply += net_worth;
       p.virtual_supply -= net_wbd * get_feed_history().current_median_history;
   } );
}

asset database::to_wbd( const asset& worth )const
{
   return util::to_wbd( get_feed_history().current_median_history, worth );
}

asset database::to_worth( const asset& wbd )const
{
   return util::to_worth( get_feed_history().current_median_history, wbd );
}
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + WORTH_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::expire_escrow_ratification()
{
   ...
   while( escrow_itr != escrow_idx.end() && !escrow_itr->is_approved() && escrow_itr->ratification_deadline <= head_block_time() )
   {
      ...
      adjust_balance( old_escrow.from, old_escrow.worth_balance );
      adjust_balance( old_escrow.from, old_escrow.wbd_balance );
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      std::array<share_type, WORTH_MAX_PROXY_RECURSION_DEPTH+1> delta;
      delta[0] = -account.vesting_shares.amount;
      for( int i = 0; i < WORTH_MAX_PROXY_RECURSION_DEPTH; ++i )
      ...
      {
         a.can_vote = false;
         a.proxy = WORTH_PROXY_TO_SELF_ACCOUNT;
      });
...
#ifdef WORTHEUM_ENABLE_WMT
   _my->_evaluator_registry.register_evaluator< wmt_setup_evaluator                      >();
   _my->_evaluator_registry.register_evaluator< wmt_setup_emissions_evaluator            >();
   _my->_evaluator_registry.register_evaluator< wmt_set_setup_parameters_evaluator       >();
   _my->_evaluator_registry.register_evaluator< wmt_set_runtime_parameters_evaluator     >();
   _my->_evaluator_registry.register_evaluator< wmt_create_evaluator                     >();
   _my->_evaluator_registry.register_evaluator< wmt_contribute_evaluator                 >();
#endif
...
void database::init_genesis( uint64_t init_supply, uint64_t wbd_init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(WORTH_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = WORTH_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = WORTH_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = WORTH_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = WORTH_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
      });
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = WORTH_TREASURY_ACCOUNT;
      } );
#endif

      create< account_object >( [&]( account_object& a )
      {
         a.name = WORTH_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = WORTH_TEMP_ACCOUNT;
         auth.owner.weight_threshold = 0;
         auth.active.weight_threshold = 0;
      });

      for( int i = 0; i < WORTH_NUM_INIT_MINERS; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = WORTH_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            a.memo_key = init_public_key;
            a.balance  = asset( i ? 0 : init_supply, WORTH_SYMBOL );
            a.wbd_balance = asset( i ? 0 : wbd_init_supply, WBD_SYMBOL );
         } );

         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = WORTH_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = WORTH_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = WORTH_INIT_MINER_NAME;
         p.time = WORTH_GENESIS_TIME;
         p.recent_slots_filled = fc::uint128::max_value();
         p.participation_count = 128;
         p.current_supply = asset( init_supply, WORTH_SYMBOL );
         p.init_wbd_supply = asset( wbd_init_supply, WBD_SYMBOL );
         p.virtual_supply = p.current_supply;
         p.maximum_block_size = WORTH_MAX_BLOCK_SIZE;
         p.reverse_auction_seconds = WORTH_REVERSE_AUCTION_WINDOW_SECONDS_HF6;
         p.wbd_stop_percent = WORTH_WBD_STOP_PERCENT_HF14;
         p.wbd_start_percent = WORTH_WBD_START_PERCENT_HF14;
         p.next_maintenance_time = WORTH_GENESIS_TIME;
         p.last_budget_time = WORTH_GENESIS_TIME;
      } );

#ifdef IS_TEST_NET
      create< feed_history_object >( [&]( feed_history_object& o )
      {
         o.current_median_history = price( asset( 1, WBD_SYMBOL ), asset( 1, WORTH_SYMBOL ) );
      });
#else
      // Nothing to do
      create< feed_history_object >( [&]( feed_history_object& o ) {});
      ...
      {
         hpo.processed_hardforks.push_back( WORTH_GENESIS_TIME );
      } );
      ...
      create< witness_schedule_object >( [&]( witness_schedule_object& wso )
      {
         ...
         wso.current_shuffled_witnesses[0] = WORTH_INIT_MINER_NAME;
         util::rd_system_params account_subsidy_system_params;
         account_subsidy_system_params.resource_unit = WORTH_ACCOUNT_SUBSIDY_PRECISION;
         account_subsidy_system_params.decay_per_time_unit_denom_shift = WORTH_RD_DECAY_DENOM_SHIFT;
         ...
         w_budget = (w_budget * WORTH_WITNESS_SUBSIDY_BUDGET_PERCENT) / WORTH_100_PERCENT;
         w_budget = std::min( w_budget, int64_t(std::numeric_limits<int32_t>::max()) );
         uint64_t w_decay = wso.median_props.account_subsidy_decay;
         w_decay = (w_decay * WORTH_WITNESS_SUBSIDY_DECAY_PERCENT) / WORTH_100_PERCENT;
...
void database::notify_changed_objects()
{
   try
   {
      ...
      get_changed_ids( ids );
      WORTH_TRY_NOTIFY( changed_objects, ids )*/
      /*
      if( _undo_db.enabled() )
      {
         ...
         WORTH_TRY_NOTIFY( changed_objects, changed_ids )
      }
      */
   }
   FC_CAPTURE_AND_RETHROW()
...
void database::check_free_memory( bool force_print, uint32_t current_block_num )
{
   ...
   if( BOOST_UNLIKELY( _shared_file_full_threshold != 0 && _shared_file_scale_rate != 0 && free_mem < ( ( uint128_t( WORTH_100_PERCENT - _shared_file_full_threshold ) * max_mem ) / WORTH_100_PERCENT ).to_uint64() ) )
   {
      uint64_t new_max = ( uint128_t( max_mem * _shared_file_scale_rate ) / WORTH_100_PERCENT ).to_uint64() + max_mem;
...
{ try {
   ...
   if( BOOST_UNLIKELY( next_block_num == 1 ) )
   {
      ...
      uint32_t n;
      for( n=0; n<WORTH_NUM_HARDFORKS; n++ )
      {
   ...
   if( has_hardfork( WORTH_HARDFORK_0_12 ) )
   {
   ...
   if( block_size < WORTH_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",WORTH_MIN_BLOCK_SIZE)
      );
   }
   ...
   if( has_hardfork( WORTH_HARDFORK_0_5__54 ) ) // Cannot remove after hardfork
   {
...
struct process_header_visitor
{
   ...
   {
      FC_ASSERT( _db.has_hardfork( WORTH_WMT_HARDFORK ), "Automated actions are not enabled until WMT hardfork." );
...
try {
   if( (head_block_num() % WORTH_FEED_INTERVAL_BLOCKS) != 0 )
      return;
   ...
   for( int i = 0; i < wso.num_scheduled_witnesses; i++ )
   {
      ...
      if( has_hardfork( WORTH_HARDFORK_0_19__822 ) )
      {
         if( now < wit.last_wbd_exchange_update + WORTH_MAX_FEED_AGE_SECONDS
            && !wit.wbd_exchange_rate.is_null() )
         {
            feeds.push_back( wit.wbd_exchange_rate );
         }
      }
      else if( wit.last_wbd_exchange_update < now + WORTH_MAX_FEED_AGE_SECONDS &&
          !wit.wbd_exchange_rate.is_null() )
      {
         feeds.push_back( wit.wbd_exchange_rate );
      }
   ...
   if( feeds.size() >= WORTH_MIN_FEEDS )
   {
      ...
      modify( get_feed_history(), [&]( feed_history_object& fho )
      {
         ...
         size_t worth_feed_history_window = WORTH_FEED_HISTORY_WINDOW_PRE_HF_16;
         if( has_hardfork( WORTH_HARDFORK_0_16__551) )
            worth_feed_history_window = WORTH_FEED_HISTORY_WINDOW;

         if( fho.price_history.size() > worth_feed_history_window )
         ...
         if( fho.price_history.size() )
         {
            ...
            if( skip_price_feed_limit_check )
               return;
#endif
            if( has_hardfork( WORTH_HARDFORK_0_14__230 ) )
            {
               // This block limits the effective median price to force WBD to remain at or
               // below 10% of the combined market cap of WORTH and WBD.
               //
               // For example, if we have 500 WORTH and 100 WBD, the price is limited to
               // 900 WBD / 500 WORTH which works out to be $1.80.  At this price, 500 Worth
               // would be valued at 500 * $1.80 = $900.  100 WBD is by definition always $100,
               // so the combined market cap is $900 + $100 = $1000.

               const auto& gpo = get_dynamic_global_properties();

               if( gpo.current_wbd_supply.amount > 0 )
               {
                  price min_price( asset( 9 * gpo.current_wbd_supply.amount, WBD_SYMBOL ), gpo.current_supply );
...
{ try {
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, WORTH_MAX_SIG_CHECK_DEPTH,
            has_hardfork( WORTH_HARDFORK_0_20 ) || is_producing() ? WORTH_MAX_AUTHORITY_MEMBERSHIP : 0,
            has_hardfork( WORTH_HARDFORK_0_20 ) || is_producing() ? WORTH_MAX_SIG_CHECK_ACCOUNTS : 0,
            has_hardfork( WORTH_HARDFORK_0_20__1944 ) ? fc::ecc::bip_0062 : fc::ecc::fc_canonical );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         WORTH_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      WORTH_ASSERT( trx.expiration <= now + fc::seconds(WORTH_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",WORTH_MAX_TIME_UNTIL_EXPIRATION));
      if( has_hardfork( WORTH_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when now == trx.expiration
         WORTH_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      WORTH_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
   ...
   while( true )
   {
      ...
      if( actions_itr == actions.end() )
      {
         ...
         if( pending_itr != pending_action_idx.end() && pending_itr->execution_time <= head_block_time() )
         {
            ...
            const auto& gpo = get_dynamic_global_properties();
            uint64_t required_actions_partition_size = ( gpo.maximum_block_size * gpo.required_actions_partition_percent ) / WORTH_100_PERCENT;
...
{
   if( !has_hardfork( WORTH_WMT_HARDFORK ) ) return;
...
FC_TODO( "#ifndef not needed after HF 20 is live" );
#ifndef IS_TEST_NET
               if( has_hardfork( WORTH_HARDFORK_0_14__278 ) && !has_hardfork( WORTH_HARDFORK_0_20__SP190 ) )
               {
                  if( head_block_num() - w.last_confirmed_block_num  > WORTH_BLOCKS_PER_DAY )
                  {
   ...
   {
      WORTH_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < WORTH_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",WORTH_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
   modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
   {
      dgp.virtual_supply = dgp.current_supply
         + ( get_feed_history().current_median_history.is_null() ? asset( 0, WORTH_SYMBOL ) : dgp.current_wbd_supply * get_feed_history().current_median_history );

      auto median_price = get_feed_history().current_median_history;

      if( !median_price.is_null() && has_hardfork( WORTH_HARDFORK_0_14__230 ) )
      {
         uint16_t percent_wbd = 0;

         if( has_hardfork( WORTH_HARDFORK_0_21 ) )
         {
            percent_wbd = uint16_t( ( ( fc::uint128_t( ( dgp.current_wbd_supply * get_feed_history().current_median_history ).amount.value ) * WORTH_100_PERCENT + dgp.virtual_supply.amount.value/2 )
               / dgp.virtual_supply.amount.value ).to_uint64() );
         }
         else
         {
            percent_wbd = uint16_t( ( ( fc::uint128_t( ( dgp.current_wbd_supply * get_feed_history().current_median_history ).amount.value ) * WORTH_100_PERCENT )
            / dgp.virtual_supply.amount.value ).to_uint64() );
         }

         if( percent_wbd <= dgp.wbd_start_percent )
            dgp.wbd_print_rate = WORTH_100_PERCENT;
         else if( percent_wbd >= dgp.wbd_stop_percent )
            dgp.wbd_print_rate = 0;
         else
            dgp.wbd_print_rate = ( ( dgp.wbd_stop_percent - percent_wbd ) * WORTH_100_PERCENT ) / ( dgp.wbd_stop_percent - dgp.wbd_start_percent );
      }
   });
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < WORTH_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > WORTH_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - WORTH_MAX_WITNESSES;
      } );
   }
   else
   {
      ...
      static_assert( WORTH_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((WORTH_100_PERCENT - WORTH_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / WORTH_100_PERCENT);
...
{
   bool has_hf_20__1815 = has_hardfork( WORTH_HARDFORK_0_20__1815 );

#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
   if( has_hf_20__1815 )
   {
      WORTH_ASSERT( new_order.sell_price.quote.symbol == old_order.sell_price.base.symbol,
      ...
      WORTH_ASSERT( new_order.sell_price.base.symbol  == old_order.sell_price.quote.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      WORTH_ASSERT( new_order.for_sale > 0 && old_order.for_sale > 0,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      WORTH_ASSERT( match_price.quote.symbol == new_order.sell_price.base.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      WORTH_ASSERT( match_price.base.symbol == old_order.sell_price.base.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
   }
...
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
   if( has_hf_20__1815 )
   {
      WORTH_ASSERT( new_order_pays == new_order.amount_for_sale() ||
   ...
   if( !has_hardfork( WORTH_HARDFORK_0_12__178 ) &&
       ( (age >= WORTH_MIN_LIQUIDITY_REWARD_PERIOD_SEC && !has_hardfork( WORTH_HARDFORK_0_10__149)) ||
       (age >= WORTH_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10 && has_hardfork( WORTH_HARDFORK_0_10__149) ) ) )
   {
      if( old_order_receives.symbol == WORTH_SYMBOL )
      {
...
void database::adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_sdb )
{
   ...
   if( itr != ridx.end() )
   {
      ...
      {
         if( head_block_time() - r.last_update >= WORTH_LIQUIDITY_TIMEOUT_SEC )
         {
            r.wbd_volume = 0;
            r.worth_volume = 0;
            r.weight = 0;
         ...
         if( is_sdb )
            r.wbd_volume += volume.amount.value;
         else
            r.worth_volume += volume.amount.value;

         r.update_weight( has_hardfork( WORTH_HARDFORK_0_10__141 ) );
   ...
   else
   {
      create<liquidity_reward_balance_object>( [&](liquidity_reward_balance_object& r )
      {
         ...
         r.update_weight( has_hardfork( WORTH_HARDFORK_0_9__141 ) );
         r.last_update = head_block_time();
      } );
   }
}
...
bool database::fill_order( const limit_order_object& order, const asset& pays, const asset& receives )
{
   try
   {
      ...
      {
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
         if( has_hardfork( WORTH_HARDFORK_0_20__1815 ) )
         {
            WORTH_ASSERT( pays < order.amount_for_sale(),
              order_fill_exception, "error filling orders: ${order} ${pays} ${receives}",
              ("order", order)("pays", pays)("receives", receives) );
         }
...
void database::clear_expired_delegations()
{
   ...
   while( itr != delegations_by_exp.end() && itr->expiration < now )
   {
      ...
      {
         if( has_hardfork( WORTH_HARDFORK_0_20__2539 ) )
         {
            util::update_manabar(
               gpo,
               a,
               has_hardfork( WORTH_HARDFORK_0_21__3336 ),
...
#ifdef WORTHEUM_ENABLE_WMT
template< typename wmt_balance_object_type, class balance_operator_type >
void database::adjust_wmt_balance( const account_name_type& name, const asset& delta, bool check_account,
   balance_operator_type balance_operator )
{
   ...
   const wmt_balance_object_type* bo = find< wmt_balance_object_type, by_owner_liquid_symbol >( boost::make_tuple( name, liquid_symbol ) );
   // Note that WMT related code, being post-20-hf needs no hf-guard to do balance checks.
   if( bo == nullptr )
   {
      // No balance object related to the WMT means '0' balance. Check delta to avoid creation of negative balance.
      FC_ASSERT( delta.amount.value >= 0, "Insufficient WMT ${wmt} funds", ("wmt", delta.symbol) );
      ...
      create< wmt_balance_object_type >( [&]( wmt_balance_object_type& wmt_balance )
      {
         wmt_balance.clear_balance( liquid_symbol );
         wmt_balance.owner = name;
         balance_operator.add_to_balance( wmt_balance );
         wmt_balance.validate();
      } );
   }
   else
   {
      ...
      FC_ASSERT( result >= 0, "Insufficient WMT ${wmt} funds", ( "wmt", delta.symbol ) );
      ...
      {
         modify( *bo, [&]( wmt_balance_object_type& wmt_balance )
         {
            balance_operator.add_to_balance( wmt_balance );
         } );
      }
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         case WORTH_ASSET_NUM_WORTH:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.balance.amount.value >= 0, "Insufficient WORTH funds" );
            }
            break;
         case WORTH_ASSET_NUM_WBD:
            if( a.wbd_seconds_last_update != head_block_time() )
            {
               acnt.wbd_seconds += fc::uint128_t(a.wbd_balance.amount.value) * (head_block_time() - a.wbd_seconds_last_update).to_seconds();
               acnt.wbd_seconds_last_update = head_block_time();

               if( acnt.wbd_seconds > 0 &&
                   (acnt.wbd_seconds_last_update - acnt.wbd_last_interest_payment).to_seconds() > WORTH_WBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.wbd_seconds / WORTH_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().wbd_interest_rate;
                  interest /= WORTH_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), WBD_SYMBOL);
                  acnt.wbd_balance += interest_paid;
                  acnt.wbd_seconds = 0;
                  acnt.wbd_last_interest_payment = head_block_time();
                  ...
                  {
                     props.current_wbd_supply += interest_paid;
                     props.virtual_supply += interest_paid * get_feed_history().current_median_history;
                  } );
               }
            }
            acnt.wbd_balance += delta;
            if( check_balance )
            {
               FC_ASSERT( acnt.wbd_balance.amount.value >= 0, "Insufficient WBD funds" );
            }
            break;
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( value_delta.symbol.asset_num )
      {
         case WORTH_ASSET_NUM_WORTH:
            ...
            {
               acnt.reward_worth_balance += value_delta;
               if( check_balance )
               {
                  FC_ASSERT( acnt.reward_worth_balance.amount.value >= 0, "Insufficient reward WORTH funds" );
               }
            }
            else
            {
               acnt.reward_vesting_worth += value_delta;
         ...
         case WORTH_ASSET_NUM_WBD:
            ...
            acnt.reward_wbd_balance += value_delta;
            if( check_balance )
            {
               FC_ASSERT( acnt.reward_wbd_balance.amount.value >= 0, "Insufficient reward WBD funds" );
            }
            break;
         default:
            FC_ASSERT( false, "invalid symbol" );
...
#ifdef WORTHEUM_ENABLE_WMT
struct wmt_regular_balance_operator
{
   wmt_regular_balance_operator( const asset& delta ) : delta(delta), is_vesting(delta.symbol.is_vesting()) {}

   void add_to_balance( account_regular_balance_object& wmt_balance )
   {
      if( is_vesting )
         wmt_balance.vesting += delta;
      else
         wmt_balance.liquid += delta;
   }
...
struct wmt_reward_balance_operator
{
   wmt_reward_balance_operator( const asset& value_delta, const asset& share_delta )
   ...
   void add_to_balance( account_rewards_balance_object& wmt_balance )
   {
      if( is_vesting )
      {
         wmt_balance.pending_vesting_value += value_delta;
         wmt_balance.pending_vesting_shares += share_delta;
      }
      else
         wmt_balance.pending_liquid += value_delta;
   }
...
void database::adjust_balance( const account_object& a, const asset& delta )
{
   ...
   bool check_balance = has_hardfork( WORTH_HARDFORK_0_20__1811 );

#ifdef WORTHEUM_ENABLE_WMT
   if( delta.symbol.space() == asset_symbol_type::wmt_nai_space )
   {
      // No account object modification for WMT balance, hence separate handling here.
      // Note that WMT related code, being post-20-hf needs no hf-guard to do balance checks.
      wmt_regular_balance_operator balance_operator( delta );
      adjust_wmt_balance< account_regular_balance_object >( a.name, delta, false/*check_account*/, balance_operator );
   }
   else
#endif
   {
...
void database::adjust_balance( const account_name_type& name, const asset& delta )
{
   ...
   if( delta.symbol.space() == asset_symbol_type::wmt_nai_space )
   {
      ...
      // Note that WMT related code, being post-20-hf needs no hf-guard to do balance checks.
      wmt_regular_balance_operator balance_operator( delta );
      adjust_wmt_balance< account_regular_balance_object >( name, delta, false/*check_account*/, balance_operator );
   }
   else
#endif
   {
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         case WORTH_ASSET_NUM_WORTH:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.savings_balance.amount.value >= 0, "Insufficient savings WORTH funds" );
            }
            break;
         case WORTH_ASSET_NUM_WBD:
            if( a.savings_wbd_seconds_last_update != head_block_time() )
            {
               acnt.savings_wbd_seconds += fc::uint128_t(a.savings_wbd_balance.amount.value) * (head_block_time() - a.savings_wbd_seconds_last_update).to_seconds();
               acnt.savings_wbd_seconds_last_update = head_block_time();

               if( acnt.savings_wbd_seconds > 0 &&
                   (acnt.savings_wbd_seconds_last_update - acnt.savings_wbd_last_interest_payment).to_seconds() > WORTH_WBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.savings_wbd_seconds / WORTH_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().wbd_interest_rate;
                  interest /= WORTH_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), WBD_SYMBOL);
                  acnt.savings_wbd_balance += interest_paid;
                  acnt.savings_wbd_seconds = 0;
                  acnt.savings_wbd_last_interest_payment = head_block_time();
            ...
            acnt.savings_wbd_balance += delta;
            if( check_balance )
            {
               FC_ASSERT( acnt.savings_wbd_balance.amount.value >= 0, "Insufficient savings WBD funds" );
            }
            break;
         default:
                                      ...
                                      const asset& share_delta /*= asset(0,VESTS_SYMBOL)*/ )
{
   ...
   // No account object modification for WMT balance, hence separate handling here.
   // Note that WMT related code, being post-20-hf needs no hf-guard to do balance checks.
   if( value_delta.symbol.space() == asset_symbol_type::wmt_nai_space )
   {
      wmt_reward_balance_operator balance_operator( value_delta, share_delta );
      adjust_wmt_balance< account_rewards_balance_object >( a.name, value_delta, false/*check_account*/, balance_operator );
      return;
   }
#endif
                                      ...
                                      const asset& share_delta /*= asset(0,VESTS_SYMBOL)*/ )
{
   ...
   {
      wmt_reward_balance_operator balance_operator( value_delta, share_delta );
      adjust_wmt_balance< account_rewards_balance_object >( name, value_delta, true/*check_account*/, balance_operator );
      return;
   }
#endif
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   if( delta.symbol.space() == asset_symbol_type::wmt_nai_space )
   {
      const auto& wmt = get< wmt_token_object, by_symbol >( delta.symbol );
      auto wmt_new_supply = wmt.current_supply + delta.amount;
      FC_ASSERT( wmt_new_supply >= 0 );
      modify( wmt, [wmt_new_supply]( wmt_token_object& wmt )
      {
         wmt.current_supply = wmt_new_supply;
      });
      return;
   }
#endif

   bool check_supply = has_hardfork( WORTH_HARDFORK_0_20__1811 );

   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < WORTH_BLOCKS_PER_DAY*7 )
   ...
   modify( props, [&]( dynamic_global_property_object& props )
   {
      switch( delta.symbol.asset_num )
      {
         case WORTH_ASSET_NUM_WORTH:
         {
            asset new_vesting( (adjust_vesting && delta.amount > 0) ? delta.amount * 9 : 0, WORTH_SYMBOL );
            props.current_supply += delta + new_vesting;
            props.virtual_supply += delta + new_vesting;
            props.total_vesting_fund_worth += new_vesting;
            if( check_supply )
            {
         ...
         case WORTH_ASSET_NUM_WBD:
            props.current_wbd_supply += delta;
            props.virtual_supply = props.current_wbd_supply * get_feed_history().current_median_history + props.current_supply;
            if( check_supply )
            {
               FC_ASSERT( props.current_wbd_supply.amount.value >= 0 );
            }
            break;
         default:
            FC_ASSERT( false, "invalid symbol" );
...
asset database::get_balance( const account_object& a, asset_symbol_type symbol )const
{
   switch( symbol.asset_num )
   {
      case WORTH_ASSET_NUM_WORTH:
         return a.balance;
      case WORTH_ASSET_NUM_WBD:
         return a.wbd_balance;
      default:
      {
#ifdef WORTHEUM_ENABLE_WMT
         FC_ASSERT( symbol.space() == asset_symbol_type::wmt_nai_space, "Invalid symbol: ${s}", ("s", symbol) );
...
{
#ifdef WORTHEUM_ENABLE_WMT
   if ( symbol.space() == asset_symbol_type::wmt_nai_space )
   {
...
asset database::get_savings_balance( const account_object& a, asset_symbol_type symbol )const
{
   switch( symbol.asset_num )
   {
      ...
      case WORTH_ASSET_NUM_WBD:
         return a.savings_wbd_balance;
      default: // Note no savings balance for WMT per comments in issue 1682.
         FC_ASSERT( !"invalid symbol" );
   }
}
...
void database::generate_optional_actions()
{
   ...
   WORTH_TRY_NOTIFY( _generate_optional_actions_signal, note );
}

void database::init_hardforks()
{
   _hardfork_versions.times[ 0 ] = fc::time_point_sec( WORTH_GENESIS_TIME );
   _hardfork_versions.versions[ 0 ] = hardfork_version( 0, 0 );
   FC_ASSERT( WORTH_HARDFORK_0_1 == 1, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_1 ] = fc::time_point_sec( WORTH_HARDFORK_0_1_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_1 ] = WORTH_HARDFORK_0_1_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_2 == 2, "Invlaid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_2 ] = fc::time_point_sec( WORTH_HARDFORK_0_2_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_2 ] = WORTH_HARDFORK_0_2_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_3 == 3, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_3 ] = fc::time_point_sec( WORTH_HARDFORK_0_3_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_3 ] = WORTH_HARDFORK_0_3_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_4 == 4, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_4 ] = fc::time_point_sec( WORTH_HARDFORK_0_4_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_4 ] = WORTH_HARDFORK_0_4_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_5 == 5, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_5 ] = fc::time_point_sec( WORTH_HARDFORK_0_5_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_5 ] = WORTH_HARDFORK_0_5_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_6 == 6, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_6 ] = fc::time_point_sec( WORTH_HARDFORK_0_6_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_6 ] = WORTH_HARDFORK_0_6_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_7 == 7, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_7 ] = fc::time_point_sec( WORTH_HARDFORK_0_7_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_7 ] = WORTH_HARDFORK_0_7_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_8 == 8, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_8 ] = fc::time_point_sec( WORTH_HARDFORK_0_8_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_8 ] = WORTH_HARDFORK_0_8_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_9 == 9, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_9 ] = fc::time_point_sec( WORTH_HARDFORK_0_9_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_9 ] = WORTH_HARDFORK_0_9_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_10 == 10, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_10 ] = fc::time_point_sec( WORTH_HARDFORK_0_10_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_10 ] = WORTH_HARDFORK_0_10_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_11 == 11, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_11 ] = fc::time_point_sec( WORTH_HARDFORK_0_11_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_11 ] = WORTH_HARDFORK_0_11_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_12 == 12, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_12 ] = fc::time_point_sec( WORTH_HARDFORK_0_12_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_12 ] = WORTH_HARDFORK_0_12_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_13 == 13, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_13 ] = fc::time_point_sec( WORTH_HARDFORK_0_13_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_13 ] = WORTH_HARDFORK_0_13_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_14 == 14, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_14 ] = fc::time_point_sec( WORTH_HARDFORK_0_14_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_14 ] = WORTH_HARDFORK_0_14_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_15 == 15, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_15 ] = fc::time_point_sec( WORTH_HARDFORK_0_15_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_15 ] = WORTH_HARDFORK_0_15_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_16 == 16, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_16 ] = fc::time_point_sec( WORTH_HARDFORK_0_16_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_16 ] = WORTH_HARDFORK_0_16_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_17 == 17, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_17 ] = fc::time_point_sec( WORTH_HARDFORK_0_17_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_17 ] = WORTH_HARDFORK_0_17_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_18 == 18, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_18 ] = fc::time_point_sec( WORTH_HARDFORK_0_18_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_18 ] = WORTH_HARDFORK_0_18_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_19 == 19, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_19 ] = fc::time_point_sec( WORTH_HARDFORK_0_19_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_19 ] = WORTH_HARDFORK_0_19_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_20 == 20, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_20 ] = fc::time_point_sec( WORTH_HARDFORK_0_20_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_20 ] = WORTH_HARDFORK_0_20_VERSION;
   FC_ASSERT( WORTH_HARDFORK_0_21 == 21, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_21 ] = fc::time_point_sec( WORTH_HARDFORK_0_21_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_21 ] = WORTH_HARDFORK_0_21_VERSION;
#ifdef IS_TEST_NET
   FC_ASSERT( WORTH_HARDFORK_0_22 == 22, "Invalid hardfork configuration" );
   _hardfork_versions.times[ WORTH_HARDFORK_0_22 ] = fc::time_point_sec( WORTH_HARDFORK_0_22_TIME );
   _hardfork_versions.versions[ WORTH_HARDFORK_0_22 ] = WORTH_HARDFORK_0_22_VERSION;
#endif


   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= WORTH_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("WORTH_NUM_HARDFORKS",WORTH_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions.versions[ hardforks.last_hardfork ] <= WORTH_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( WORTH_BLOCKCHAIN_HARDFORK_VERSION >= WORTH_BLOCKCHAIN_VERSION );
   FC_ASSERT( WORTH_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions.versions[ WORTH_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      {
         while( hardforks.last_hardfork < WORTH_NUM_HARDFORKS
               && _hardfork_versions.times[ hardforks.last_hardfork + 1 ] <= head_block_time()
               && hardforks.last_hardfork < LATEST_HARD_FORK )
         {
            apply_hardfork( hardforks.last_hardfork + 1 );
         }
      }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= WORTH_NUM_HARDFORKS; i++ )
   {
      if( i <= LATEST_HARD_FORK )
...
void database::apply_hardfork( uint32_t hardfork )
{
   ...
   switch( hardfork )
   {
      case WORTH_HARDFORK_0_1:
         perform_vesting_share_split( 1000000 );
         break;
      case WORTH_HARDFORK_0_2:
         retally_witness_votes();
         break;
      case WORTH_HARDFORK_0_3:
         retally_witness_votes();
         break;
      case WORTH_HARDFORK_0_4:
         reset_virtual_schedule_time(*this);
         break;
      case WORTH_HARDFORK_0_5:
         break;
      case WORTH_HARDFORK_0_6:
         retally_witness_vote_counts();
         retally_comment_children();
         break;
      case WORTH_HARDFORK_0_7:
         break;
      case WORTH_HARDFORK_0_8:
         retally_witness_vote_counts(true);
         break;
      case WORTH_HARDFORK_0_9:
         {
         ...
         break;
      case WORTH_HARDFORK_0_10:
         retally_liquidity_weight();
         break;
      case WORTH_HARDFORK_0_11:
         break;
      case WORTH_HARDFORK_0_12:
         {
            ...
            for( auto itr = comment_idx.begin(); itr != comment_idx.end(); ++itr )
            {
               ...
               if( itr->parent_author == WORTH_ROOT_POST_PARENT )
               {
                  ...
                  if( itr->last_payout == fc::time_point_sec::min() && itr->cashout_time == fc::time_point_sec::maximum() )
                  {
                     ...
                     {
                        c.cashout_time = head_block_time() + WORTH_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                     });
                  }
                  ...
                  else if( itr->last_payout > fc::time_point_sec() )
                  {
                     modify( *itr, [&]( comment_object& c )
                     {
                        c.cashout_time = c.last_payout + WORTH_SECOND_CASHOUT_WINDOW;
                     });
                  }
               }
            }

            modify( get< account_authority_object, by_account >( WORTH_MINER_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( WORTH_NULL_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( WORTH_TEMP_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });
         ...
         break;
      case WORTH_HARDFORK_0_13:
         break;
      case WORTH_HARDFORK_0_14:
         break;
      case WORTH_HARDFORK_0_15:
         break;
      case WORTH_HARDFORK_0_16:
         {
            modify( get_feed_history(), [&]( feed_history_object& fho )
            {
               while( fho.price_history.size() > WORTH_FEED_HISTORY_WINDOW )
                  fho.price_history.pop_front();
            });
         }
         break;
      case WORTH_HARDFORK_0_17:
         {
            static_assert(
               WORTH_MAX_VOTED_WITNESSES_HF0 + WORTH_MAX_MINER_WITNESSES_HF0 + WORTH_MAX_RUNNER_WITNESSES_HF0 == WORTH_MAX_WITNESSES,
               "HF0 witness counts must add up to WORTH_MAX_WITNESSES" );
            static_assert(
               WORTH_MAX_VOTED_WITNESSES_HF17 + WORTH_MAX_MINER_WITNESSES_HF17 + WORTH_MAX_RUNNER_WITNESSES_HF17 == WORTH_MAX_WITNESSES,
               "HF17 witness counts must add up to WORTH_MAX_WITNESSES" );

            modify( get_witness_schedule_object(), [&]( witness_schedule_object& wso )
            {
               wso.max_voted_witnesses = WORTH_MAX_VOTED_WITNESSES_HF17;
               wso.max_miner_witnesses = WORTH_MAX_MINER_WITNESSES_HF17;
               wso.max_runner_witnesses = WORTH_MAX_RUNNER_WITNESSES_HF17;
            });
            ...
            {
               rfo.name = WORTH_POST_REWARD_FUND_NAME;
               rfo.last_update = head_block_time();
               rfo.content_constant = WORTH_CONTENT_CONSTANT_HF0;
               rfo.percent_curation_rewards = WORTH_1_PERCENT * 25;
               rfo.percent_content_rewards = WORTH_100_PERCENT;
               rfo.reward_balance = gpo.total_reward_fund_worth;

               rfo.recent_claims = WORTH_HF_17_RECENT_CLAIMS;
            ...
            modify( gpo, [&]( dynamic_global_property_object& g )
            {
               g.total_reward_fund_worth = asset( 0, WORTH_SYMBOL );
            ...
            */
            ...
            root_posts.reserve( WORTH_HF_17_NUM_POSTS );
            vector< const comment_object* > replies;
            replies.reserve( WORTH_HF_17_NUM_REPLIES );
            ...
            for( const auto& itr : root_posts )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( c.created + WORTH_CASHOUT_WINDOW_SECONDS, c.cashout_time );
               });
            }
            ...
            for( const auto& itr : replies )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( calculate_discussion_payout_time( c ), c.created + WORTH_CASHOUT_WINDOW_SECONDS );
               });
            }
         }
         break;
      case WORTH_HARDFORK_0_18:
         break;
      case WORTH_HARDFORK_0_19:
         {
            modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
            {
               gpo.vote_power_reserve_rate = WORTH_REDUCED_VOTE_POWER_RATE;
            });

            modify( get< reward_fund_object, by_name >( WORTH_POST_REWARD_FUND_NAME ), [&]( reward_fund_object &rfo )
            {

               rfo.recent_claims = WORTH_HF_19_RECENT_CLAIMS;
         ...
         break;
      case WORTH_HARDFORK_0_20:
         {
            modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
            {
               gpo.delegation_return_period = WORTH_DELEGATION_RETURN_PERIOD_HF20;
               gpo.reverse_auction_seconds = WORTH_REVERSE_AUCTION_WINDOW_SECONDS_HF20;
               gpo.wbd_stop_percent = WORTH_WBD_STOP_PERCENT_HF20;
               gpo.wbd_start_percent = WORTH_WBD_START_PERCENT_HF20;
            ...
            for( const auto& witness : wso.current_shuffled_witnesses )
            {
               ...
               if( witness != account_name_type() )
               {
                  ...
                  {
                     w.props.account_creation_fee = asset( w.props.account_creation_fee.amount * WORTH_CREATE_ACCOUNT_WITH_WORTH_MODIFIER, WORTH_SYMBOL );
                  });
               }
            }
                    modify( get< reward_fund_object, by_name >( WORTH_POST_REWARD_FUND_NAME ), [&]( reward_fund_object& rfo )
         {
            rfo.percent_curation_rewards = 50 * WORTH_1_PERCENT;
            rfo.author_reward_curve = linear;
            rfo.curation_reward_curve = square_root;
            rfo.content_constant = WORTH_CONTENT_CONSTANT_HF21;

            rfo.recent_claims = WORTH_HF21_CONVERGENT_LINEAR_RECENT_CLAIMS;

         });
            modify( wso, [&]( witness_schedule_object& wso )
            {
               wso.median_props.account_creation_fee = asset( wso.median_props.account_creation_fee.amount * WORTH_CREATE_ACCOUNT_WITH_WORTH_MODIFIER, WORTH_SYMBOL );
            });
         }
         break;
      case WORTH_HARDFORK_0_21:
      {
         modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.wps_fund_percent = WORTH_PROPOSAL_FUND_PERCENT_HF21;
            gpo.content_reward_percent = WORTH_CONTENT_REWARD_PERCENT_HF21;
            gpo.downvote_pool_percent = WORTH_DOWNVOTE_POOL_PERCENT_HF21;
            gpo.reverse_auction_seconds = WORTH_REVERSE_AUCTION_WINDOW_SECONDS_HF21;
         });

         auto account_auth = find< account_authority_object, by_account >( WORTH_TREASURY_ACCOUNT );
         if( account_auth == nullptr )
            create< account_authority_object >( [&]( account_authority_object& auth )
            {
               auth.account = WORTH_TREASURY_ACCOUNT;
               auth.owner.weight_threshold = 1;
               auth.active.weight_threshold = 1;
               auth.posting.weight_threshold = 1;
            });
         ...
         modify( get_account( WORTH_TREASURY_ACCOUNT ), [&]( account_object& a )
         {
            a.recovery_account = WORTH_TREASURY_ACCOUNT;
         });

         auto rec_req = find< account_recovery_request_object, by_account >( WORTH_TREASURY_ACCOUNT );
         if( rec_req )
            remove( *rec_req );

         auto change_request = find< change_recovery_account_request_object, by_account >( WORTH_TREASURY_ACCOUNT );
         if( change_request )
      ...
      case WORTH_WMT_HARDFORK:
      {
         ...
         modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.required_actions_partition_percent = 25 * WORTH_1_PERCENT;
         });
...
void database::validate_invariants()const
{
   try
   {
      ...
      asset total_supply = asset( 0, WORTH_SYMBOL );
      asset total_wbd = asset( 0, WBD_SYMBOL );
      asset total_vesting = asset( 0, VESTS_SYMBOL );
      asset pending_vesting_worth = asset( 0, WORTH_SYMBOL );
      ...
      for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
      {
         ...
         total_supply += itr->reward_worth_balance;
         total_wbd += itr->wbd_balance;
         total_wbd += itr->savings_wbd_balance;
         total_wbd += itr->reward_wbd_balance;
         total_vesting += itr->vesting_shares;
         total_vesting += itr->reward_vesting_balance;
         pending_vesting_worth += itr->reward_vesting_worth;
         total_vsf_votes += ( itr->proxy == WORTH_PROXY_TO_SELF_ACCOUNT ?
                                 itr->witness_vote_weight() :
                                 ( WORTH_MAX_PROXY_RECURSION_DEPTH > 0 ?
                                      itr->proxied_vsf_votes[WORTH_MAX_PROXY_RECURSION_DEPTH - 1] :
      ...
      {
         if( itr->amount.symbol == WORTH_SYMBOL )
            total_supply += itr->amount;
         else if( itr->amount.symbol == WBD_SYMBOL )
            total_wbd += itr->amount;
         else
      ...
      {
         if( itr->sell_price.base.symbol == WORTH_SYMBOL )
         {
            total_supply += asset( itr->for_sale, WORTH_SYMBOL );
         }
         else if ( itr->sell_price.base.symbol == WBD_SYMBOL )
         {
            total_wbd += asset( itr->for_sale, WBD_SYMBOL );
         }
      }
      ...
      {
         total_supply += itr->worth_balance;
         total_wbd += itr->wbd_balance;

         if( itr->pending_fee.symbol == WORTH_SYMBOL )
            total_supply += itr->pending_fee;
         else if( itr->pending_fee.symbol == WBD_SYMBOL )
            total_wbd += itr->pending_fee;
         else
            FC_ASSERT( false, "found escrow pending fee that is not WBD or WORTH" );
      }
      ...
      for( auto itr = savings_withdraw_idx.begin(); itr != savings_withdraw_idx.end(); ++itr )
      {
         if( itr->amount.symbol == WORTH_SYMBOL )
            ...
            FC_ASSERT( false, "found savings withdraw that is not WBD or WORTH" );
      }
...
#ifdef WORTHEUM_ENABLE_WMT
      const auto& wmt_contribution_idx = get_index< wmt_contribution_index, by_id >();

      for ( auto itr = wmt_contribution_idx.begin(); itr != wmt_contribution_idx.end(); ++itr )
      {
         total_supply += itr->contribution;
      }
#endif

      total_supply += gpo.total_vesting_fund_worth + gpo.total_reward_fund_worth + gpo.pending_rewarded_vesting_worth;

      FC_ASSERT( gpo.current_supply == total_supply, "", ("gpo.current_supply",gpo.current_supply)("total_supply",total_supply) );
      FC_ASSERT( gpo.current_wbd_supply + gpo.init_wbd_supply == total_wbd, "", ("gpo.current_wbd_supply",gpo.current_wbd_supply)("total_wbd",total_wbd) );
      ...
      FC_ASSERT( gpo.pending_rewarded_vesting_worth == pending_vesting_worth, "", ("pending_rewarded_vesting_worth",gpo.pending_rewarded_vesting_worth)("pending_vesting_worth", pending_vesting_worth));

      FC_ASSERT( gpo.virtual_supply >= gpo.current_supply );
      if ( !get_feed_history().current_median_history.is_null() )
      {
         FC_ASSERT( gpo.current_wbd_supply * get_feed_history().current_median_history + gpo.current_supply
            == gpo.virtual_supply, "", ("gpo.current_wbd_supply",gpo.current_wbd_supply)("get_feed_history().current_median_history",get_feed_history().current_median_history)("gpo.current_supply",gpo.current_supply)("gpo.virtual_supply",gpo.virtual_supply) );
      }
   }
...
/**
 * WMT version of validate_invariants.
 */
void database::validate_wmt_invariants()const
{
   try
   {
      // Get total balances.
      ...
      // - Process regular balances, collecting WMT counterparts of 'balance' & 'vesting_shares'.
      ...
      // - Process reward balances, collecting WMT counterparts of 'reward_worth_balance', 'reward_vesting_balance' & 'reward_vesting_worth'.
      ...
      for( auto itr = limit_order_idx.begin(); itr != limit_order_idx.end(); ++itr )
      {
         if( itr->sell_price.base.symbol.space() == asset_symbol_type::wmt_nai_space )
         {
      ...
      // - Reward funds
#pragma message( "TODO: Add reward_fund_object iteration here once they support WMTs." )

      // - Escrow & savings - no support of WMT is expected.

      // Do the verification of total balances.
      auto itr = get_index< wmt_token_index, by_id >().begin();
      auto end = get_index< wmt_token_index, by_id >().end();
      for( ; itr != end; ++itr )
      {
         const wmt_token_object& wmt = *itr;
         asset_symbol_type vesting_symbol = wmt.liquid_symbol.get_paired_symbol();
         auto totalIt = theMap.find( wmt.liquid_symbol );
         // Check liquid WMT supply.
         asset total_liquid_supply = totalIt == theMap.end() ? asset(0, wmt.liquid_symbol) :
            ( totalIt->second.liquid + totalIt->second.pending_liquid );
         total_liquid_supply += asset( wmt.total_vesting_fund_wmt, wmt.liquid_symbol )
                             /*+ gpo.total_reward_fund_worth */
                             + asset( wmt.pending_rewarded_vesting_wmt, wmt.liquid_symbol );
#pragma message( "TODO: Supplement ^ once WMT rewards are implemented" )
         FC_ASSERT( asset(wmt.current_supply, wmt.liquid_symbol) == total_liquid_supply,
                    "", ("wmt current_supply",wmt.current_supply)("total_liquid_supply",total_liquid_supply) );
         // Check vesting WMT supply.
         ...
         asset wmt_vesting_supply = asset(wmt.total_vesting_shares + wmt.pending_rewarded_vesting_shares, vesting_symbol);
         FC_ASSERT( wmt_vesting_supply == total_vesting_supply,
                    "", ("wmt vesting supply",wmt_vesting_supply)("total_vesting_supply",total_vesting_supply) );
         // Check pending_vesting_value
         asset pending_vesting_value = totalIt == theMap.end() ? asset(0, wmt.liquid_symbol) : totalIt->second.pending_vesting_value;
         FC_ASSERT( asset(wmt.pending_rewarded_vesting_wmt, wmt.liquid_symbol) == pending_vesting_value, "",
            ("wmt pending_rewarded_vesting_wmt", wmt.pending_rewarded_vesting_wmt)("pending_vesting_value", pending_vesting_value));
      }
   }
   FC_CAPTURE_LOG_AND_RETHROW( (head_block_num()) );
}
   ...
   try
   {
      ...
      for( const auto& account : get_index< account_index, by_id >() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.vesting_withdraw_rate  = asset( a.to_withdraw / WORTH_VESTING_WITHDRAW_INTERVALS_PRE_HF_16, VESTS_SYMBOL );
            if( a.vesting_withdraw_rate.amount == 0 )
               a.vesting_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < WORTH_MAX_PROXY_RECURSION_DEPTH; ++i )
...
// Low memory nodes only need immediate child count, full nodes track total children
         ...
         while( parent )
         {
            ...
            if( parent->parent_author != WORTH_ROOT_POST_PARENT )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != WORTH_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != WORTH_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.name, account_name_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.name )
