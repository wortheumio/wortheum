...
Building Wortheum requires 8GB of RAM.
...
Builds worths to be a consensus-only low memory node. Data and fields not
...
### BUILD_WORTHEUM_TESTNET=[OFF/ON]

Builds Wortheum for use in a private testnet. Also required for building unit tests.
...
For Ubuntu 16.04 users, after installing the right packages with `apt` Wortheum
...
Wortheum requires Boost 1.58 (as in Ubuntu 16.04) and works with versions up to 1.60 (including).
So building Wortheum on Ubuntu 14.04 requires downloading and installing a more recent
version of Boost.
...
Ubuntu 14.04) when compiling in C++11 mode (which Wortheum does).
...
Then the instructions are the same as for wortheum:
...
wortheum. Until then, this will allow you to install boost 1.60.0.
...
e.g.:
    ...
    make -j$(sysctl -n hw.logicalcpu) worths

This will only build `worths`.
