...
of a running Wortheum Debug Node. The Debug Node is a plugin for Wortheum that allows realtime
...
[Python Wortheum library](https://github.com/xeroc/python-wortheumlib).
...
To use the script include `from worthdebugnode import DebugNode`

There are a couple of examples already made that you can try modifying yourself.

[debug_hardforks.py](https://github.com/wortheumio/wortheum/python_scripts/tests/debug_hardforks.py)
...
[debugnode.py](https://github.com/wortheumio/wortheum/python_scripts/worthdebugnode/debugnode.py#L212)
...
``` Python
debug_node = DebugNode( str( worths ), str( data_dir ) )
with debug_node:
...
specify the worths binary location and a populated data directory. For most configurations
this will be `programs/worths/worths` and `witness_node_data_dir` respectively, from the
git root directory for Wortheum.
