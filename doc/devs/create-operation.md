...
This is developer documentation for creating brand-new operations on the WORTHEUM blockchain.

- (1) Define `wmt_elevate_account_operation` structure in `wmt_operations.hpp`
- (2) Create `FC_REFLECT` definition for the operation struct.
- (3) Implement `validate()` for the operation struct.
- (4) Add operation to `worth::protocol::operation`
...
- (4a) The file `operations.hpp` defines the `worth::protocol::operation`
...
- (5a) You must add `WORTH_DEFINE_EVALUATOR` macro in `evaluator.hpp` to
...
[implementation](https://github.com/wortheumio/wortheum/blob/a0a69b10189d93a9be4da7e7fd9d5358af956e34/libraries/chain/database.cpp#L364)
of `get_account(name)`.
...
- (7a) Unit tests for WMT functionality should go in `wmt_tests.cpp`.
...
- (9a) Add `wmt_token_object_type` to `enum object_type` in `worth_objects.hpp` and add
to `FC_REFLECT_ENUM` bubble list at the bottom of that file
- (9b) Declare (but do not define) `class wmt_token_object;` in `worth_objects.hpp`
- (9c) Define `typedef oid< wmt_token_object > wmt_token_id_type` in `worth_objects.hpp`
- (9d) Create object header file (one header file per object) in `wmt_objects` directory.
Include the new header from `wmt_objects.hpp`.
- (9e) All WMT objects are consensus, and therefore should exist in `worth::chain` namespace
- (9f) The object class should subclass `object< wmt_token_object_type, wmt_token_object >`
...
- (9o) Call `add_core_index< wmt_token_index >(*this);` in `database::initialize_indexes()` to
...
defined by an `enum` in `worth_object_types.hpp`, any new objects must be added here.  In SQL
terms, if we imagine each *database table* has an integer ID, then `wmt_token_object_type` is
the ID value that refers to the `wmt_token_object` *table*.
...
alias `typedef oid< wmt_token_object > wmt_id_type` is added to `worth_object_types.hpp`.

- (9f) The `wmt_token_object` class subclasses
`chainbase::object< wmt_token_object_type, wmt_token_object >`.  This is the
...
`typedef oid< wmt_token_object > id_type;`

As a consequence of the above, `wmt_token_id_type`, `oid< wmt_token_object >` and
`wmt_token_object::id_type` all refer to a type representing an integer object ID, with a
compile-time "note" attached that this object ID is a primary key for the `wmt_token_object`
database table.  The table ID of that database table is an integer value given by
`wmt_token_object_type` or `wmt_token_object::type_id`.
...
is designed as a reusable library, not tightly coupled to Wortheum, it contains no reference
to any `worth` namespaces.  So the name `by_id` must refer to `chainbase::by_id`.  If you
define a `struct by_id;` in the `worth::chain` namespace, the result will be that every
...
- (9m) The template definition of `wmt_token_index` looks intimidating (and the definitions of
...
So the `wmt_token_index` definition is basically a DDL which describes the available indexes --
...
- (9m) All indexes used in Wortheum must be `ordered_unique`.  In theory, hashed or non-unique
...
`db.get< wmt_token_object, by_control_account >( "alice" );` which throws an exception
...
`auto& idx = get_index< wmt_token_index >().indices().get< by_control_account >()` to get a reference
...
- (10) The following discussion is specific to the `wmt_token_object` example code, and
isn't directly applicable to other objects.

For `wmt_token_index` two single-column indexes are defined, `by_id`
on the `id` field, and `by_control_account` on the `control_account` field.

Now that an object is created for each WMT, the `is_wmt` field in the database describing
whether an object has an associated WMT is redundant information and can be removed.  We want to
continue to provide the same interface to JSON-RPC clients, so we keep the `is_wmt` field in the RPC
object.  But we now fill it in based on whether an `wmt_token_object` exists with a matching
`control_account`.

Also, we delete the `is_wmt` check in the evaluator.  But the unit test code which tests that
