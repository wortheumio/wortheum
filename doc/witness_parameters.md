...
The role of a witness in the Wortheum Blockchain is to verify incoming transactions, produce blocks when scheduled, and partake in the Wortheum governance model by voting on several parameters.

These parameters control various aspects of the operation of the blockchain that are not easily defined in code at compile time. One example is the WORTHEUM price feed that defines the conversion rate between WORTHEUM and WBD.
...
This operation was added in Wortheum v0.20.0 to replace the `witness_update_operation` which was not easily extendable. While it is recommended to use `witness_set_properties_operation`, `witness_update_operation` will continue to work.

## Properties

### account_creation_fee

This is the fee in WORTHEUM that must be paid to create an account. This field must be non-negative.
...
The annual interest rate paid to WBD holders. WBD interest is compounded on balance changes, no more than once every 30 days.

### wbd_exchange_rate

The exchange rate for WORTH/WBD to be used for printing WBD as rewards as well as WBD->WORTH conversions.
