...
struct tx_signing_result
{
   ...
   worth::protocol::public_key_type key;
   worth::protocol::signature_type  sig;
};
...
int main(int argc, char** argv, char** envp)
{
   ...
   while( std::cin )
   {
      ...
      try
      {
         ...
         auto priv_key = worth::utilities::wif_to_key( sreq.wif );

         if(priv_key)
         {
            sres.sig = priv_key->sign_compact( sres.sig_digest );
            sres.key = worth::protocol::public_key_type( priv_key->get_public_key() );
