...
using worth::protocol::calc_percent_reward_per_block;
using worth::protocol::calc_percent_reward_per_round;
using worth::protocol::calc_percent_reward_per_hour;
...
rvec shows total number of WORTH satoshis created since genesis for:
...
- Missed blocks not modeled (lowers WORTH supply, small)
- Miner queue length very approximately modeled (assumed to go to 100 during the first blocks and then stay there) (may lower or raise WORTH supply, very small)
- Creation / destruction of WORTH used to back WBD not modeled (moves WORTH supply in direction opposite to changes in dollar value of 1 WORTH, large)
- Interest paid to WBD not modeled (raises WORTH supply, medium)
- Lost / forgotten private keys / wallets and deliberate burning of WORTH not modeled (lowers WORTH supply, unknown but likely small)
...
/*
#define WORTH_GENESIS_TIME                    (fc::time_point_sec(1458835200))
#define WORTH_MINING_TIME                     (fc::time_point_sec(1458838800))
#define WORTH_FIRST_CASHOUT_TIME              (fc::time_point_sec(1467590400))  /// July 4th
*/

   uint32_t liquidity_begin_block = (15000 - 12000) / 3;
   ...
   {
      uint32_t vesting_factor = (block_num < WORTH_START_VESTING_BLOCK) ? 0 : 9;

      share_type curate_reward   = calc_percent_reward_per_block< WORTH_CURATE_APR_PERCENT >( current_supply );
      reward_delta[ CURATE_OFF ] = std::max( curate_reward, WORTH_MIN_CURATE_REWARD.amount );
      reward_delta[ VCURATE_OFF ] = reward_delta[ CURATE_OFF ] * vesting_factor;

      share_type content_reward  = calc_percent_reward_per_block< WORTH_CONTENT_APR_PERCENT >( current_supply );
      reward_delta[ CONTENT_OFF ] = std::max( content_reward, WORTH_MIN_CONTENT_REWARD.amount );
      reward_delta[ VCONTENT_OFF ] = reward_delta[ CONTENT_OFF ] * vesting_factor;

      share_type producer_reward = calc_percent_reward_per_block< WORTH_PRODUCER_APR_PERCENT >( current_supply );
      reward_delta[ PRODUCER_OFF ] = std::max( producer_reward, WORTH_MIN_PRODUCER_REWARD.amount );
      ...
      if( (block_num % WORTH_MAX_WITNESSES) == 0 )
         ++pow_deficit;

      if( pow_deficit > 0 )
      {
         pow_reward = calc_percent_reward_per_round< WORTH_POW_APR_PERCENT >( current_supply );
         pow_reward = std::max( pow_reward, WORTH_MIN_POW_REWARD.amount );
         if( block_num < WORTH_START_MINER_VOTING_BLOCK )
            pow_reward *= WORTH_MAX_WITNESSES;
         --pow_deficit;
      }
      ...
      if( (block_num > liquidity_begin_block) && ((block_num % WORTH_LIQUIDITY_REWARD_BLOCKS) == 0) )
      {
         liquidity_reward = calc_percent_reward_per_hour< WORTH_LIQUIDITY_APR_PERCENT >( current_supply );
         liquidity_reward = std::max( liquidity_reward, WORTH_MIN_LIQUIDITY_REWARD.amount );
      }
   ...
   for( uint32_t b=1; b<10*WORTH_BLOCKS_PER_YEAR; b++ )
   {
