...
target_link_libraries( {plugin_provider}_{plugin_name} wortheum_app wortheum_chain wortheum_protocol )
...
{{
   public:
      {plugin_name}_api( const worth::app::api_context& ctx );
...
class {plugin_name}_plugin : public worth::app::plugin
{{
   public:
      {plugin_name}_plugin( worth::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <worth/app/api_context.hpp>
#include <worth/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( worth::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      worth::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( worth::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const worth::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( worth::app::application& app );
...
{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( worth::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( worth::app::application* app )
   : plugin(app)
{{
...
WORTH_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
...
def main(argv):
    ...
    parser.add_argument("provider", help="Name of plugin provider (worth for plugins developed by Wortheum)")
