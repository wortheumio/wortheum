...
void check_variant( const std::string& s, const worth::protocol::fixed_string_impl< Storage >& fs )
{
   fc::variant vs, vfs;
   fc::to_variant( s, vs );
   ...
   std::string s2;
   worth::protocol::fixed_string_impl< Storage > fs2;
...
template< typename Storage >
void check_pack( const std::string& s, const worth::protocol::fixed_string_impl< Storage >& fs )
{
   ...
   ss.seekg(0);
   worth::protocol::fixed_string_impl< Storage > unpacked;
...
int main( int argc, char** argv, char** envp )
{
   ...
   std::vector< worth::protocol::fixed_string< 16 > > all_fixed_string_16s;
   ...
   */
   ...
   for( size_t i=0; i<all_strings.size(); i++ )
   {
      ...
      worth::protocol::fixed_string< 16 > fs(s);
      std::string sfs = fs;
      if( s != fs )
      {
      ...
      for( const uint32_t& j : sim_index[i] )
      {
         ...
         worth::protocol::fixed_string< 16 > ft(t);
         check( s, t, (s< t) == (fs< ft) );
         check( s, t, (s<=t) == (fs<=ft) );
         check( s, t, (s> t) == (fs> ft) );
         check( s, t, (s>=t) == (fs>=ft) );
   ...
   std::vector< worth::protocol::fixed_string< 24 > > all_fixed_string_24s;
   ...
   {
      const std::string& s = all_strings[i];
      worth::protocol::fixed_string< 24 > fs(s);
      std::string sfs = fs;
      if( s != fs )
      {
         std::cout << "problem on " << s << std::endl;
      ...
      {
         const std::string& t = all_strings[j];
         worth::protocol::fixed_string< 24 > ft(t);
         check( s, t, (s< t) == (fs< ft) );
         check( s, t, (s<=t) == (fs<=ft) );
         check( s, t, (s> t) == (fs> ft) );
         check( s, t, (s>=t) == (fs>=ft) );
   ...
   errors = 0;
   all_strings = std::vector< std::string >();
   std::vector< worth::protocol::fixed_string< 32 > > all_fixed_string_32s;
   sim_index = std::vector< std::vector< uint32_t > >();
   ...
   {
      const std::string& s = all_strings[i];
      worth::protocol::fixed_string< 32 > fs(s);
      std::string sfs = fs;
      if( s != fs )
      {
         std::cout << "problem on " << s << std::endl;
      ...
      {
         const std::string& t = all_strings[j];
         worth::protocol::fixed_string< 32 > ft(t);
         check( s, t, (s< t) == (fs< ft) );
         check( s, t, (s<=t) == (fs<=ft) );
         check( s, t, (s> t) == (fs> ft) );
         check( s, t, (s>=t) == (fs>=ft) );
